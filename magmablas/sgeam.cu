/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/zgeam.cu, normal z -> s, Fri Aug 25 13:18:32 2023
    ...
    magma_queue_t queue )
{
    ...
    if ( MAGMA_S_EQUAL( alpha, MAGMA_S_ZERO ) && 
        ...
        sgeam_kernel_nn<<<grid, threads, 0, queue->cuda_stream() >>>
            (m, n,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    }
    else if ( transA == MagmaNoTrans )
        sgeam_kernel_nc<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else if ( transB == MagmaNoTrans )
        sgeam_kernel_cn<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else 
        sgeam_kernel_cc<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
}
