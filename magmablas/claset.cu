/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/zlaset.cu, normal z -> c, Fri Aug 25 13:18:38 2023
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if (uplo == MagmaLower) {
            claset_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
        else if (uplo == MagmaUpper) {
            claset_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
        else {
            claset_full_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if (uplo == MagmaLower) {
            claset_lower_kernel_vbatched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
        else if (uplo == MagmaUpper) {
            claset_upper_kernel_vbatched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
        else {
            claset_full_kernel_vbatched<<< grid, threads, 0, queue->cuda_stream() >>>
            (m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
    }
}
