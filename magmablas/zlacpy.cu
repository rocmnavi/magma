    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            zlacpy_lower_kernel_vbatched<<< grid, threads, 0, queue->cuda_stream() >>>
            ( m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else if ( uplo == MagmaUpper ) {
            zlacpy_upper_kernel_vbatched<<< grid, threads, 0, queue->cuda_stream() >>>
            ( m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else {
            zlacpy_full_kernel_vbatched <<< grid, threads, 0, queue->cuda_stream() >>>
            ( m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
    }
}
