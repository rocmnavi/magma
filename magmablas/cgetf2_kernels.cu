/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/zgetf2_kernels.cu, normal z -> c, Fri Aug 25 13:18:49 2023
*/
    ...
    magma_int_t step, magma_int_t gbstep, magma_queue_t queue)
{
    ...
    if( length <= 15360 ) {
        ...
        cublasIcamax(queue->cublas_handle(), length, x, 1, (int*)(ipiv));
        magma_cpivcast<<< 1, 1, 0, queue->cuda_stream() >>>( ipiv );
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        switch(n){
            case  1: cscal_cgeru_1d_kernel_batched< 1><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  2: cscal_cgeru_1d_kernel_batched< 2><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  3: cscal_cgeru_1d_kernel_batched< 3><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  4: cscal_cgeru_1d_kernel_batched< 4><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  5: cscal_cgeru_1d_kernel_batched< 5><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  6: cscal_cgeru_1d_kernel_batched< 6><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  7: cscal_cgeru_1d_kernel_batched< 7><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            case  8: cscal_cgeru_1d_kernel_batched< 8><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);break;
            default: cscal_cgeru_1d_generic_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>(m, n, dA_array+i, ai, aj, lda, info_array+i, step, gbstep);
        }
    }
    return 0;
}
    ...
    magma_queue_t queue)
{
    ...
    */
    ...
    dim3 threads(tbx, 1, 1);
    switch(n){
        case 1: cscal_cgeru_1d_kernel_native<1><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 2: cscal_cgeru_1d_kernel_native<2><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 3: cscal_cgeru_1d_kernel_native<3><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 4: cscal_cgeru_1d_kernel_native<4><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 5: cscal_cgeru_1d_kernel_native<5><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 6: cscal_cgeru_1d_kernel_native<6><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 7: cscal_cgeru_1d_kernel_native<7><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        case 8: cscal_cgeru_1d_kernel_native<8><<<grid, threads, 0, queue->cuda_stream()>>>( m, dA, lda, info, step, gbstep);break;
        default: cscal_cgeru_1d_generic_kernel_native<<<grid, threads, 0, queue->cuda_stream()>>>( m, n, dA, lda, info, step, gbstep);
    }
    return 0;
}
    ...
    magma_queue_t queue )
{
    ...
    cudaError_t e = cudaLaunchKernel((void*)cgetf2_fused_kernel_batched<N>, grid, threads, kernel_args, shmem, queue->cuda_stream());
    if( e != cudaSuccess ) {
