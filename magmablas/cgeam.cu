/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/zgeam.cu, normal z -> c, Fri Aug 25 13:18:32 2023
    ...
    magma_queue_t queue )
{
    ...
    if ( MAGMA_C_EQUAL( alpha, MAGMA_C_ZERO ) && 
        ...
        cgeam_kernel_nn<<<grid, threads, 0, queue->cuda_stream() >>>
            (m, n,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    }
    else if ( transA == MagmaNoTrans )
        cgeam_kernel_nc<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else if ( transB == MagmaNoTrans )
        cgeam_kernel_cn<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else 
        cgeam_kernel_cc<<<grid, threads, 0, queue->cuda_stream() >>>
            (n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
}
