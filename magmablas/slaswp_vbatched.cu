/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/zlaswp_vbatched.cu, normal z -> s, Fri Aug 25 13:18:57 2023

       @author Ahmad Abdelfattah
*/
#include "magma_internal.h"
#include "batched_kernel_param.h"
#include "slaswp_device.cuh"
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        slaswp_left_rowserial_kernel_vbatched
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        slaswp_right_rowserial_kernel_vbatched
        <<< grid, min_BLK_SIZE_n, 0, queue->cuda_stream() >>>
        (n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_offset, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        slaswp_left_rowparallel_kernel_vbatched
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        size_t shmem = sizeof(float) * height * width;
        slaswp_right_rowparallel_kernel_vbatched
        <<< grid, height, shmem, queue->cuda_stream() >>>
        ( n, width, M, N, dA_array, Ai, Aj, ldda, pivinfo_array, pivinfo_i, k1, k2);
    }
}
