/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas/ztrtri_diag_batched.cu, normal z -> c, Fri Aug 25 13:18:53 2023
    ...
    magma_int_t resetozero, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            ...
            ctrtri_diag_lower_kernel_batched<<< diaggrid, IB, 0, queue->cuda_stream() >>>
            ...
            for( int jb=IB; jb < NB; jb *= 2 ) {
                ...
                switch (jb) {
                    case 16:
                        triple_cgemm16_part1_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm16_part2_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    case 32:
                        triple_cgemm32_part1_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm32_part2_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    case 64:
                        triple_cgemm64_part1_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm64_part2_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    default:
                        triple_cgemm_above64_part1_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm_above64_part2_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm_above64_part3_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                }
                if ( kb >= n ) break;
            }
            ...
            dim3 diaggrid( nblocks, 1, ibatch );  // emulate 3D grid
            ctrtri_diag_upper_kernel_batched<<< diaggrid, IB, 0, queue->cuda_stream() >>>
            ( diag, n, dA_array+i, ldda, dinvA_array+i );
            ...
            for( int jb=IB; jb < NB; jb *= 2 ) {
                ...
                switch (jb) {
                    case 16:
                        triple_cgemm16_part1_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm16_part2_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    case 32:
                        triple_cgemm32_part1_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm32_part2_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    case 64:
                        triple_cgemm64_part1_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm64_part2_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                    default:
                        triple_cgemm_above64_part1_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm_above64_part2_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        triple_cgemm_above64_part3_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA_array+i, ldda, dinvA_array+i, jb, npages );
                        break;
                }
                if ( kb >= n ) break;
            }
