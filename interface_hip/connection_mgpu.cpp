    ...
    magma_int_t *ncmplx, magma_int_t ngpu)
{
    ...
    for( magma_int_t d = 0; d < ngpu; ++d ) {
        ...
        magma_setdevice( d );
        hipGetDeviceProperties( &prop, int(d) );
        ...
        for( magma_int_t d2 = d+1; d2 < ngpu; ++d2 ) {
            // check for unified memory & enable peer memory access between all GPUs.
            magma_setdevice( d2 );
            hipGetDeviceProperties( &prop, int(d2) );
            #ifdef MAGMA_HAVE_CUDA
            if ( ! prop.unifiedAddressing ) {
            #elif defined(MAGMA_HAVE_HIP)
            // assume it does, HIP does not have support for checking this
            ...
            /* TODO err = */ hipDeviceCanAccessPeer( &samecomplex, int(d), int(d2) );
            ...
            if (samecomplex == 1) {
                ...
                // FOR d:
                magma_setdevice( d );
                err = hipDeviceEnablePeerAccess( int(d2), 0 );
                ...
                if ( err != hipSuccess && err != hipErrorPeerAccessAlreadyEnabled ) {
                    printf( "device %lld hipDeviceEnablePeerAccess error %lld\n",
                ...
                // FOR d2:
                magma_setdevice( d2 );
                err = hipDeviceEnablePeerAccess( int(d), 0 );
                //printf("enabling devide %lld ==> %lld  error %lld\n",
