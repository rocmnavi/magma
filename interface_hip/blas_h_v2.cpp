    ...
    magma_queue_t queue )
{
    ...
    if( arch >= 530 ) {
        ...
        hipblasSetMathMode(queue->hipblas_handle(), hipBLAS_TENSOR_OP_MATH);
        #endif
        ...
        hipblasSetMathMode(queue->hipblas_handle(), hipBLAS_DEFAULT_MATH);
        #endif
    }
    else {
    ...
    magma_queue_t queue )
{
    ...
    if( arch >= 530 ) {
        ...
        hipblasGemmEx( queue->hipblas_handle(),
                      ...
                      int(m), int(n), int(k),
                      &alpha, dA,        HIPBLAS_R_16F, int(ldda),
                              dB,        HIPBLAS_R_16F, int(lddb),
                      &beta,  dC,        HIPBLAS_R_32F, int(lddc),
                      HIPBLAS_R_32F, hipBLAS_GEMM_DEFAULT_TENSOR_OP);
        #if CUDA_VERSION >= 9000
        // roll back to default
        hipblasSetMathMode(queue->hipblas_handle(), hipBLAS_DEFAULT_MATH);
        #endif
