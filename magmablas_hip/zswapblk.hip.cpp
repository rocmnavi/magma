    ...
    magma_queue_t queue )
{
    ...
    if ( order == MagmaColMajor ) {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_zswapblkcm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
    else {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_zswapblkrm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
}
