    ...
    magma_queue_t queues[] )
{
    ...
    for (dev=0; dev < ngpu; dev++) {
        ...
        if ( upper ) {
            hipLaunchKernelGGL(zhemv_kernel_U_mgpu, dim3(grid), dim3(threads), 0, queues[dev]->hip_stream() , 
                n, dA_dev, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
            
            hipLaunchKernelGGL(zhemv_kernel_U_mgpu_sum, dim3(grid), dim3(threads_sum), 0, queues[dev]->hip_stream() , 
                n, alpha, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
        }
        else {
            hipLaunchKernelGGL(zhemv_kernel_L_mgpu, dim3(grid), dim3(threads), 0, queues[dev]->hip_stream() , 
                n, dA_dev, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
            
            hipLaunchKernelGGL(zhemv_kernel_L_mgpu_sum, dim3(grid), dim3(threads_sum), 0, queues[dev]->hip_stream() , 
                n, alpha, ldda, dx_dev, 1, dwork_dev,
                new_gpu_id, ngpu, block_offset );
        }
    }
