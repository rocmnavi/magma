...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlag2c.hip.cpp, mixed zc -> ds, Thu Jan 25 22:57:51 2024
       @author Mark Gates
*/
...
// static __device__ int magma_dlag2s_flag = 0;
__device__ int magma_dlag2s_flag = 0;
...
/***************************************************************************//**
    ...
    RMAX. If not, the conversion is aborted and a magma_dlag2s_flag is raised.
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    hipMemcpyToSymbol(HIP_SYMBOL(magma_dlag2s_flag), info, sizeof(magma_dlag2s_flag) );    // magma_dlag2s_flag = 0
    
    hipLaunchKernelGGL(dlag2s_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, A, lda, SA, ldsa, rmax );
    
    hipMemcpyFromSymbol(info, HIP_SYMBOL(magma_dlag2s_flag), sizeof(magma_dlag2s_flag) );  // info = magma_dlag2s_flag
}
