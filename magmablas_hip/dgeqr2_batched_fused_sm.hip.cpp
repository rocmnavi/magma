...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeqr2_batched_fused_sm.hip.cpp, normal z -> d, Mon Oct 28 11:12:18 2024
*/
    ...
    magma_int_t *info_array, magma_int_t batchCount)
{
    ...
    for(int j = 0; j < N; j++){
        ...
        dgeqr2_compute_norm(M-j-1, &sA(j+1,j), snorm, tx, ntx);
        // there is a sync at the end of dgeqr2_compute_norm
        norm_no_alpha = snorm[0];
        norm = norm_no_alpha + MAGMA_D_REAL(alpha) * MAGMA_D_REAL(alpha) + MAGMA_D_IMAG(alpha) * MAGMA_D_IMAG(alpha);
        norm = sqrt(norm);
        ...
        // have the diagonal
        alpha = (zero_nrm) ? alpha : MAGMA_D_MAKE(beta, MAGMA_D_ZERO);
