...
/*
   ...
   @generated from magmablas_hip/zgetf2_devicefunc.hip.hpp, normal z -> d, Mon Jul 15 16:58:31 2024
 */
                         ...
                         magma_int_t *info, int step, int gbstep)
{
    ...
    double rTmp = fabs(MAGMA_D_REAL( shared_y[0] ) ) + fabs( MAGMA_D_IMAG( shared_y[0] ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_D_ONE : MAGMA_D_DIV(MAGMA_D_ONE, shared_y[0]);
                         ...
                         magma_int_t *info, int step, int gbstep)
{
    ...
    double rTmp;
    rA   = dA[0];
    rTmp = fabs(MAGMA_D_REAL( rA ) ) + fabs( MAGMA_D_IMAG( rA ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_D_ONE : MAGMA_D_DIV(MAGMA_D_ONE, rA);
    rA  = dA[ gtx ];
    rA *= reg;
