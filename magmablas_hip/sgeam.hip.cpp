...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeam.hip.cpp, normal z -> s, Thu Jan 25 22:57:49 2024
...
/***************************************************************************//**
    Purpose
    -------
    sgeam adds/transposes matrices:
    ...
    @param[in]
    dA      REAL array, dimension (LDDA,k), where k is
    ...
    @param[in]
    dB      REAL array, dimension (LDDB,k), where k is
    ...
    magma_queue_t queue )
{
    ...
    if ( MAGMA_S_EQUAL( alpha, MAGMA_S_ZERO ) && 
         MAGMA_S_EQUAL(  beta, MAGMA_S_ZERO ) )
        // set to 0
        magmablas_slaset( MagmaFull, m, n, MAGMA_S_ZERO, MAGMA_S_ZERO, dC, lddc, queue );
    else if ( transA == MagmaNoTrans && transB == MagmaNoTrans ){
        dim3 grid( magma_ceildiv( m, NB ), magma_ceildiv( n, NB ) ); 
        hipLaunchKernelGGL(sgeam_kernel_nn, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    }
    else if ( transA == MagmaNoTrans )
        hipLaunchKernelGGL(sgeam_kernel_nc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else if ( transB == MagmaNoTrans )
        hipLaunchKernelGGL(sgeam_kernel_cn, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else 
        hipLaunchKernelGGL(sgeam_kernel_cc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
}
