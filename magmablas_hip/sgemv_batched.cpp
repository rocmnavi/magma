/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgemv_batched.cpp, normal z -> s, Thu Jan 25 22:58:00 2024
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(m == n && n <= 32) {
        info = magmablas_sgemv_batched_smallsq(
                trans, n,
                alpha, dA_array, ldda,
                       dx_array, incx,
                beta,  dy_array, incy,
...
/***************************************************************************//**
    ...
    @param[in]
    dA      Pointer to the first REAL array 'A' of DIMENSION ( ldda, n )
            in the batch
    ...
    @param[in]
    dx      Pointer to the first REAL array 'x' in the batch, of dimension
            n if trans == MagmaNoTrans
            m if trans == MagmaTrans or MagmaConjTrans
    ...
    @param[out]
    dy      Pointer to the first REAL array 'y' in the batch, of dimension
            m if trans == MagmaNoTrans
            n if trans == MagmaTrans or MagmaConjTrans
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(m == n && n <= 32) {
        info = magmablas_sgemv_batched_strided_smallsq(
                    trans, n,
