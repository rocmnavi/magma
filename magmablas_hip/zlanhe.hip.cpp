    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        hipLaunchKernelGGL(zlanhe_inf_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        hipLaunchKernelGGL(zlanhe_inf_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(zlanhe_max_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
    else {
        hipLaunchKernelGGL(zlanhe_max_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
}
