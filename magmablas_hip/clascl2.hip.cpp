...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlascl2.hip.cpp, normal z -> c, Thu Jan 25 22:57:54 2024
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        hipLaunchKernelGGL(clascl2_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaUpper) {
        hipLaunchKernelGGL(clascl2_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaFull) {
        hipLaunchKernelGGL(clascl2_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
}
