    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(zsymmetrize_tiles_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, mstride, nstride );
    }
    else {
        hipLaunchKernelGGL(zsymmetrize_tiles_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, mstride, nstride );
    }
}
