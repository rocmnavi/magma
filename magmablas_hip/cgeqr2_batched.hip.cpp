...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeqr2_batched.hip.cpp, normal z -> c, Mon Jul 15 16:57:46 2024
*/
...
#include "clarfg_devicesfunc.hip.hpp"
...
/******************************************************************************/
__global__
void cgeqr2_column_sm_kernel_batched(
        int m, int n,
        magmaFloatComplex** dA_array, magma_int_t Ai, magma_int_t Aj, magma_int_t lda,
        magmaFloatComplex **dtau_array, magma_int_t taui )
{
        ...
        magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    arginfo = magma_cgeqr2_fused_reg_batched(
    ...
    magma_int_t nthreads = magma_get_cgeqr2_fused_sm_batched_nthreads(m, n);
    arginfo = magma_cgeqr2_fused_sm_batched(
                m, n, dA_array, Ai, Aj, ldda,
                     ...
                     magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    arginfo = magma_cgeqr2_fused_batched(m, n, dA_array, Ai, Aj, ldda, dtau_array, taui, info_array, batchCount, queue);
    if ( arginfo == 0 ) return arginfo;
    ...
    // cgeqr2_sm_kernel_batched, cgeqr2_column_sm_kernel_batched, cgeqr2_kernel_batched
    magma_int_t static_shmem = (BLOCK_SIZE + 1 ) * ( sizeof(magmaFloatComplex) + sizeof(float) );

    // dynamic shared memory
    magma_int_t dynamic_shmem_sm_kernel        = sizeof(magmaFloatComplex) * m * k;
    magma_int_t dynamic_shmem_column_sm_kernel = sizeof(magmaFloatComplex) * m;
    ...
    if ( total_shmem_sm_kernel <= shmem_max) {
        cudaFuncSetAttribute(cgeqr2_sm_kernel_batched, cudaFuncAttributeMaxDynamicSharedMemorySize, dynamic_shmem_sm_kernel);
    }

    if ( total_shmem_column_sm_kernel <= shmem_max) {
        cudaFuncSetAttribute(cgeqr2_column_sm_kernel_batched, cudaFuncAttributeMaxDynamicSharedMemorySize, dynamic_shmem_column_sm_kernel);
    }
    #else
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( total_shmem_sm_kernel <= shmem_max ) {
            ...
            hipLaunchKernelGGL(cgeqr2_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(magmaFloatComplex)*(m*k), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
            ...
            hipLaunchKernelGGL(cgeqr2_column_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(magmaFloatComplex)*(m), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
        else {
            //not use dynamic shared memory at all
            hipLaunchKernelGGL(cgeqr2_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
    }
