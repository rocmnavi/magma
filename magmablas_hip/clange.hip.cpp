...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlange.hip.cpp, normal z -> c, Mon Jul 15 16:57:39 2024
       @author Mark Gates
*/
...
/* Based on clange_one_kernel code, above.
    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        hipLaunchKernelGGL(clange_inf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaMaxNorm ) {
        dim3 grid( magma_ceildiv( m, NB_X ) );
        hipLaunchKernelGGL(clange_max_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaOneNorm ) {
        dim3 grid( n );
        hipLaunchKernelGGL(clange_one_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        ...
        dim3 grid( n );
        hipLaunchKernelGGL(clange_fro_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
