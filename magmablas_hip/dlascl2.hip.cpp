...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlascl2.hip.cpp, normal z -> d, Mon Oct 28 11:12:08 2024
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        hipLaunchKernelGGL(dlascl2_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaUpper) {
        hipLaunchKernelGGL(dlascl2_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaFull) {
        hipLaunchKernelGGL(dlascl2_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
}
