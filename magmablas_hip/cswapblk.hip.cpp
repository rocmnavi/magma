...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zswapblk.hip.cpp, normal z -> c, Mon Oct 28 11:12:10 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( order == MagmaColMajor ) {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_cswapblkcm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
    else {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_cswapblkrm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
}
