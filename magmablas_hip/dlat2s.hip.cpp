...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlat2c.hip.cpp, mixed zc -> ds, Thu Jan 25 22:57:55 2024
       @author Mark Gates
*/
...
//static __device__ int magma_dlat2s_flag = 0;
__device__ int magma_dlat2s_flag = 0;
...
/***************************************************************************//**
    ...
    RMAX. If not, the conversion is aborted and a magma_dlat2s_flag is raised.
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    hipMemcpyToSymbol(HIP_SYMBOL(magma_dlat2s_flag), info, sizeof(magma_dlat2s_flag) );    // magma_dlat2s_flag = 0
    
    if (uplo == MagmaLower) {
        hipLaunchKernelGGL(dlat2s_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, rmax);
    }
    else if (uplo == MagmaUpper) {
        hipLaunchKernelGGL(dlat2s_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, rmax);
    }
    
    hipMemcpyFromSymbol(info, HIP_SYMBOL(magma_dlat2s_flag), sizeof(magma_dlat2s_flag) );  // info = magma_dlat2s_flag
}
