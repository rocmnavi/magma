/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgemv_batched.cpp, normal z -> d, Mon Jul 15 16:57:46 2024
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(m == n && n <= 32) {
        info = magmablas_dgemv_batched_smallsq(
                trans, n,
                alpha, dA_array, ldda,
                       dx_array, incx,
                beta,  dy_array, incy,
...
/***************************************************************************//**
    ...
    @param[in]
    dA      Pointer to the first DOUBLE PRECISION array 'A' of DIMENSION ( ldda, n )
            in the batch
    ...
    @param[in]
    dx      Pointer to the first DOUBLE PRECISION array 'x' in the batch, of dimension
            n if trans == MagmaNoTrans
            m if trans == MagmaTrans or MagmaConjTrans
    ...
    @param[out]
    dy      Pointer to the first DOUBLE PRECISION array 'y' in the batch, of dimension
            m if trans == MagmaNoTrans
            n if trans == MagmaTrans or MagmaConjTrans
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(m == n && n <= 32) {
        info = magmablas_dgemv_batched_strided_smallsq(
                    trans, n,
