    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(zlacpy_sym_in_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(zlacpy_sym_in_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
    else {
        hipLaunchKernelGGL(zlacpy_sym_in_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
}
