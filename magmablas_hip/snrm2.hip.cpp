...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/dznrm2.hip.cpp, normal z -> s, Fri Aug 25 13:16:58 2023
    ...
    magma_queue_t queue ) 
{
    ...
    dim3 blocks( n );    
    hipLaunchKernelGGL(magmablas_snrm2_check_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, dxnorm, dlsticc );
}
    ...
    magma_queue_t queue )
{
    ...
    dim3 blocks( 1, 1 );
    hipLaunchKernelGGL(magmablas_snrm2_smkernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dxnorm );
}
    ...
    magma_queue_t queue )
{
    dim3 threads( k );
    dim3 blocks( 1 );
    hipLaunchKernelGGL(magma_snrm2_adjust_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() , dxnorm, dc);
}
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_snrm2_row_check_adjust_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() , k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_queue_t queue ) 
{
    dim3 threads( BLOCK_SIZE );
    dim3 blocks( n );    
    hipLaunchKernelGGL(magmablas_snrm2_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, dxnorm );
