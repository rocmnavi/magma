...
#include "hgemm_template_device.hip.hpp"
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(int s = 0; s < batchCount; s+=maxbatch){
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hgemm_template_batched_nn_kernel<T, DIM_X, DIM_Y, BLK_M, BLK_N, BLK_K, TC_M, TC_N, TC_K>), dim3(dimGrid), dim3(dimBlock), shmem, queue->hip_stream() , m, n, k, dA_array+s, ldda, dB_array+s, lddb, dC_array+s, lddc, alpha, beta, roffA, coffA, roffB, coffB, roffC, coffC);
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(int s = 0; s < batchCount; s+=maxbatch){
        int batch = min(maxbatch, batchCount-s);
        dim3 dimGrid( magma_ceildiv( m, BLK_M ), magma_ceildiv( n, BLK_N ), batch );
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hgemm_template_batched_nt_kernel<T, DIM_X, DIM_Y, BLK_M, BLK_N, BLK_K, TC_M, TC_N, TC_K>), dim3(dimGrid), dim3(dimBlock), shmem, queue->hip_stream() , m, n, k, dA_array+s, ldda, dB_array+s, lddb, dC_array+s, lddc, alpha, beta, roffA, coffA, roffB, coffB, roffC, coffC);
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(int s = 0; s < batchCount; s+=maxbatch){
        int batch = min(maxbatch, batchCount-s);
        dim3 dimGrid( magma_ceildiv( m, BLK_M ), magma_ceildiv( n, BLK_N ), batch );
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hgemm_template_batched_tn_kernel<T, DIM_X, DIM_Y, BLK_M, BLK_N, BLK_K, TC_M, TC_N, TC_K>), dim3(dimGrid), dim3(dimBlock), shmem, queue->hip_stream() , m, n, k, dA_array+s, ldda, dB_array+s, lddb, dC_array+s, lddc, alpha, beta, roffA, coffA, roffB, coffB, roffC, coffC);
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(int s = 0; s < batchCount; s+=maxbatch){
        int batch = min(maxbatch, batchCount-s);
        dim3 dimGrid( magma_ceildiv( m, BLK_M ), magma_ceildiv( n, BLK_N ), batch );
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hgemm_template_batched_tt_kernel<T, DIM_X, DIM_Y, BLK_M, BLK_N, BLK_K, TC_M, TC_N, TC_K>), dim3(dimGrid), dim3(dimBlock), shmem, queue->hip_stream() , m, n, k, dA_array+s, ldda, dB_array+s, lddb, dC_array+s, lddc, alpha, beta, roffA, coffA, roffB, coffB, roffC, coffC);
    }
}
