...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/ztranspose_conj_inplace.hip.cpp, normal z -> c, Mon Jul 15 16:57:42 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( nblock % 2 == 1 ) {
        ...
        hipLaunchKernelGGL(ctranspose_conj_inplace_odd, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
    else {
        dim3 grid( nblock+1, nblock/2 );
        hipLaunchKernelGGL(ctranspose_conj_inplace_even, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
}
