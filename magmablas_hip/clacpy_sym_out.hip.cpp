...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlacpy_sym_out.hip.cpp, normal z -> c, Mon Oct 28 11:12:05 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(clacpy_sym_out_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(clacpy_sym_out_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
    else {
        hipLaunchKernelGGL(clacpy_sym_out_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
}
