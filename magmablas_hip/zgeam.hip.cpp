    ...
    magma_queue_t queue )
{
    ...
    if ( MAGMA_Z_EQUAL( alpha, MAGMA_Z_ZERO ) && 
        ...
        hipLaunchKernelGGL(zgeam_kernel_nn, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    }
    else if ( transA == MagmaNoTrans )
        hipLaunchKernelGGL(zgeam_kernel_nc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else if ( transB == MagmaNoTrans )
        hipLaunchKernelGGL(zgeam_kernel_cn, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else 
        hipLaunchKernelGGL(zgeam_kernel_cc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
}
