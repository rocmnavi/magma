...
/*
    -- MAGMA (version 2.0) --
       ...
       @author Mark Gates
       @generated from magmablas_hip/zlanhe.hip.cpp, normal z -> c, Mon Oct 28 11:12:06 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        hipLaunchKernelGGL(clanhe_inf_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        hipLaunchKernelGGL(clanhe_inf_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(clanhe_max_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
    else {
        hipLaunchKernelGGL(clanhe_max_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
}
