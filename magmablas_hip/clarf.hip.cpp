...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlarf.hip.cpp, normal z -> c, Fri Aug 25 13:16:55 2023
       @author Azzam Haidar
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_clarf_smkernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, n, dv, dtau, dc, lddc );
}
    ...
    magma_queue_t queue )
{
    ...
    if ( n > 0 ) {
        hipLaunchKernelGGL(magma_clarf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dv, dtau, dC, lddc);
    }
