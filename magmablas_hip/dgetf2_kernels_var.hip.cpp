...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgetf2_kernels_var.hip.cpp, normal z -> d, Mon Oct 28 11:12:25 2024
*/
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(idamax_kernel_vbatched, dim3(grid), dim3(threads), zamax * (sizeof(double) + sizeof(int)), queue->hip_stream() , length, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_i, info_array, step, gbstep );
        ...
        magma_int_t** ipiv_array, int piv_adjustment)
{
    ...
    double *dAorg = dA_array[batchid];
    double *dA    = dAorg + Aj * my_ldda + Ai;
    double *dA1   = dA;
    double *dA2   = dA + jp;

    // read the abs. value of the pivot:
    // in src/dgetf2_vbatched.cpp idamax is called at dA(Ai+gbj, Aj+gbj)
    // while dswap is called at dA(Ai+gbj, Aj), so for this kernel
    // the pivot is at Ai (local value) and (Aj + piv_adjustment)
    double rA = dAorg[ (Aj+piv_adjustment) * my_ldda + Ai ];
    double rx_abs_max = fabs( MAGMA_D_REAL(rA) ) + fabs( MAGMA_D_IMAG(rA) );
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(dswap_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() , max_n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, piv_adjustment);
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dscal_dger_1d_generic_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream(), max_M, max_N, M+i, N+i, dA_array+i, Ai, Aj, ldda+i, info_array+i, step, gbstep);
    }
    return 0;
}
        ...
        magma_int_t *info_array,  int gbstep, int batchCount )
{
    ...
    for(int j = 0; j < my_minmn; j++){
        ...
        reg = (rx_abs_max == MAGMA_D_ZERO) ? MAGMA_D_ONE : MAGMA_D_DIV( MAGMA_D_ONE, sA(j,j) );
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)dgetf2_fused_sm_kernel_vbatched, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
...
{
    HIP_DYNAMIC_SHARED( double, data)
    const int tx = threadIdx.x;
    ...
    for(int j = 0; j < max_N; j++) {
        sA[j * slda + tx] = (j == tx) ? MAGMA_D_ONE : MAGMA_D_ZERO;
    }
    __syncthreads();
    ...
    magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)dgetf2_fused_kernel_vbatched<max_N>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
        //printf("error in %s : failed to launch kernel %s\n", __func__, hipGetErrorString(e));
        arginfo = -100;
    }
