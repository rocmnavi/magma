...
/*
   ...
   @generated from magmablas_hip/zgetf2_devicefunc.hip.hpp, normal z -> s, Mon Oct 28 11:13:25 2024
 */
                         ...
                         magma_int_t *info, int step, int gbstep)
{
    ...
    float rTmp = fabs(MAGMA_S_REAL( shared_y[0] ) ) + fabs( MAGMA_S_IMAG( shared_y[0] ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_S_ONE : MAGMA_S_DIV(MAGMA_S_ONE, shared_y[0]);
                         ...
                         magma_int_t *info, int step, int gbstep)
{
    ...
    float rTmp;
    rA   = dA[0];
    rTmp = fabs(MAGMA_S_REAL( rA ) ) + fabs( MAGMA_S_IMAG( rA ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_S_ONE : MAGMA_S_DIV(MAGMA_S_ONE, rA);
    rA  = dA[ gtx ];
    rA *= reg;
