...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeqr2_batched_fused.hip.hpp, normal z -> c, Mon Jul 15 16:58:31 2024
*/
    ...
    magma_int_t lwork_tmp, magma_int_t *info_array, magma_int_t check_launch_only, magma_int_t batchCount )
{
    ...
    if( tx < m ) {
        ...
        sA(tx, 0) = rA[0];
        tmp       = (tx < 1) ? MAGMA_C_ZERO : rA[0];    // exclude first element (alpha)
        sum += MAGMA_C_REAL( tmp ) * MAGMA_C_REAL( tmp ) +
    ...
    for(int j = 0; j < N; j++) {
        ...
        // compute the norm of the current column
        {
            ...
            if( tx == 0 ) {
                ...
                #if 0
                snorm[0] = sqrt( sum );
                sum      = sum - (MAGMA_C_REAL(alpha) * MAGMA_C_REAL(alpha)) - (MAGMA_C_IMAG(alpha) * MAGMA_C_IMAG(alpha));
                snorm[1] = sum;
                #else
                snorm[1] = sum;  // store norm^2 without alpha
                sum     += (MAGMA_C_REAL(alpha) * MAGMA_C_REAL(alpha)) + (MAGMA_C_IMAG(alpha) * MAGMA_C_IMAG(alpha));
                snorm[0] = sqrt( sum );
                #endif
            }
            __syncthreads();
        ...
        if(!zero_nrm ) {
            ...
            scale = (tx > j) ? MAGMA_C_DIV( MAGMA_C_ONE,  alpha - MAGMA_C_MAKE(beta, 0)) : scale;
            tau   = MAGMA_C_MAKE( (beta - real(alpha))  / beta, -imag(alpha) / beta );
            rA[j] *= scale;
            tmp    = (tx == j) ? MAGMA_C_MAKE(beta, MAGMA_D_ZERO) : rA[j];
            if(tx == j) {
                stau[j] = tau;
                rA[j]   = MAGMA_C_ONE;
            }
        ...
        // now compute: A = A - v * y
        {
            ...
            if(j < N-1) {
                ...
                tmp         = (tx < (j+2)) ? MAGMA_C_ZERO : rA[j+1];    // exclude first element (alpha)
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)cgeqr2_fused_reg_kernel_batched<M32, N>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
