...
__global__ void hlaswp_kernel(
    int n,
    magmaHalf *dAT, int ldda,
    hlaswp_params_t params )
{
...
/***************************************************************************//**
    ...
    ** Unlike LAPACK, here A is stored row-wise (hence dAT). **
    ...
    @param[in,out]
    dAT     HALF array on GPU, stored row-wise, dimension (LDDA,M)
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        hipLaunchKernelGGL(hlaswp_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k,0), ldda, params );
    }
...
// batches of pivots. On Fermi, it is faster than magmablas_hlaswp
// (including copying pivots to the GPU).

__global__ void hlaswp2_kernel(
    int n,
    magmaHalf *dAT, int ldda,
    int npivots,
    ...
    magma_queue_t queue )
{
    ...
    dim3 threads( NTHREADS );
    dim3 grid( magma_ceildiv( n, NTHREADS ) );
    hipLaunchKernelGGL(hlaswp2_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k1-1,0), ldda, nb, d_ipiv, inci );
}
