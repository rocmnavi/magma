...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeqr2_batched.hip.cpp, normal z -> d, Mon Jul 15 16:57:46 2024
*/
...
#include "dlarfg_devicesfunc.hip.hpp"
                     ...
                     magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( total_shmem_sm_kernel <= shmem_max ) {
            ...
            hipLaunchKernelGGL(dgeqr2_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(double)*(m*k), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
            ...
            hipLaunchKernelGGL(dgeqr2_column_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(double)*(m), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
        else {
            //not use dynamic shared memory at all
            hipLaunchKernelGGL(dgeqr2_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
    }
