    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (uplo == MagmaLower) {
        hipLaunchKernelGGL(clat2z_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, SA, ldsa, A, lda);
    }
    else if (uplo == MagmaUpper) {
        hipLaunchKernelGGL(clat2z_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, SA, ldsa, A, lda);
    }
}
