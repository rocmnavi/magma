...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarf.hip.cpp, normal z -> s, Mon Oct 28 11:12:06 2024
       @author Azzam Haidar
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_slarf_smkernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, n, dv, dtau, dc, lddc );
}
    ...
    magma_queue_t queue )
{
    ...
    if ( n > 0 ) {
        hipLaunchKernelGGL(magma_slarf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dv, dtau, dC, lddc);
    }
