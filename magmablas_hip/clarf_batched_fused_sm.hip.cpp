...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarf_batched_fused_sm.hip.cpp, normal z -> c, Mon Oct 28 11:12:20 2024
*/
        ...
        for(int j = 0; j < NB; j++) {
            ...
            if(ty_ < NB) {
                for(int i = tx_; i < m; i+=tpc) {
                    zsum += sA(i,ty_) * MAGMA_C_CONJ( sV(i,j) );
                }
                sT(tx_,ty_) = zsum;
            }
            __syncthreads();
...
template<int NB>
static magma_int_t
magma_clarf_fused_sm_kernel_driver_batched(
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    magma_int_t shmem = 0;
    shmem += SLDA(m)   * NB * sizeof(magmaFloatComplex);  // sA
    shmem += SLDA(m)   * NB * sizeof(magmaFloatComplex);  // sV
    ...
    if (shmem <= shmem_max) {
        cudaFuncSetAttribute(clarf_fused_sm_kernel_batched<NB>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem);
    }
    #else
    ...
    hipError_t e = hipLaunchKernel((void*)clarf_fused_sm_kernel_batched<NB>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    switch(nb) {
        case 1: arginfo = magma_clarf_fused_sm_kernel_driver_batched<1>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, nthreads, check_launch_only, batchCount, queue ); break;
        case 2: arginfo = magma_clarf_fused_sm_kernel_driver_batched<2>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, nthreads, check_launch_only, batchCount, queue ); break;
        case 4: arginfo = magma_clarf_fused_sm_kernel_driver_batched<4>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, nthreads, check_launch_only, batchCount, queue ); break;
        case 8: arginfo = magma_clarf_fused_sm_kernel_driver_batched<8>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, nthreads, check_launch_only, batchCount, queue ); break;
        #if defined(MAGMA_HAVE_CUDA) && !defined(PRECISION_z)
        case 16: arginfo = magma_clarf_fused_sm_kernel_driver_batched<16>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, nthreads, check_launch_only, batchCount, queue ); break;
        #endif
        default: arginfo = -100;
    }
    return arginfo;
