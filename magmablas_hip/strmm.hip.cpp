/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/ztrmm.hip.cpp, normal z -> s, Mon Oct 28 11:12:10 2024
...
magma_int_t magma_get_strmm_nb(magma_int_t n)
{
        ...
        magma_queue_t queue )
{
    ...
    {
        case 0: // lNx
            trmm_template_lNx<float, STRMM_NB>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
        case 1: // lTx
            trmm_template_lTx<float, STRMM_NB, 0>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
        case 2: // lCx
            trmm_template_lTx<float, STRMM_NB, 1>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
        case 3: // rNx
            trmm_template_rNx<float, STRMM_NB>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
        case 4: // rTx
            trmm_template_rTx<float, STRMM_NB, 0>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
        case 5: // rCx
            trmm_template_rTx<float, STRMM_NB, 1>
            (uplo, diag, m, n, alpha, dA, ldda, dB, lddb, queue);
            break;
...
#define dB(i,j) dB + j*lddb + i
    ...
    // stopping condition
    if(nrowA <= STRMM_NB){
        magmablas_strmm_small( side, uplo, transA, diag,
            m, n, alpha,
            dA, ldda,
            dB, lddb, queue );
        return;
    ...
    // at this point we can tell that nrowA > STRMM_NB
    switch(shape)
    {
        case 0: // lNl
            {
                const int m1 = magma_get_strmm_nb(m);
                const int m2 = m - m1;
        ...
        case 1: // lNU
            {
                const int m2 = magma_get_strmm_nb(m);
                const int m1 = m - m2;
        ...
        case 4: // rNL
            {
                const int n2 = magma_get_strmm_nb(n);
                const int n1 = n - n2;
        ...
        case 5: // rNU
            {
                const int n1 = magma_get_strmm_nb(n);
                const int n2 = n - n1;
