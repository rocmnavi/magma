...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlarfx.hip.cpp, normal z -> d, Fri Aug 25 13:16:57 2023
    ...
    magma_queue_t queue )
{
    ...
    if (iter == 0) {
        hipLaunchKernelGGL(magma_dlarfx_kernel, dim3(N), dim3(BLOCK_SIZE), 0, queue->hip_stream() ,  m, v, tau, C, ldc, xnorm, dT+iter*N, iter );
    }
    else {
        hipLaunchKernelGGL(magma_dlarfx_kernel, dim3(N), dim3(BLOCK_SIZE), 0, queue->hip_stream() ,  m, v, tau, C, ldc, xnorm, work, iter );
    }
    ...
    if (iter > 0) {
        ...
        //    ( dT, N, dT+iter*N);
        hipLaunchKernelGGL(magma_dtrmv_kernel2, dim3(iter), dim3(iter), 0, queue->hip_stream() ,  dT, N, work, dT+iter*N, tau );
    }
}
