    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    hipMemcpyToSymbol(HIP_SYMBOL(magma_zlat2c_flag), info, sizeof(magma_zlat2c_flag) );    // magma_zlat2c_flag = 0
    
    if (uplo == MagmaLower) {
        hipLaunchKernelGGL(zlat2c_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, rmax);
    }
    else if (uplo == MagmaUpper) {
        hipLaunchKernelGGL(zlat2c_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, rmax);
    }
    
    hipMemcpyFromSymbol(info, HIP_SYMBOL(magma_zlat2c_flag), sizeof(magma_zlat2c_flag) );  // info = magma_zlat2c_flag
}
