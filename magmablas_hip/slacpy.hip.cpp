...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlacpy.hip.cpp, normal z -> s, Mon Jul 15 16:57:38 2024
...
void slacpy_full_kernel_batched(
    int m, int n,
    float const * const *dAarray, magma_int_t Ai, magma_int_t Aj, int ldda,
    float **dBarray, magma_int_t Bi, magma_int_t Bj, int lddb )
{
    ...
    if ( uplo == MagmaLower ) {
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(slacpy_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(slacpy_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
            }
        }
    }
        ...
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(slacpy_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(slacpy_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            hipLaunchKernelGGL(slacpy_lower_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
        else if ( uplo == MagmaUpper ) {
            hipLaunchKernelGGL(slacpy_upper_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
        else {
            hipLaunchKernelGGL(slacpy_full_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            hipLaunchKernelGGL(slacpy_lower_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else if ( uplo == MagmaUpper ) {
            hipLaunchKernelGGL(slacpy_upper_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else {
            hipLaunchKernelGGL(slacpy_full_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
    }
}
