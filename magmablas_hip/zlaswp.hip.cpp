    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        hipLaunchKernelGGL(zlaswp_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k,0), ldda, params );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        hipLaunchKernelGGL(zlaswpx_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA(k,0), ldx, ldy, params );
    }
    ...
    magma_queue_t queue )
{
    ...
    dim3 threads( NTHREADS );
    dim3 grid( magma_ceildiv( n, NTHREADS ) );
    hipLaunchKernelGGL(zlaswp2_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k1-1,0), ldda, nb, d_ipiv, inci );
}
