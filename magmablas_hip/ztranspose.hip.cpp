    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(ztranspose_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dAT, lddat );
}
    ...
    magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(ztranspose_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA_array+i, ldda, dAT_array+i, lddat );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        dim3 grid( magma_ceildiv( m, NB ), magma_ceildiv( n, NB ), ibatch );
        hipLaunchKernelGGL(ztranspose_kernel_batched_stride, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, stride, dA_array+i, ldda, dAT_array+i, lddat );
    }
}
