...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgbtf2_kernels.hip.cpp, normal z -> c, Mon Oct 28 11:12:15 2024
*/
...
void
cgbtf2_adjust_ju_fillin_kernel_batched(
    int n, int kl, int ku,
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(cgbtf2_adjust_ju_fillin_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream(), n, kl, ku, dAB_array, lddab, dipiv_array, ju_array, gbstep, batchCount);
}

/******************************************************************************/
__global__ __launch_bounds__(GBTF2_SWAP_MAX_THREADS)
void cgbtf2_swap_kernel_batched(
        ...
        int* ju_array, magma_int_t gbstep)
{
    ...
    magmaFloatComplex *dAB = dAB_array[batchid] + aj * lddab + ai;
    ...
    if( !(jp == 0) ) {
        ...
        magmaFloatComplex *dR1 = dAB;      // 1st row with the diagonal
        magmaFloatComplex *dR2 = dAB + jp; // 2nd row with the pivot
    ...
    int* ju_array, magma_int_t gbstep, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(cgbtf2_swap_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream(), dAB_array, ai, aj, lddab, dipiv_array, ipiv_offset, ju_array, gbstep);
...
void cgbtf2_scal_ger_kernel_batched(
    int m, int n, int kl, int ku,
    magmaFloatComplex **dAB_array, int ai, int aj, int lddab,
    int* ju_array, int gbstep, magma_int_t *info_array)
{
    ...
    magmaFloatComplex* dAB = dAB_array[batchid] + aj * lddab + ai;
    magmaFloatComplex  rA  = MAGMA_C_ZERO, reg = MAGMA_C_ZERO;

    if( gtx > 0 && gtx < km ) {
        reg = MAGMA_C_DIV(MAGMA_C_ONE, dAB[0]);
        rA  = dAB[ gtx ];
        rA *= reg;
        dAB[ gtx ] = rA;
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t s = 0; s < batchCount; s+=max_batchCount) {
        ...
        hipLaunchKernelGGL(cgbtf2_scal_ger_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream(), m, n, kl, ku, dAB_array+s, ai, aj, lddab, ju_array+s, gbstep, info_array+s);
    }
    return 0;
}
...
/******************************************************************************/
__global__
void cgbtf2_native_kernel(
    int m, int n, int nb, int kl, int ku,
    magmaFloatComplex *dA, int ldda, magma_int_t *ipiv,
...
#define dA(i,j) dA[(j)*ldda + (i)]
    ...
    int jp = 0;
    float rx_abs_max = 0;
    magmaFloatComplex tmp = MAGMA_C_ZERO, reg = MAGMA_C_ZERO;

    // setup shared memory
    magmaFloatComplex* sA = zdata;
    float*             sX = (float*)(sA + mband);
    int*                sI = (int*)(sX + kl+1);
    ...
    for(int j = 0; j < nb; j++) {
        ...
        if(bx == j) {
            if(km >= 128) {
                for(int i = tx; i < km; i+=ntx) {
                    sX[i] = fabs( MAGMA_C_REAL(sA[kv+i]) ) + fabs( MAGMA_C_IMAG(sA[kv+i]) );
                    sI[i] = i;
                }
                __syncthreads();
        ...
        if(bx == j) {
            reg = ( rx_abs_max == MAGMA_D_ZERO ) ? MAGMA_C_ONE : MAGMA_C_DIV(MAGMA_C_ONE, sA[kv] );
    ...
    magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    magma_int_t shmem = 0;
    shmem += mband  * sizeof(magmaFloatComplex);
    shmem += (kl+1) * sizeof(float);
    ...
    for(magma_int_t gbstep = 0; gbstep < n; gbstep += nb) {
        ...
        hipError_t e = hipLaunchCooperativeKernel((void*)cgbtf2_native_kernel, grid, threads, kernel_args, shmem, queue->hip_stream());
    }
    ...
    magma_int_t* info, magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    magma_cgbtf2_native_work(m, n, kl, ku, NULL, ldda, NULL, info, NULL, lwork, queue);

    void* device_work = NULL;
    magma_malloc(&device_work, lwork[0]);

    magma_cgbtf2_native_work(m, n, kl, ku, dA, ldda, ipiv, info, device_work, lwork, queue);
...
#define sA(i, j) sA[(j)*slda + (i)]
    ...
    for(int j = 0; j < total_factorize; j++) {
        ...
        if( bx ==  pivoter) {
            ...
            if(km >= 128) {
                for(int i = tx; i < km; i+=ntx) {
                    sX[i] = fabs( MAGMA_C_REAL(sA(kv+i,lj)) ) + fabs( MAGMA_C_IMAG(sA(kv+i,lj)) );
                    sI[i] = i;
                }
                __syncthreads();
            ...
            reg = ( rx_abs_max == MAGMA_D_ZERO ) ? MAGMA_C_ONE : MAGMA_C_DIV(MAGMA_C_ONE, sA(kv,lj) );
            for(int i = tx; i < (km-1); i+=ntx) {
                sA(kv+1+i,lj) *= reg;
            }
            __syncthreads();
    ...
    magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    magma_int_t shmem = 0;
    shmem += slda  * nb1 * sizeof(magmaFloatComplex);
    shmem += (kl+1) * sizeof(float);
    shmem += (kl+1) * sizeof(int);
    ...
    for(magma_int_t gbstep = 0; gbstep < n; gbstep += NB) {
        ...
        hipError_t e = hipLaunchCooperativeKernel((void*)cgbtf2_native_kernel_v2, grid, threads, kernel_args, shmem, queue->hip_stream());
        if(e != hipSuccess) {
            printf("ERROR: %s \n", hipGetErrorString(e));
            *info = -100;
            return *info;
    ...
    magma_int_t* info, magma_queue_t queue)
{
    ...
    *info  = 0;
    ...
    // query workspace
    magma_int_t lwork[1] = {-1};
    magma_cgbtf2_native_v2_work(m, n, kl, ku, NULL, ldda, NULL, info, NULL, lwork, queue);

    void* device_work = NULL;
    magma_malloc(&device_work, lwork[0]);

    magma_cgbtf2_native_v2_work(m, n, kl, ku, dA, ldda, ipiv, info, device_work, lwork, queue);
