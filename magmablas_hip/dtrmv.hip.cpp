/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/ztrmv.hip.cpp, normal z -> d, Mon Jul 15 16:57:42 2024
...
magma_int_t magma_get_dtrmv_nb(magma_int_t n)
{
        ...
        magma_queue_t queue )
{
    if(transA == MagmaConjTrans) {
        trmv_template<double, DTRMV_NB, 1>
        (uplo, transA, diag, n, dA, ldda, dX, incx, queue);
    }
    else {
        trmv_template<double, DTRMV_NB, 0>
        (uplo, transA, diag, n, dA, ldda, dX, incx, queue);
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////////
void
magmablas_dtrmv_core(
        magma_uplo_t uplo, magma_trans_t transA, magma_diag_t diag,
        magma_int_t n,
        double *dA, magma_int_t ldda,
        double *dX, magma_int_t incx,
...
#define dX(j)   dX + j*incx
    ...
    // stopping condition
    if(n <= DTRMV_NB){
        magmablas_dtrmv_small( uplo, transA, diag, n, dA, ldda, dX, incx, queue );
        return;
    }
    ...
    // at this point we can tell that nrowA > DTRMV_NB
    switch(shape)
    {
        case 0: // Nl
            {
                const int n1 = magma_get_dtrmv_nb(n);
                const int n2 = n - n1;
        ...
        case 1: // NU
            {
                const int n2 = magma_get_dtrmv_nb(n);
                const int n1 = n - n2;
...
/*
    ...
    DTRMV  performs the matrix-vector operation
        ...
        magma_queue_t queue )
{
    ...
    magmablas_dtrmv_core( uplo, transA, diag, n, dA, ldda, dX, incx, queue );
