...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgerbt_kernels.hip.cpp, normal z -> c, Mon Jul 15 16:57:37 2024
    ...
    magmaFloatComplex *dv, int Vi)
{
    ...
    magmaFloatComplex a00 = MAGMA_C_ZERO, a10 = MAGMA_C_ZERO, a01 = MAGMA_C_ZERO, a11 = MAGMA_C_ZERO;
    magmaFloatComplex b1 = MAGMA_C_ZERO, b2 = MAGMA_C_ZERO, b3 = MAGMA_C_ZERO, b4 = MAGMA_C_ZERO;
...
magmablas_celementary_multiplication_kernel(
    ...
    magmaFloatComplex *dv, int Vi)
{
    magmablas_celementary_multiplication_devfunc( Am, An, dA, Ai, Aj, ldda, du, Ui, dv, Vi );
}
    ...
    magmaFloatComplex *dv, int Vi)
{
    ...
    magmablas_celementary_multiplication_devfunc( Am, An, dA_array[batchid], Ai, Aj, ldda, du, Ui, dv, Vi );
}
    ...
    magmaFloatComplex *du, magmaFloatComplex *db)
{
    ...
    a1 = (idx < n1) ? du[ 0] * db[ 0] : MAGMA_C_ZERO;
    a2 = (idx < n2) ? du[n1] * db[n1] : MAGMA_C_ZERO;
    ...
    magmaFloatComplex *du, int offsetu, magmaFloatComplex *db, int lddb, int offsetb)
{
    for(int i = 0; i < nrhs; i++) {
        magmablas_capply_vector_devfunc(n, du+offsetu, db + i*lddb + offsetb);
    }
}
    ...
    magmaFloatComplex *du, int offsetu, magmaFloatComplex **db_array, int lddb, int offsetb )
{
    ...
    for(int i = 0; i < nrhs; i++) {
        magmablas_capply_vector_devfunc(n, du+offsetu, db_array[batchid]+ i * lddb + offsetb);
    }
}
    ...
    magmaFloatComplex *du,magmaFloatComplex *db )
{
    ...
    if (idx < n1) {
        ...
        a1 = db[0] + ((idx < n2) ? db[n1] : MAGMA_C_ZERO);
        a2 = db[0] - ((idx < n2) ? db[n1] : MAGMA_C_ZERO);
    }
...
{
    for(int i = 0; i < nrhs; i++) {
        magmablas_capply_transpose_vector_devfunc(n, du+offsetu, db+ i*lddb + offsetb);
    }
}
    ...
    magmaFloatComplex *du, int offsetu, magmaFloatComplex **db_array, int lddb, int offsetb )
{
    int batchid = blockIdx.y;
    for(int i = 0; i < nrhs; i++) {
        magmablas_capply_transpose_vector_devfunc(n, du+offsetu, db_array[batchid]+ i*lddb + offsetb);
    }
}
