...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarfbx.hip.cpp, normal z -> d, Thu Jan 25 22:57:53 2024
...
/*
    Call 
        hipLaunchKernelGGL(magma_dgemv_kernel3, dim3(n), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork, tau)
    to compute
...
{
    /* dwork = V**H c     */
    hipLaunchKernelGGL(magma_dgemv_kernel1, dim3(k), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork); 

    /* dwork = T**H dwork */
    hipLaunchKernelGGL(magma_dtrmv_tkernel, dim3(k), dim3(k), 0, queue->hip_stream() ,  dT, ldt, dwork, dwork+k);
 
    /* c = c - V dwork    */
    dim3  blocks3( magma_ceildiv( m, BLOCK_SIZE ) );
    dim3 threads3( BLOCK_SIZE );     
    hipLaunchKernelGGL(magma_dgemv_kernel2, dim3(blocks3), dim3(threads3), 0, queue->hip_stream() ,  m, k, V, ldv, dwork+k, c);
}
