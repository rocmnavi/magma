...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlaswp_batched.hip.cpp, normal z -> c, Thu Jan 25 22:58:03 2024
...
#define output_array(i,j) output_array, i, j
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( n < SWP_WIDTH) {
            ...
            hipLaunchKernelGGL(claswp_rowparallel_kernel_batched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, n, height, input_array+i, input_i, input_j, ldi, output_array+i, output_i, output_j, ldo, pivinfo_array+i );
        }
        else {
            size_t shmem = sizeof(magmaFloatComplex) * height * SWP_WIDTH;
            hipLaunchKernelGGL(claswp_rowparallel_kernel_batched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, SWP_WIDTH, height, input_array+i, input_i, input_j, ldi, output_array+i, output_i, output_j, ldo, pivinfo_array+i );
        }
    }
#undef  input_array
#undef output_attay
    ...
    magma_queue_t queue)
{
    ...
    {
        size_t shmem = sizeof(magmaFloatComplex) * height * n;
        hipLaunchKernelGGL(claswp_rowparallel_kernel, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, n, height, input, ldi, output, ldo, pivinfo );
    }
    else
    {
        size_t shmem = sizeof(magmaFloatComplex) * height * SWP_WIDTH;
        hipLaunchKernelGGL(claswp_rowparallel_kernel, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, SWP_WIDTH, height, input, ldi, output, ldo, pivinfo );
    }
}
                   ...
                   magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(claswp_rowserial_kernel_batched, dim3(grid), dim3(max_BLK_SIZE__n), 0, queue->hip_stream() , n, dA_array+i, lda, k1, k2, ipiv_array+i);
    }
}
                   ...
                   magma_int_t* dipiv, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(claswp_rowserial_kernel_native, dim3(grid), dim3(max_BLK_SIZE_n), 0, queue->hip_stream() , n, dA, lda, k1, k2, dipiv);
}
    ...
    magma_int_t *dipiv, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(claswp_columnserial_kernel, dim3(grid), dim3(CLASWP_COL_NTH), 0, queue->hip_stream() , n, dA, lda, k1, k2, dipiv);
}
                   ...
                   magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(claswp_columnserial_kernel_batched, dim3(grid), dim3(min_CLASWP_COL_NTH__n), 0, queue->hip_stream() , n, dA_array+i, lda, k1, k2, ipiv_array+i);
    }
}
