    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        hipLaunchKernelGGL(zlascl_2x2_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, dW, lddw, dA, ldda);
    }
    else {
        hipLaunchKernelGGL(zlascl_2x2_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, dW, lddw, dA, ldda);
    }
}
