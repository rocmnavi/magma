...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zhemv.hip.cpp, normal z -> s, Mon Oct 28 11:12:04 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( upper ) {
        hipLaunchKernelGGL(ssymv_kernel_U, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(ssymv_kernel_U_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    else {
        hipLaunchKernelGGL(ssymv_kernel_L, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(ssymv_kernel_L_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    return info;
}
