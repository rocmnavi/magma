...
/*
   ...
   @generated from magmablas_hip/zpotf2_kernels.hip.cpp, normal z -> s, Mon Jul 15 16:57:48 2024
 */
...
#include "spotf2_devicesfunc.hip.hpp"
        ...
        magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for (magma_int_t j = 0; j < n; j += POTF2_NB) {
        ...
        if (ib == POTF2_NB) {
            hipLaunchKernelGGL(spotf2_smlpout_fixwidth_kernel_batched, dim3(dimGrid), dim3(threads), shared_mem_size, queue->hip_stream() , rows, dA_array, ai, aj, lda, j, gbstep, info_array, batchCount);
        }
        else {
            hipLaunchKernelGGL(spotf2_smlpout_anywidth_kernel_batched, dim3(dimGrid), dim3(threads), shared_mem_size, queue->hip_stream() , rows, ib, dA_array, ai, aj, lda, j, gbstep, info_array, batchCount);
        }
    }
        ...
        magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if( n % POTF2_NB == 0){
        hipLaunchKernelGGL(spotf2_smlpin_fixwidth_kernel_batched, dim3(grid), dim3(threads), shared_mem_size, queue->hip_stream() , n, dA_array, ai, aj, lda, 0, gbstep, info_array, batchCount);
    }
    else{
        hipLaunchKernelGGL(spotf2_smlpin_anywidth_kernel_batched, dim3(grid), dim3(threads), shared_mem_size, queue->hip_stream() , n, dA_array, ai, aj, lda, 0, gbstep, info_array, batchCount);
    }
        ...
        magma_int_t *dinfo, magma_queue_t queue)
{
    ...
    for (magma_int_t j = 0; j < n; j += POTF2_NB) {
        ...
        if (ib == POTF2_NB)
        {
            hipLaunchKernelGGL(spotf2_smlpout_fixwidth_kernel, dim3(dimGrid), dim3(threads), shared_mem_size, queue->hip_stream() , rows, dA, lda, j, gbstep, dinfo );
        } else {
            hipLaunchKernelGGL(spotf2_smlpout_anywidth_kernel, dim3(dimGrid), dim3(threads), shared_mem_size, queue->hip_stream() , rows, ib, dA, lda, j, gbstep, dinfo );
        }
    }
        ...
        magma_int_t *dinfo, magma_queue_t queue)
{
    ...
    if( n % POTF2_NB == 0){
        hipLaunchKernelGGL(spotf2_smlpin_fixwidth_kernel, dim3(grid), dim3(threads), shared_mem_size, queue->hip_stream() , n, dA, ldda, 0, gbstep, dinfo);
    }
    else{
        hipLaunchKernelGGL(spotf2_smlpin_anywidth_kernel, dim3(grid), dim3(threads), shared_mem_size, queue->hip_stream() , n, dA, ldda, 0, gbstep, dinfo);
    }
    return arginfo;
}
