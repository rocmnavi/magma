        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(zlaswp_left_rowserial_kernel_vbatched, dim3(grid), dim3(min_BLK_SIZE_n), 0, queue->hip_stream() , n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_offset, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(zlaswp_right_rowserial_kernel_vbatched, dim3(grid), dim3(min_BLK_SIZE_n), 0, queue->hip_stream() , n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_offset, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(zlaswp_left_rowparallel_kernel_vbatched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, width, M, N, dA_array, Ai, Aj, ldda, pivinfo_array, pivinfo_i, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        size_t shmem = sizeof(magmaDoubleComplex) * height * width;
        hipLaunchKernelGGL(zlaswp_right_rowparallel_kernel_vbatched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, width, M, N, dA_array, Ai, Aj, ldda, pivinfo_array, pivinfo_i, k1, k2);
    }
}
