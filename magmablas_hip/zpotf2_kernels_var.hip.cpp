        ...
        magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        for(magma_int_t j = 0; j < max_n; j+= POTF2_NB) {
            ...
            //hipLaunchKernelGGL(zpotf2_smlpout_kernel_vbatched, dim3(dimGrid), dim3(threads), shared_mem_size, queue , n, dA_array, lda, j, gbstep, info_array);
            hipLaunchKernelGGL(zpotf2_smlpout_kernel_vbatched_v2, dim3(dimGrid), dim3(threads), shared_mem_size, queue->hip_stream() , max_n, n+i, dA_array+i, lda+i, j, gbstep, info_array+i);
        }
    }
    return arginfo;
}
