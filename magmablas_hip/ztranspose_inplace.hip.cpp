    ...
    magma_queue_t queue )
{
    ...
    if ( nblock % 2 == 1 ) {
        ...
        hipLaunchKernelGGL(ztranspose_inplace_odd, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
    else {
        dim3 grid( nblock+1, nblock/2 );
        hipLaunchKernelGGL(ztranspose_inplace_even, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
}
