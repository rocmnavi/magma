    ...
    magma_queue_t queue )
{
    ...
    if (upper)
        hipLaunchKernelGGL(zheinertia_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, ipiv, dneig);
    else
        hipLaunchKernelGGL(zheinertia_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, ipiv, dneig);
