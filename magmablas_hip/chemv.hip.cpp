...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zhemv.hip.cpp, normal z -> c, Fri Aug 25 13:16:53 2023
...
/***************************************************************************//**
    ...
    routine and provides the same interface as hipblas.
    ...
    magma_queue_t queue )
{
    ...
    if ( upper ) {
        hipLaunchKernelGGL(chemv_kernel_U, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(chemv_kernel_U_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    else {
        hipLaunchKernelGGL(chemv_kernel_L, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(chemv_kernel_L_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    return info;
}
