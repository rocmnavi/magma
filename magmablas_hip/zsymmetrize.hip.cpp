    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(zsymmetrize_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda );
    }
    else {
        hipLaunchKernelGGL(zsymmetrize_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda );
    }
}
