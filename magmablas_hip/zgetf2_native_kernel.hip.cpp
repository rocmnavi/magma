    ...
    magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)zgetf2_native_kernel<TX, NPAGES>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
    ...
    magma_int_t *info, magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(zgetf2_native_init_kernel, dim3(1), dim3(max_n_npages), 0, queue->hip_stream() ,  n, npages, ipiv, update_flag);
