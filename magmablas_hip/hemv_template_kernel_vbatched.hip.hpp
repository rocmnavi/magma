...
#include "hemv_template_device.hip.hpp"
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i=0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hemv_diag_template_vbatched_kernel<T, NB, TY>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  uplo, n+i,
          alpha, dA_array+i, ldda+i,
                 dX_array+i, incx+i,
          beta,  dY_array+i, incy+i,
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i=0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hemv_lower_template_vbatched_kernel<T, NB, TY>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n+i, alpha,
          dA_array+i, ldda+i,
          dX_array+i, incx+i,
          dY_array+i, incy+i,
          max_n, offA, offX, offY, spec_n);
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i=0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(hemv_upper_template_vbatched_kernel<T, NB, TY>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n+i, alpha,
          dA_array+i, ldda+i,
          dX_array+i, incx+i,
          dY_array+i, incy+i,
          max_n, offA, offX, offY, spec_n);
