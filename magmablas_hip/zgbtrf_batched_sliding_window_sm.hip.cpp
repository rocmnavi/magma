...
#include "zgbtf2_devicefunc.hip.hpp"
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)zgbtrf_batched_sliding_window_loopout_kernel_sm<NTX>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)zgbtrf_batched_sliding_window_loopin_kernel_sm<NTX>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
        //printf("error in %s : failed to launch kernel %s\n", __func__, hipGetErrorString(e));
        arginfo = -100;
    }
