    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        hipLaunchKernelGGL(zlascl2_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaUpper) {
        hipLaunchKernelGGL(zlascl2_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
    else if (type == MagmaFull) {
        hipLaunchKernelGGL(zlascl2_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, dA, ldda);
    }
}
