    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    while( ! done ) {
        ...
        if (type == MagmaLower) {
            hipLaunchKernelGGL(zlascl_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaUpper) {
            hipLaunchKernelGGL(zlascl_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaFull) {
            hipLaunchKernelGGL(zlascl_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
