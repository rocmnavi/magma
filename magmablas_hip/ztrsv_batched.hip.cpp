    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ztrsv_notrans_kernel_outplace_batched<BLOCK_SIZE, DIM_X, DIM_Y, TILE_SIZE, flag, uplo, trans, diag>), dim3(grid), dim3(threads), shmem, queue->hip_stream(), n, A_array+i, lda, b_array+i, incb, x_array+i);
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ztrsv_trans_kernel_outplace_batched<BLOCK_SIZE, DIM_X, DIM_Y, TILE_SIZE, flag, uplo, trans, diag>), dim3(grid), dim3(threads), shmem, queue->hip_stream(), n, A_array+i, lda, b_array+i, incb, x_array+i);
    }
}
