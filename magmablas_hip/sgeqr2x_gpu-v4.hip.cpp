...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zgeqr2x_gpu-v4.hip.cpp, normal z -> s, Fri Aug 25 13:17:05 2023
    ...
    for (magma_int_t b=0; b < k; b += BS) {
        for (i = b; i < min(k, b+BS); ++i) {
            ...
            if (i-b > 0) {
                ...
                if ( i-1 > 0 ) {
                    hipLaunchKernelGGL(magma_sgemv_kernel3, dim3(i-1), dim3(BLOCK_SIZE), 0, queue->hip_stream() ,  m-i+1, dA(i-1,0), ldda, dA(i-1, i-1), dwork2, dtau+i-1);
                    hipLaunchKernelGGL(magma_strmv_kernel2, dim3(i-1), dim3(i-1), 0, queue->hip_stream() ,  dT(0,0), k, dwork2, dT(0,i-1), dtau+i-1);
                }

                /* dwork = V**H c */
                hipLaunchKernelGGL(magma_sgemv_kernel1, dim3(i-b), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m-b, dA(b, b),  ldda, dA(b,i), dwork2);

                /* dwork = T**H dwork2 */
                hipLaunchKernelGGL(magma_strmv_tkernel, dim3(i-b), dim3(i-b), 0, queue->hip_stream() , dT(b,b), k, dwork2, dwork2+i-b);

                /* c = c - V dwork2 */
                if ( m-b > 0 ) {
                    dim3  blocks3( magma_ceildiv( m-b, BLOCK_SIZE ) );
                    dim3 threads3( BLOCK_SIZE );
                    hipLaunchKernelGGL(magma_sgemv_kernel2, dim3(blocks3), dim3(threads3), 0, queue->hip_stream() , m-b, i-b, dA(b,b), ldda,  dwork2+i-b, dA(b, i));
                }
            }

            /*   Adjust the dnorm[i] to hold the norm of A(i:m,i) */
            if ( i > 0 ) {
                hipLaunchKernelGGL(magma_snrm2_adjust_kernel, dim3(1), dim3(i), 0, queue->hip_stream() , dnorm+i, dA(0, i));
            }
