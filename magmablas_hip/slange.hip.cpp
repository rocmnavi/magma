...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlange.hip.cpp, normal z -> s, Mon Jul 15 16:57:39 2024
       @author Mark Gates
*/
...
/* Based on slange_one_kernel code, above.
    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        hipLaunchKernelGGL(slange_inf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaMaxNorm ) {
        dim3 grid( magma_ceildiv( m, NB_X ) );
        hipLaunchKernelGGL(slange_max_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaOneNorm ) {
        dim3 grid( n );
        hipLaunchKernelGGL(slange_one_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  n, dwork );  // note n instead of m
    }
    else if ( norm == MagmaFrobeniusNorm ) {
        dim3 grid( n );
        hipLaunchKernelGGL(slange_fro_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_sum_reduce_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  n, dwork );  // note n instead of m
    }
