...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zpotf2.hip.cpp, normal z -> d, Mon Oct 28 11:12:15 2024
*/
...
void dpotf2_dscal( magma_int_t n, double *x, magma_int_t incx, magma_int_t* device_info, magma_queue_t queue );
void dpotf2_ddot(magma_int_t n, double *x, magma_int_t incx, int step, magma_int_t* device_info, magma_queue_t queue );
...
#define magma_dpotf2_gemv magmablas_dgemv

#else
#define magma_dpotf2_gemv magma_dgemv
    ...
    if (uplo == MagmaUpper) {
        for (j = 0; j < n; j++) {
            dpotf2_ddot( j, dA(0,j), 1, step+j, device_info, queue ); // including ddot product and update a(j,j)
            if (j < n) {
                #ifdef COMPLEX
                magmablas_dlacgv( j, dA(0, j), 1, queue );
                #endif
                magma_dpotf2_gemv( MagmaTrans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                ...
                magmablas_dlacgv( j, dA(0, j), 1, queue );
                #endif
                dpotf2_dscal( n-j, dA(j,j), ldda, device_info, queue );
            }
        }
    }
    else {
        for (j = 0; j < n; j++) {
            dpotf2_ddot( j, dA(j,0), ldda, step+j, device_info, queue ); // including ddot product and update a(j,j)
            if (j < n) {
                #ifdef COMPLEX
                magmablas_dlacgv( j, dA(j, 0), ldda, queue );
                #endif
                magma_dpotf2_gemv( MagmaNoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                ...
                magmablas_dlacgv( j, dA(j, 0), ldda, queue );
                #endif
                dpotf2_dscal( n-j, dA(j,j), 1, device_info, queue );
            }
        }
    }
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    magma_dpotf2_native(uplo, n, dA, ldda, 0, device_info, queue );
...
__global__ void dpotf2_ddot_kernel(int n, double *x, int incx, int threadSize, int step, magma_int_t* device_info)
{
    ...
    if (tx == 0) {
        double xreal = MAGMA_D_REAL(x[n*incx]) - sdata[0];
        if(xreal < 0) {
            device_info[0] = (magma_int_t)step;
        }
        else{
            x[n*incx] = MAGMA_D_MAKE( sqrt(xreal), 0 );
        }
    }
}
    ...
    int step, magma_int_t* device_info, magma_queue_t queue )
{
    ...
    */
    ...
    hipLaunchKernelGGL(dpotf2_ddot_kernel, dim3(1), dim3(threadSize), shmem, queue->hip_stream() , n, x, incx, threadSize, step, device_info);
}

__global__ void dpotf2_dscal_kernel(int n, double *x, int incx, magma_int_t* device_info)
{
    ...
    magma_int_t* device_info, magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(dpotf2_dscal_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, x, incx, device_info);
}


#ifdef COMPLEX

__global__ void dpotf2_dlacgv_kernel(int n, double *x, int incx)
{
    ...
    magma_queue_t queue )
{
    ...
    dim3 grid(num_blocks,1);
    hipLaunchKernelGGL(dpotf2_dlacgv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, x, incx);
}
