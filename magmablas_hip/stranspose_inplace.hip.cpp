...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/ztranspose_inplace.hip.cpp, normal z -> s, Mon Jul 15 16:57:42 2024
    ...
    float *matrix, int lda )
{
    ...
    float *A = matrix + ii+i + (size_t)(jj+j)*(size_t)lda;
    if ( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        float *B = matrix + jj+i + (size_t)(ii+j)*(size_t)lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
    ...
    magma_queue_t queue )
{
    ...
    if ( nblock % 2 == 1 ) {
        ...
        hipLaunchKernelGGL(stranspose_inplace_odd, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
    else {
        dim3 grid( nblock+1, nblock/2 );
        hipLaunchKernelGGL(stranspose_inplace_even, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
}
