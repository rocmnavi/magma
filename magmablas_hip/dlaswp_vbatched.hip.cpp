...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlaswp_vbatched.hip.cpp, normal z -> d, Mon Oct 28 11:12:25 2024
...
__global__ void dlaswp_left_rowserial_kernel_vbatched(
                int n,
...
__global__ void dlaswp_right_rowserial_kernel_vbatched(
                int n,
                magma_int_t *M, magma_int_t *N,
                double **dA_array, int Ai, int Aj, magma_int_t *ldda,
                magma_int_t** ipiv_array, int ipiv_i,
...
/******************************************************************************/
__global__
void dlaswp_left_rowparallel_kernel_vbatched(
                                int n, int width,
                                magma_int_t* M, magma_int_t* N,
                                double **dA_array,  int Ai,  int Aj,  magma_int_t* ldda,
                                ...
                                int k1, int k2)
{
    ...
    const int my_max_n = Ai - Aj;
    const int my_n     = min(n, my_max_n);
    dlaswp_rowparallel_devfunc( my_n, width, my_height,
                                dA, my_ldda,
                                dA, my_ldda,
                                pivinfo);
}
...
// serial swap that does swapping one row by one row
// this is the vbatched routine, for swapping to the right of the panel
__global__ void dlaswp_right_rowparallel_kernel_vbatched(
                int n, int width,
                magma_int_t *M, magma_int_t *N,
                double **dA_array, int Ai, int Aj, magma_int_t *ldda,
                magma_int_t** pivinfo_array, int pivinfo_i,
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dlaswp_left_rowserial_kernel_vbatched, dim3(grid), dim3(min_BLK_SIZE_n), 0, queue->hip_stream() , n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_offset, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dlaswp_right_rowserial_kernel_vbatched, dim3(grid), dim3(min_BLK_SIZE_n), 0, queue->hip_stream() , n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_offset, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        size_t shmem = sizeof(double) * height * width;
        hipLaunchKernelGGL(dlaswp_left_rowparallel_kernel_vbatched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, width, M, N, dA_array, Ai, Aj, ldda, pivinfo_array, pivinfo_i, k1, k2);
    }
}
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        size_t shmem = sizeof(double) * height * width;
        hipLaunchKernelGGL(dlaswp_right_rowparallel_kernel_vbatched, dim3(grid), dim3(height), shmem, queue->hip_stream() ,  n, width, M, N, dA_array, Ai, Aj, ldda, pivinfo_array, pivinfo_i, k1, k2);
    }
}
