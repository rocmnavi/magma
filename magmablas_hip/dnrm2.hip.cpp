...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/dznrm2.hip.cpp, normal z -> d, Thu Jan 25 22:57:55 2024
    ...
    magma_queue_t queue ) 
{
    ...
    dim3 blocks( n );    
    hipLaunchKernelGGL(magmablas_dnrm2_check_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, dxnorm, dlsticc );
}
    ...
    magma_queue_t queue )
{
    ...
    dim3 blocks( 1, 1 );
    hipLaunchKernelGGL(magmablas_dnrm2_smkernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dxnorm );
}
    ...
    magma_queue_t queue )
{
    dim3 threads( k );
    dim3 blocks( 1 );
    hipLaunchKernelGGL(magma_dnrm2_adjust_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() , dxnorm, dc);
}
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_dnrm2_row_check_adjust_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() , k, tol, dxnorm, dxnorm2, dC, lddc, dlsticc);
}
    ...
    magma_queue_t queue ) 
{
    dim3 threads( BLOCK_SIZE );
    dim3 blocks( n );    
    hipLaunchKernelGGL(magmablas_dnrm2_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, dxnorm );
