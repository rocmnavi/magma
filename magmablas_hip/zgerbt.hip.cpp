    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magmablas_zapply_transpose_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n1, nrhs, du,    n, db, lddb,  0);
    hipLaunchKernelGGL(magmablas_zapply_transpose_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n2, nrhs, du, n+n1, db, lddb, n1);

    threads = block_length;
    grid = magma_ceildiv( n, 2*block_length );
    hipLaunchKernelGGL(magmablas_zapply_transpose_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, nrhs, du, 0, db, lddb, 0);
}
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magmablas_zapply_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, nrhs, dv, 0, db, lddb, 0);

    threads = block_length;
    grid = magma_ceildiv( n, 4*block_length );

    hipLaunchKernelGGL(magmablas_zapply_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n1, nrhs, dv,    n, db, lddb,  0);
    hipLaunchKernelGGL(magmablas_zapply_vector_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n2, nrhs, dv, n+n1, db, lddb, n1);
}
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magmablas_zelementary_multiplication_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n1, n1, dA,  0,  0, ldda, du,  0, dv,  0);
    hipLaunchKernelGGL(magmablas_zelementary_multiplication_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n1, n2, dA,  0, n1, ldda, du,  0, dv, n1);
    hipLaunchKernelGGL(magmablas_zelementary_multiplication_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n2, n1, dA, n1,  0, ldda, du, n1, dv,  0);
    hipLaunchKernelGGL(magmablas_zelementary_multiplication_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n2, n2, dA, n1, n1, ldda, du, n1, dv, n1);

    dim3 threads2(block_height, block_width);
    dim3 grid2( magma_ceildiv( n, 2*block_height ),
                magma_ceildiv( n, 2*block_width  ));
    hipLaunchKernelGGL(magmablas_zelementary_multiplication_kernel, dim3(grid2), dim3(threads2), 0, queue->hip_stream() , n, n, dA, 0, 0, ldda, du, -n, dv, -n);
}
