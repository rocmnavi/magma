...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlaswp.hip.cpp, normal z -> d, Mon Oct 28 11:12:09 2024
    ...
    dlaswp_params_t params )
{
    ...
    if ( tid < n ) {
        ...
        for( int i1 = 0; i1 < params.npivots; ++i1 ) {
            ...
            double *A2 = dAT + (size_t)i2*(size_t)ldda;
            double temp = *A1;
            *A1 = *A2;
            *A2 = temp;
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        hipLaunchKernelGGL(dlaswp_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k,0), ldda, params );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( int k = k1-1; k < k2; k += MAX_PIVOTS ) {
        ...
        hipLaunchKernelGGL(dlaswpx_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA(k,0), ldx, ldy, params );
    }
    ...
    magma_queue_t queue )
{
    ...
    dim3 threads( NTHREADS );
    dim3 grid( magma_ceildiv( n, NTHREADS ) );
    hipLaunchKernelGGL(dlaswp2_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dAT(k1-1,0), ldda, nb, d_ipiv, inci );
}
