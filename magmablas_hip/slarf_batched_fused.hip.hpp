...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarf_batched_fused.hip.hpp, normal z -> s, Thu Jan 25 22:58:59 2024
*/
    ...
    for(iib = 0; iib < (n/NB)*NB; iib+=NB) {
        ...
        for(int j = 0; j < NB; j++) {
            ...
            for(int jj = 0; jj < NB; jj++) {
                sA(tx,jj) = MAGMA_S_CONJ( sV(tx,j) ) * rA[jj];
            }
            __syncthreads();
    ...
    if(n - iib > 0) {
        ...
        for(int j = 0; j < NB; j++) {
            ...
            if(ty_ < nn) {
                #pragma unroll
                for(i = 0; i < M32-TPC; i+=TPC) {
                    zsum += sA(tx_+i,ty_) * MAGMA_S_CONJ( sV(tx_+i,j) );
                }
...
////////////////////////////////////////////////////////////////////////////////
//              SLARF fused register kernel driver
////////////////////////////////////////////////////////////////////////////////
template<int M32, int NB>
static magma_int_t
magma_slarf_fused_reg_kernel_driver_batched(
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    magma_int_t shmem = 0;
    shmem += SLDA(M32) * NB * sizeof(float);  // sA
    shmem += SLDA(M32) * NB * sizeof(float);  // sV
    ...
    if (shmem <= shmem_max) {
        cudaFuncSetAttribute(slarf_fused_reg_kernel_batched<M32, NB, TPC>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem);
    }
    #else
    ...
    hipError_t e = hipLaunchKernel((void*)slarf_fused_reg_kernel_batched<M32, NB, TPC>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
...
template<int M32>
static magma_int_t
magma_slarf_fused_reg_NB_batched(
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    magma_int_t arginfo = 0;
    switch(nb) {
        case 1: arginfo = magma_slarf_fused_reg_kernel_driver_batched<M32, 1>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, check_launch_only, batchCount, queue ); break;
        case 2: arginfo = magma_slarf_fused_reg_kernel_driver_batched<M32, 2>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, check_launch_only, batchCount, queue ); break;
        case 4: arginfo = magma_slarf_fused_reg_kernel_driver_batched<M32, 4>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, check_launch_only, batchCount, queue ); break;
        case 8: arginfo = magma_slarf_fused_reg_kernel_driver_batched<M32, 8>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, check_launch_only, batchCount, queue ); break;
        #if defined(MAGMA_HAVE_CUDA) && !defined(PRECISION_z)
        case 16: arginfo = magma_slarf_fused_reg_kernel_driver_batched<M32,16>( m, n, ib, dA_array, Ai, Aj, ldda, dV_array, Vi, Vj, lddv, dtau_array, taui, check_launch_only, batchCount, queue ); break;
        #endif
        default: arginfo = -100;
    }
    return arginfo;
