    ...
    if (uplo == MagmaLower) {
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(zlaset_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(zlaset_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, offdiag, dA(i*super_NB, j*super_NB), ldda );
                }
            }
        }
    }
        ...
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(zlaset_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(zlaset_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, offdiag, dA(i*super_NB, j*super_NB), ldda );
                }
        ...
        {
            size_t size = m*n;
            hipError_t err = hipMemsetAsync( dA, 0, size*sizeof(magmaDoubleComplex), queue->hip_stream() );
            ...
            for( unsigned int i=0; i < super_grid.x; ++i ) {
                ...
                for( unsigned int j=0; j < super_grid.y; ++j ) {  // full row
                    ...
                    grid.y = magma_ceildiv( nn, BLK_Y );
                    if ( i == j ) {  // diagonal super block
                        hipLaunchKernelGGL(zlaset_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                    }
                    else {           // off diagonal super block
                        hipLaunchKernelGGL(zlaset_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, offdiag, offdiag, dA(i*super_NB, j*super_NB), ldda );
                    }
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if (uplo == MagmaLower) {
            hipLaunchKernelGGL(zlaset_lower_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
        else if (uplo == MagmaUpper) {
            hipLaunchKernelGGL(zlaset_upper_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
        else {
            hipLaunchKernelGGL(zlaset_full_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, offdiag, diag, dAarray+i, Ai, Aj, ldda);
        }
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if (uplo == MagmaLower) {
            hipLaunchKernelGGL(zlaset_lower_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
        else if (uplo == MagmaUpper) {
            hipLaunchKernelGGL(zlaset_upper_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
        else {
            hipLaunchKernelGGL(zlaset_full_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m+i, n+i, offdiag, diag, dAarray+i, ldda+i);
        }
    }
}
