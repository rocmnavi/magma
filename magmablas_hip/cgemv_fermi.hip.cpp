...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgemv_fermi.hip.cpp, normal z -> c, Mon Oct 28 11:12:03 2024
*/
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(cgemvn_template_kernel_fermi<DIM_X, DIM_Y, TILE_SIZE>), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
}
    ...
    magma_queue_t queue)
{
    ...
    if (trans == MagmaConjTrans) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cgemvc_template_kernel_fermi< DIM_X, DIM_Y, TILE_SIZE, MagmaConjTrans >), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
    }
    else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cgemvc_template_kernel_fermi< DIM_X, DIM_Y, TILE_SIZE, MagmaTrans >), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
    }
}
    ...
    magma_queue_t queue)
{
    ...
    if( m == 0 || n == 0 || (alpha == MAGMA_C_ZERO && beta == MAGMA_C_ONE) ) return;
