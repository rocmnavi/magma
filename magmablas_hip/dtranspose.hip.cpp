...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/ztranspose.hip.cpp, normal z -> d, Fri Aug 25 13:17:00 2023
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(dtranspose_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dAT, lddat );
}
    ...
    magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dtranspose_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA_array+i, ldda, dAT_array+i, lddat );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        dim3 grid( magma_ceildiv( m, NB ), magma_ceildiv( n, NB ), ibatch );
        hipLaunchKernelGGL(dtranspose_kernel_batched_stride, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, stride, dA_array+i, ldda, dAT_array+i, lddat );
    }
}
