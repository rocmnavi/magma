...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/ztranspose_inplace.hip.cpp, normal z -> d, Mon Oct 28 11:12:10 2024
    ...
    double *matrix, int lda )
{
    ...
    double *A = matrix + ii+i + (size_t)(jj+j)*(size_t)lda;
    if ( ii == jj ) {
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        ...
        double *B = matrix + jj+i + (size_t)(ii+j)*(size_t)lda;
        if ( ii+i < n && jj+j < n ) {
            sA[j][i] = *A;
        }
        if ( jj+i < n && ii+j < n ) {
    ...
    magma_queue_t queue )
{
    ...
    if ( nblock % 2 == 1 ) {
        ...
        hipLaunchKernelGGL(dtranspose_inplace_odd, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
    else {
        dim3 grid( nblock+1, nblock/2 );
        hipLaunchKernelGGL(dtranspose_inplace_even, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, dA, ldda );
    }
}
