        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(izamax_kernel_vbatched, dim3(grid), dim3(threads), zamax * (sizeof(double) + sizeof(int)), queue->hip_stream() , length, M, N, dA_array, Ai, Aj, ldda, ipiv_array, ipiv_i, info_array, step, gbstep );
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(zswap_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() , max_n, M, N, dA_array, Ai, Aj, ldda, ipiv_array, piv_adjustment);
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(zscal_zgeru_1d_generic_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream(), max_M, max_N, M+i, N+i, dA_array+i, Ai, Aj, ldda+i, info_array+i, step, gbstep);
    }
    return 0;
}
...
{
    HIP_DYNAMIC_SHARED( magmaDoubleComplex, data)
    const int tx = threadIdx.x;
    ...
    magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)zgetf2_fused_kernel_vbatched<max_N>, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
        //printf("error in %s : failed to launch kernel %s\n", __func__, hipGetErrorString(e));
        arginfo = -100;
    }
