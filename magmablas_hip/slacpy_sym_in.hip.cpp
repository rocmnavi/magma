...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlacpy_sym_in.hip.cpp, normal z -> s, Thu Jan 25 22:57:51 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(slacpy_sym_in_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(slacpy_sym_in_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
    else {
        hipLaunchKernelGGL(slacpy_sym_in_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dB, lddb );
    }
}
