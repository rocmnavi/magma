...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarfbx.hip.cpp, normal z -> s, Mon Jul 15 16:57:39 2024
...
/*
    Call 
        hipLaunchKernelGGL(magma_sgemv_kernel3, dim3(n), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork, tau)
    to compute
...
{
    /* dwork = V**H c     */
    hipLaunchKernelGGL(magma_sgemv_kernel1, dim3(k), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork); 

    /* dwork = T**H dwork */
    hipLaunchKernelGGL(magma_strmv_tkernel, dim3(k), dim3(k), 0, queue->hip_stream() ,  dT, ldt, dwork, dwork+k);
 
    /* c = c - V dwork    */
    dim3  blocks3( magma_ceildiv( m, BLOCK_SIZE ) );
    dim3 threads3( BLOCK_SIZE );     
    hipLaunchKernelGGL(magma_sgemv_kernel2, dim3(blocks3), dim3(threads3), 0, queue->hip_stream() ,  m, k, V, ldv, dwork+k, c);
}
