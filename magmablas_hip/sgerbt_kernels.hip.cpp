...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgerbt_kernels.hip.cpp, normal z -> s, Mon Oct 28 11:12:04 2024
    ...
    float *dv, int Vi)
{
    ...
    float a00 = MAGMA_S_ZERO, a10 = MAGMA_S_ZERO, a01 = MAGMA_S_ZERO, a11 = MAGMA_S_ZERO;
    float b1 = MAGMA_S_ZERO, b2 = MAGMA_S_ZERO, b3 = MAGMA_S_ZERO, b4 = MAGMA_S_ZERO;
...
magmablas_selementary_multiplication_kernel(
    ...
    float *dv, int Vi)
{
    magmablas_selementary_multiplication_devfunc( Am, An, dA, Ai, Aj, ldda, du, Ui, dv, Vi );
}
    ...
    float *dv, int Vi)
{
    ...
    magmablas_selementary_multiplication_devfunc( Am, An, dA_array[batchid], Ai, Aj, ldda, du, Ui, dv, Vi );
}
    ...
    float *du, float *db)
{
    ...
    a1 = (idx < n1) ? du[ 0] * db[ 0] : MAGMA_S_ZERO;
    a2 = (idx < n2) ? du[n1] * db[n1] : MAGMA_S_ZERO;
    ...
    float *du, int offsetu, float *db, int lddb, int offsetb)
{
    for(int i = 0; i < nrhs; i++) {
        magmablas_sapply_vector_devfunc(n, du+offsetu, db + i*lddb + offsetb);
    }
}
    ...
    float *du, int offsetu, float **db_array, int lddb, int offsetb )
{
    ...
    for(int i = 0; i < nrhs; i++) {
        magmablas_sapply_vector_devfunc(n, du+offsetu, db_array[batchid]+ i * lddb + offsetb);
    }
}
    ...
    float *du,float *db )
{
    ...
    if (idx < n1) {
        ...
        a1 = db[0] + ((idx < n2) ? db[n1] : MAGMA_S_ZERO);
        a2 = db[0] - ((idx < n2) ? db[n1] : MAGMA_S_ZERO);
    }
...
{
    for(int i = 0; i < nrhs; i++) {
        magmablas_sapply_transpose_vector_devfunc(n, du+offsetu, db+ i*lddb + offsetb);
    }
}
    ...
    float *du, int offsetu, float **db_array, int lddb, int offsetb )
{
    int batchid = blockIdx.y;
    for(int i = 0; i < nrhs; i++) {
        magmablas_sapply_transpose_vector_devfunc(n, du+offsetu, db_array[batchid]+ i*lddb + offsetb);
    }
}
