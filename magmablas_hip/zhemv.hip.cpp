    ...
    magma_queue_t queue )
{
    ...
    if ( upper ) {
        hipLaunchKernelGGL(zhemv_kernel_U, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(zhemv_kernel_U_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    else {
        hipLaunchKernelGGL(zhemv_kernel_L, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, dx, incx, dwork);
        
        hipLaunchKernelGGL(zhemv_kernel_L_sum, dim3(grid), dim3(threads_sum), 0, queue->hip_stream() , n, alpha, ldda, beta, dy, incy, dwork);
    }
    return info;
}
