...
/*
    Call 
        hipLaunchKernelGGL(magma_zgemv_kernel3, dim3(n), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork, tau)
    to compute
...
{
    /* dwork = V**H c     */
    hipLaunchKernelGGL(magma_zgemv_kernel1, dim3(k), dim3(BLOCK_SIZE), 0, queue->hip_stream() , m, V, ldv, c, dwork); 

    /* dwork = T**H dwork */
    hipLaunchKernelGGL(magma_ztrmv_tkernel, dim3(k), dim3(k), 0, queue->hip_stream() ,  dT, ldt, dwork, dwork+k);
 
    /* c = c - V dwork    */
    dim3  blocks3( magma_ceildiv( m, BLOCK_SIZE ) );
    dim3 threads3( BLOCK_SIZE );     
    hipLaunchKernelGGL(magma_zgemv_kernel2, dim3(blocks3), dim3(threads3), 0, queue->hip_stream() ,  m, k, V, ldv, dwork+k, c);
}
