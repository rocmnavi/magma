...
/*
   ...
   @generated from magmablas_hip/zgetf2_devicefunc.hip.hpp, normal z -> c, Fri Aug 25 13:18:26 2023
 */


#ifndef MAGMABLAS_CGETF2_DEVICES_Z_H
#define MAGMABLAS_CGETF2_DEVICES_Z_H
...
// kernel level (unlike cswap_device)
static __device__ __inline__
void cswap_device_v2(
            magma_int_t n,
            magmaFloatComplex_ptr x1, magma_int_t incx1,
            magmaFloatComplex_ptr x2, magma_int_t incx2 )
{
    const int tx = threadIdx.x;

    if (tx < n) {
        magmaFloatComplex tmp  = x1[tx * incx1];
                         ...
                         magma_int_t *info, int step, int gbstep)
{
    ...
    float rTmp = fabs(MAGMA_C_REAL( shared_y[0] ) ) + fabs( MAGMA_C_IMAG( shared_y[0] ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_C_ONE : MAGMA_C_DIV(MAGMA_C_ONE, shared_y[0]);
...
/******************************************************************************/
static __device__ __inline__
void cscal_cgeru_generic_device( int m, int n,
                         magmaFloatComplex_ptr dA, int lda,
                         magma_int_t *info, int step, int gbstep)
{
    const int tx  = threadIdx.x;
    ...
    float rTmp;
    rA   = dA[0];
    rTmp = fabs(MAGMA_C_REAL( rA ) ) + fabs( MAGMA_C_IMAG( rA ) );

    reg = (rTmp == MAGMA_D_ZERO) ? MAGMA_C_ONE : MAGMA_C_DIV(MAGMA_C_ONE, rA);
    rA  = dA[ gtx ];
    rA *= reg;
...
static __device__ __inline__
void
cgetf2_fused_device( int m, int minmn, magmaFloatComplex rA[WIDTH], magma_int_t* dipiv,
                     magmaFloatComplex* swork, int &linfo, int gbstep, int &rowid)
{
    const int tx = threadIdx.x;
...
#endif // MAGMABLAS_CGETF2_DEVICES_Z_H
