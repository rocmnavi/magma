...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zsymmetrize_tiles.hip.cpp, normal z -> c, Fri Aug 25 13:17:00 2023
       @author Mark Gates
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaUpper ) {
        hipLaunchKernelGGL(csymmetrize_tiles_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, mstride, nstride );
    }
    else {
        hipLaunchKernelGGL(csymmetrize_tiles_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dA, ldda, mstride, nstride );
    }
}
