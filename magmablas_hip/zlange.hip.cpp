    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        hipLaunchKernelGGL(zlange_inf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaMaxNorm ) {
        dim3 grid( magma_ceildiv( m, NB_X ) );
        hipLaunchKernelGGL(zlange_max_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaOneNorm ) {
        dim3 grid( n );
        hipLaunchKernelGGL(zlange_one_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        ...
        dim3 grid( n );
        hipLaunchKernelGGL(zlange_fro_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
