    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(zgemvn_template_kernel_fermi<DIM_X, DIM_Y, TILE_SIZE>), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
}
    ...
    magma_queue_t queue)
{
    ...
    if (trans == MagmaConjTrans) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zgemvc_template_kernel_fermi< DIM_X, DIM_Y, TILE_SIZE, MagmaConjTrans >), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
    }
    else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zgemvc_template_kernel_fermi< DIM_X, DIM_Y, TILE_SIZE, MagmaTrans >), dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, alpha, A, lda, x, incx, beta, y, incy);
    }
}
