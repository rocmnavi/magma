...
/*                                                                                                     
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zheinertia.hip.cpp, normal z -> s, Fri Aug 25 13:16:52 2023 
*/
...
__global__ void
ssiinertia_upper_kernel(int n, magmaFloat_const_ptr dA, int ldda, magma_int_t *ipiv, int *dneig)
{
    ...
    if (k<n && k-1>=0 )
        if ( ipiv[k]-1 < 0 && ipiv[k-1] == ipiv[k]) {
            ...
            if ( count%2 == 1 ){
                diag = MAGMA_S_ABS(dA[(k-1)+k*ldda]);
                t=0.0;
                if (diag > 0.) 
                    peig++;
                else if (diag < 0.)
    ...
    for(i=i; i<sc; i++){
        ...
        if (ipiv[k]-1 < 0){   
            if (t != 0.) {
                ...
                t = MAGMA_S_ABS( dA[k+(k+1)*ldda] ); 
                diag = (diag/t) * MAGMA_S_REAL( dA[(k+1)*(1+ldda)] ) - t; 
            }
        }
...
__global__ void
ssiinertia_lower_kernel(int n, magmaFloat_const_ptr dA, int ldda, magma_int_t *ipiv, int *dneig)
{
    const int tx  = threadIdx.x;
    const int blk = blockIdx.x;
    int peig = 0, neig = 0, zeig = 0;
    ...
    if (k<n && k-1>=0 )
        if ( ipiv[k]-1 < 0 && ipiv[k-1] == ipiv[k]) {
            ...
            // if count is odd, it means that the current pivot is a second element of a 2-by-2 diagonal block
            if ( count%2 == 1 ){
                diag = MAGMA_S_ABS(dA[(k-1)*ldda+k]);
                t=0.0;
                if (diag > 0.)
                    peig++;
                else if (diag < 0.)
...
/***************************************************************************//**
    Purpose
    -------
    magmablas_ssiinertia computes the inertia of a symmetric and block 
    ...
    magma_queue_t queue )
{
    ...
    if (upper)
        hipLaunchKernelGGL(ssiinertia_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, ipiv, dneig);
    else
        hipLaunchKernelGGL(ssiinertia_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, dA, ldda, ipiv, dneig);

    return info;
}

// end magmablas_ssiinertia
