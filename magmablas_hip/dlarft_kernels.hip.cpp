...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarft_kernels.hip.cpp, normal z -> d, Mon Jul 15 16:57:40 2024
       @author Azzam Haidar
*/
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(dlarft_dtrmv_sm32x32_kernel, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau, Tin, ldtin, Tout, ldtout);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dlarft_dtrmv_sm32x32_kernel_batched, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau_array+i, taui,
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(dlarft_recdtrmv_sm32x32_kernel, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(dlarft_recdtrmv_sm32x32_kernel_batched, dim3(grid), dim3(threads), shmem, queue->hip_stream() ,  m, n,  tau_array+i, taui,
