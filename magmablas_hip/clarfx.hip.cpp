...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlarfx.hip.cpp, normal z -> c, Mon Jul 15 16:57:40 2024
    ...
    magma_queue_t queue )
{
    ...
    if (iter == 0) {
        hipLaunchKernelGGL(magma_clarfx_kernel, dim3(N), dim3(BLOCK_SIZE), 0, queue->hip_stream() ,  m, v, tau, C, ldc, xnorm, dT+iter*N, iter );
    }
    else {
        hipLaunchKernelGGL(magma_clarfx_kernel, dim3(N), dim3(BLOCK_SIZE), 0, queue->hip_stream() ,  m, v, tau, C, ldc, xnorm, work, iter );
    }

    if (iter > 0) {
        //magma_ctrmv_kernel
        //    <<< 1, iter, 0, queue->hip_stream() >>>
        //    ( dT, N, dT+iter*N);
        hipLaunchKernelGGL(magma_ctrmv_kernel2, dim3(iter), dim3(iter), 0, queue->hip_stream() ,  dT, N, work, dT+iter*N, tau );
    }
}
