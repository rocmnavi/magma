    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        hipLaunchKernelGGL(zlascl_diag_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, lddd, dA, ldda);
    }
    else if (type == MagmaUpper) {
        hipLaunchKernelGGL(zlascl_diag_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, dD, lddd, dA, ldda);
    }
}
