...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlascl.hip.cpp, normal z -> c, Mon Jul 15 16:57:40 2024
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    while( ! done ) {
        ...
        if (type == MagmaLower) {
            hipLaunchKernelGGL(clascl_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaUpper) {
            hipLaunchKernelGGL(clascl_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaFull) {
            hipLaunchKernelGGL(clascl_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
