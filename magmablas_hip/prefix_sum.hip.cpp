        ...
        magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(prefix_sum_kernel, dim3(grid_sum), dim3(threads_sum), 0, queue->hip_stream(), ivec, ovec, length, workspace, 1);
    ...
    if(nTB > 1)
    {
        ...
        hipLaunchKernelGGL(prefix_sum_kernel, dim3(grid_sumw), dim3(threads_sumw), 0, queue->hip_stream(), workspace, workspace, lwork, (magma_int_t*)NULL, 0);
        
        // update the sum
        dim3 threads_update(SCAN_SEG_SIZE, 1, 1);
        dim3 grid_update(nTB-1, 1, 1);
        hipLaunchKernelGGL(prefix_update_kernel, dim3(grid_update), dim3(threads_update), 0, queue->hip_stream(), ovec, length, workspace);
    }
}
//----------------------------------------------------------------------------------------
void 
