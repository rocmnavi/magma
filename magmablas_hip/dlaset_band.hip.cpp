...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlaset_band.hip.cpp, normal z -> d, Fri Aug 25 13:16:58 2023
    ...
    magma_queue_t queue)
{
    ...
    if (uplo == MagmaUpper) {
        ...
        hipLaunchKernelGGL(dlaset_band_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, offdiag, diag, dA, ldda);
    }
    else if (uplo == MagmaLower) {
        dim3 threads( min(k,m) );
        dim3 grid( magma_ceildiv( min(m,n), NB ) );
        hipLaunchKernelGGL(dlaset_band_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, offdiag, diag, dA, ldda);
    }
}
