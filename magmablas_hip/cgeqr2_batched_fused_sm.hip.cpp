...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zgeqr2_batched_fused_sm.hip.cpp, normal z -> c, Mon Jul 15 16:57:47 2024
*/

#include <hip/hip_runtime.h>    // for CUDA_VERSION
#include "magma_internal.h"
#include "magma_templates.h"
#include "cgeqr2_batched_fused.hip.hpp"
    ...
    magma_int_t *info_array, magma_int_t batchCount)
{
    ...
    for(int j = 0; j < N; j++){
        ...
        cgeqr2_compute_norm(M-j-1, &sA(j+1,j), snorm, tx, ntx);
        // there is a sync at the end of cgeqr2_compute_norm
        norm_no_alpha = snorm[0];
        norm = norm_no_alpha + MAGMA_C_REAL(alpha) * MAGMA_C_REAL(alpha) + MAGMA_C_IMAG(alpha) * MAGMA_C_IMAG(alpha);
        norm = sqrt(norm);
        ...
        // have the diagonal
        alpha = (zero_nrm) ? alpha : MAGMA_C_MAKE(beta, MAGMA_D_ZERO);
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    hipError_t e = hipLaunchKernel((void*)cgeqr2_fused_sm_kernel_batched, grid, threads, kernel_args, shmem, queue->hip_stream());
    if( e != hipSuccess ) {
