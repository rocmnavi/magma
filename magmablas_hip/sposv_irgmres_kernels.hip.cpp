...
/*
   ...
   @generated from magmablas_hip/zposv_irgmres_kernels.hip.cpp, normal z -> s, Mon Jul 15 16:57:52 2024
    ...
    magma_queue_t queue)
{
    ...
    dim3 threads(bx, 1, 1);
    hipLaunchKernelGGL(extract_diag_sqrt_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(), min_mn, dA, ldda, dD, incd);
}
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(sscal_shift_hpd_kernel<DIMX, DIMY>), dim3(grid), dim3(threads), 0, queue->hip_stream(), uplo, n, dA, ldda, dD, incd, miu, cn, eps);
}
        ...
        magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(dimv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(), n, alpha, dD, incd, dx, incx, beta,  dy, incy, true);
}