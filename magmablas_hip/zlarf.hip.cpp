    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zlarf_smkernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, n, dv, dtau, dc, lddc );
}
    ...
    magma_queue_t queue )
{
    ...
    if ( n > 0 ) {
        hipLaunchKernelGGL(magma_zlarf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, dv, dtau, dC, lddc);
    }
