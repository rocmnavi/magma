    ...
    magma_queue_t queue )
{
    ...
    if (incx >= 0)
        hipLaunchKernelGGL(zclaswp_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, m, ipiv);
    else
        hipLaunchKernelGGL(zclaswp_inv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, A, lda, SA, ldsa, m, ipiv);
}
