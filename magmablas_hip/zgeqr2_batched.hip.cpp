...
#include "zlarfg_devicesfunc.hip.hpp"
                     ...
                     magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( total_shmem_sm_kernel <= shmem_max ) {
            ...
            hipLaunchKernelGGL(zgeqr2_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(magmaDoubleComplex)*(m*k), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
            ...
            hipLaunchKernelGGL(zgeqr2_column_sm_kernel_batched, dim3(grid), dim3(threads), sizeof(magmaDoubleComplex)*(m), queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
        else {
            //not use dynamic shared memory at all
            hipLaunchKernelGGL(zgeqr2_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, k, dA_array+i, Ai, Aj, ldda, dtau_array+i, taui);
        }
    }
