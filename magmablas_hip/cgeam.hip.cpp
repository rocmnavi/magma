...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zgeam.hip.cpp, normal z -> c, Fri Aug 25 13:16:51 2023
...
cgeam_kernel_nn(
    ...
    const magmaFloatComplex *B, int ldb,
...
cgeam_kernel_nc(
    ...
    magmaFloatComplex *C, int ldc)
{
    __shared__ magmaFloatComplex sB[NB][NX+1];
...
/***************************************************************************//**
    Purpose
    -------
    cgeam adds/transposes matrices:
    ...
    @param[in]
    dA      COMPLEX array, dimension (LDDA,k), where k is
    ...
    @param[in]
    dB      COMPLEX array, dimension (LDDB,k), where k is
    ...
    magma_queue_t queue )
{
    ...
    if ( MAGMA_C_EQUAL( alpha, MAGMA_C_ZERO ) && 
         MAGMA_C_EQUAL(  beta, MAGMA_C_ZERO ) )
        // set to 0
        magmablas_claset( MagmaFull, m, n, MAGMA_C_ZERO, MAGMA_C_ZERO, dC, lddc, queue );
    else if ( transA == MagmaNoTrans && transB == MagmaNoTrans ){
        dim3 grid( magma_ceildiv( m, NB ), magma_ceildiv( n, NB ) ); 
        hipLaunchKernelGGL(cgeam_kernel_nn, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    }
    else if ( transA == MagmaNoTrans )
        hipLaunchKernelGGL(cgeam_kernel_nc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else if ( transB == MagmaNoTrans )
        hipLaunchKernelGGL(cgeam_kernel_cn, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
    else 
        hipLaunchKernelGGL(cgeam_kernel_cc, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, m,  alpha, dA, ldda, beta, dB, lddb, dC, lddc);
}
