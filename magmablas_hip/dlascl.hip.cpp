...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlascl.hip.cpp, normal z -> d, Fri Aug 25 13:16:57 2023
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    while( ! done ) {
        ...
        if (type == MagmaLower) {
            hipLaunchKernelGGL(dlascl_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaUpper) {
            hipLaunchKernelGGL(dlascl_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
        else if (type == MagmaFull) {
            hipLaunchKernelGGL(dlascl_full, dim3(grid), dim3(threads), 0, queue->hip_stream() , m, n, mul, dA, ldda);
        }
