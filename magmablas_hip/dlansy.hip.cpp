...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @author Mark Gates
       @generated from magmablas_hip/zlanhe.hip.cpp, normal z -> d, Fri Aug 25 13:16:55 2023
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        hipLaunchKernelGGL(dlansy_inf_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
    else {
        hipLaunchKernelGGL(dlansy_inf_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork, n_full_block, n_mod_bs );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        hipLaunchKernelGGL(dlansy_max_kernel_lower, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
    else {
        hipLaunchKernelGGL(dlansy_max_kernel_upper, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  n, A, lda, dwork );
    }
}
