...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zpotf2.hip.cpp, normal z -> s, Mon Jul 15 16:57:45 2024
*/
...
void spotf2_sscal( magma_int_t n, float *x, magma_int_t incx, magma_int_t* device_info, magma_queue_t queue );
void spotf2_sdot(magma_int_t n, float *x, magma_int_t incx, int step, magma_int_t* device_info, magma_queue_t queue );
...
#define magma_spotf2_gemv magmablas_sgemv

#else
#define magma_spotf2_gemv magma_sgemv
    ...
    if (uplo == MagmaUpper) {
        for (j = 0; j < n; j++) {
            spotf2_sdot( j, dA(0,j), 1, step+j, device_info, queue ); // including sdot product and update a(j,j)
            if (j < n) {
                #ifdef COMPLEX
                magmablas_slacgv( j, dA(0, j), 1, queue );
                #endif
                magma_spotf2_gemv( MagmaTrans, j, n-j-1,
                             alpha, dA(0, j+1), ldda,
                                    dA(0, j),   1,
                ...
                magmablas_slacgv( j, dA(0, j), 1, queue );
                #endif
                spotf2_sscal( n-j, dA(j,j), ldda, device_info, queue );
            }
        }
    }
    else {
        for (j = 0; j < n; j++) {
            spotf2_sdot( j, dA(j,0), ldda, step+j, device_info, queue ); // including sdot product and update a(j,j)
            if (j < n) {
                #ifdef COMPLEX
                magmablas_slacgv( j, dA(j, 0), ldda, queue );
                #endif
                magma_spotf2_gemv( MagmaNoTrans, n-j-1, j,
                             alpha, dA(j+1, 0), ldda,
                                    dA(j,0),    ldda,
                ...
                magmablas_slacgv( j, dA(j, 0), ldda, queue );
                #endif
                spotf2_sscal( n-j, dA(j,j), 1, device_info, queue );
            }
        }
    }
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    magma_spotf2_native(uplo, n, dA, ldda, 0, device_info, queue );
...
__global__ void spotf2_sdot_kernel(int n, float *x, int incx, int threadSize, int step, magma_int_t* device_info)
{
    ...
    if (tx == 0) {
        float xreal = MAGMA_S_REAL(x[n*incx]) - sdata[0];
        if(xreal < 0) {
            device_info[0] = (magma_int_t)step;
        }
        else{
            x[n*incx] = MAGMA_S_MAKE( sqrt(xreal), 0 );
        }
    }
}
    ...
    int step, magma_int_t* device_info, magma_queue_t queue )
{
    ...
    */
    ...
    hipLaunchKernelGGL(spotf2_sdot_kernel, dim3(1), dim3(threadSize), shmem, queue->hip_stream() , n, x, incx, threadSize, step, device_info);
}

__global__ void spotf2_sscal_kernel(int n, float *x, int incx, magma_int_t* device_info)
{
    ...
    magma_int_t* device_info, magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(spotf2_sscal_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, x, incx, device_info);
}


#ifdef COMPLEX

__global__ void spotf2_slacgv_kernel(int n, float *x, int incx)
{
    ...
    magma_queue_t queue )
{
    ...
    dim3 grid(num_blocks,1);
    hipLaunchKernelGGL(spotf2_slacgv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() , n, x, incx);
}
