...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlange.hip.cpp, normal z -> d, Fri Aug 25 13:16:55 2023
       @author Mark Gates
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        hipLaunchKernelGGL(dlange_inf_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaMaxNorm ) {
        dim3 grid( magma_ceildiv( m, NB_X ) );
        hipLaunchKernelGGL(dlange_max_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        hipLaunchKernelGGL(magma_max_nan_kernel, dim3(1), dim3(512), 0, queue->hip_stream() ,  m, dwork );
    }
    else if ( norm == MagmaOneNorm ) {
        dim3 grid( n );
        hipLaunchKernelGGL(dlange_one_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
        ...
        dim3 grid( n );
        hipLaunchKernelGGL(dlange_fro_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dA, ldda, dwork );
