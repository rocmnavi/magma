...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zlacpy.hip.cpp, normal z -> d, Thu Jan 25 22:57:51 2024
    ...
    if ( uplo == MagmaLower ) {
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(dlacpy_lower_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(dlacpy_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
            }
        }
    }
        ...
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    hipLaunchKernelGGL(dlacpy_upper_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
                else {           // off diagonal super block
                    hipLaunchKernelGGL(dlacpy_full_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  mm, nn, dA(i*super_NB, j*super_NB), ldda, dB(i*super_NB, j*super_NB), lddb );
                }
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            hipLaunchKernelGGL(dlacpy_lower_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
        else if ( uplo == MagmaUpper ) {
            hipLaunchKernelGGL(dlacpy_upper_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
        else {
            hipLaunchKernelGGL(dlacpy_full_kernel_batched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, dAarray+i, Ai, Aj, ldda, dBarray+i, Bi, Bj, lddb );
        }
    }
}
    ...
    magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        if ( uplo == MagmaLower ) {
            hipLaunchKernelGGL(dlacpy_lower_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else if ( uplo == MagmaUpper ) {
            hipLaunchKernelGGL(dlacpy_upper_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
        else {
            hipLaunchKernelGGL(dlacpy_full_kernel_vbatched, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m+i, n+i, dAarray+i, ldda+i, dBarray+i, lddb+i );
        }
    }
}
