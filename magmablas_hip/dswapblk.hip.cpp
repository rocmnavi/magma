...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas_hip/zswapblk.hip.cpp, normal z -> d, Mon Jul 15 16:57:41 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( order == MagmaColMajor ) {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_dswapblkcm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
    else {
        for( k=(i1-1); k < i2; k += BLOCK_SIZE )
        {
            ...
            hipLaunchKernelGGL(magmagpu_dswapblkrm, dim3(blocks), dim3(blocksize), 0, queue->hip_stream() ,  params );
        }
    }
}
