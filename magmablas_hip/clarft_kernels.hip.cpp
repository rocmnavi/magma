...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from magmablas_hip/zlarft_kernels.hip.cpp, normal z -> c, Fri Aug 25 13:16:56 2023
       @author Azzam Haidar
*/
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(clarft_ctrmv_sm32x32_kernel, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau, Tin, ldtin, Tout, ldtout);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(clarft_ctrmv_sm32x32_kernel_batched, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau_array+i, taui,
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(clarft_recctrmv_sm32x32_kernel, dim3(grid), dim3(threads), shmem, queue->hip_stream() , m, n,  tau, Trec, ldtrec, Ttri, ldttri);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount) {
        ...
        hipLaunchKernelGGL(clarft_recctrmv_sm32x32_kernel_batched, dim3(grid), dim3(threads), shmem, queue->hip_stream() ,  m, n,  tau_array+i, taui,
