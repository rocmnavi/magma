/*
   ...
   @generated from testing/testing_zgetrf_vbatched.cpp, normal z -> d, Mon Jul 15 16:58:07 2024
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
get_LU_forward_error(
    ...
    double *refLU, magma_int_t ref_lda, magma_int_t *refIPIV,
    double *resLU, magma_int_t res_lda, magma_int_t *resIPIV,
    double* error, magma_int_t* pivots_match)
{
    magma_int_t i, j;
    double work[1], matnorm, residual;
    *error = MAGMA_D_ZERO;
    matnorm = lapackf77_dlange("f", &M, &N, refLU, &ref_lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            resLU[i+j*res_lda] = MAGMA_D_SUB( resLU[i+j*res_lda], refLU[i+j*ref_lda] );
        }
    }
    residual = lapackf77_dlange("f", &M, &N, resLU, &res_lda, work);
...
int main( int argc, char** argv)
{
    ...
    magma_int_t     seed = 0;
    double *hA, *hR, *hA_magma, *hTmp;
    double *dA;
    double **dA_array = NULL, **hA_array = NULL, **hR_array = NULL, **hdA_array = NULL;
    ...
    TESTING_CHECK( magma_malloc_cpu((void**)&hR_array,  batchCount * sizeof(double*)) );
    TESTING_CHECK( magma_malloc_cpu((void**)&hdA_array, batchCount * sizeof(double*)) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int s = 0; s < batchCount; s++) {
                ...
                piv_size   += h_min_mn[s];
                gflops     += FLOPS_DGETRF( h_M[s], h_N[s] ) / 1e9;
            }

            TESTING_CHECK( magma_imalloc_cpu( &ipiv,     piv_size ));
            TESTING_CHECK( magma_dmalloc_cpu( &hA,       hA_size  ));
            TESTING_CHECK( magma_dmalloc_cpu( &hA_magma, hA_size  ));
            TESTING_CHECK( magma_dmalloc_pinned( &hR,    hA_size  ));

            TESTING_CHECK( magma_dmalloc( &dA,       dA_size ));
            ...
            lapackf77_dlarnv( &ione, ISEED, &hA_size, hA );
            ...
            memset(hA_array[id] + col1 * h_lda[id], 0, h_M[id] * sizeof(double));
            memset(hA_array[id] + col2 * h_lda[id], 0, h_M[id] * sizeof(double));
            #endif

            memcpy(hR, hA, hA_size * sizeof(double));

            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            magma_setvector(batchCount, sizeof(double*), hdA_array, 1, dA_array, 1, opts.queue);
            ...
            if(opts.version == 1) {
                ...
                info = magma_dgetrf_vbatched_max_nocheck(
                        d_M, d_N, d_min_mn,
            ...
            hTmp = hA_magma;
            for(int s = 0; s < batchCount; s++) {
                magma_dgetmatrix( h_M[s], h_N[s], hdA_array[s], h_ldda[s], hTmp, h_lda[s], opts.queue );
                hTmp += h_lda[s] * h_N[s];
            }
            ...
            if ( opts.lapack ) {
                ...
                for (magma_int_t s=0; s < batchCount; s++) {
                    magma_int_t locinfo;
                    lapackf77_dgetrf(&h_M[s], &h_N[s], hA_array[s], &h_lda[s], hipiv_array[s], &locinfo);
                    if (locinfo != 0) {
