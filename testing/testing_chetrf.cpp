/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zhetrf.cpp, normal z -> c, Thu Jan 25 23:03:09 2024
    ...
    real_Double_t *time)
{
    ...
    magma_cgetmatrix(n, n, dA, ldda, A, lda, opts.queue );
    magma_cgetvector(n, dx, 1, x, 1, opts.queue );
    lapackf77_chetrs( lapack_uplo_const(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    magma_csetvector(n, x, 1, dx, 1, opts.queue );
    ...
    magma_chetrs_gpu( uplo, n, ione, dA, ldda, ipiv, dx, n, &info, opts.queue );
    ...
    if (info != 0) {
        printf("magma_chetrs returned error %lld: %s.\n",
               (long long) info, magma_strerror( info ));
    }
    // reset to original A
    init_matrix( opts, n, n, A, lda );
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (nopiv) {
                ...
                */
                ...
                //    h_A[kk+(N-1)*lda] = h_A[N-1+kk*lda] = 0.;
                TESTING_CHECK( magma_malloc( (void**)&dinert, 3*sizeof(int)) );
                magmablas_cheinertia(opts.uplo, N, d_A, ldda, ipiv, dinert, opts.queue);
                magma_getvector( 3, sizeof(int), dinert, 1, inert, 1, opts.queue );
                printf("inertia: positive / negative / zero = %d / %d / %d\n",
                       inert[0], inert[1], inert[2]);
                magma_free(dinert);
