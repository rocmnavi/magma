/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_ztrsv_batched.cpp, normal z -> c, Mon Jul 15 16:58:04 2024
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (s=0; s < batchCount; s++) {
                magmaFloatComplex* hAs = h_A + s * lda * Ak;
                lapackf77_cgetrf( &Ak, &Ak, hAs, &lda, ipiv, &info );
                for( j = 0; j < Ak; ++j ) {
                    for( i = 0; i < j; ++i ) {
