/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zgeqrf_batched.cpp, normal z -> c, Mon Oct 28 11:12:46 2024
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            #endif
            #else
            hipblasCgeqrfBatched( opts.handle, int(M), int(N),
                                 (hipblasComplex**)dA_array, int(ldda),
                                 (hipblasComplex**)dtau_array,
                                 &device_info, int(batchCount) );
            #endif
            ...
            if (opts.check) {
                ...
                TESTING_CHECK( magma_cmalloc_cpu( &Q,    batchCount*ldq*min_mn ));  // M by K
                TESTING_CHECK( magma_cmalloc_cpu( &R,    batchCount*ldr*N ));       // K by N
