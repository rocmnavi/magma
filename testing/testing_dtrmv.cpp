/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_ztrmv.cpp, normal z -> d, Mon Oct 28 11:12:28 2024
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &hxmagma, N      ));
            ...
            magmablas_dtrmv( opts.uplo, opts.transA, opts.diag,
                         N,
                         dA(0,0), ldda,
                         dx(0),   1, opts.queue );
            ...
            magma_dgetvector( N, dx(0), 1, hxmagma, 1, opts.queue );
            ...
            if ( opts.lapack ) {
                ...
                // initial C = 0, alpha = 1
                blasf77_daxpy( &N, &c_neg_one, hx, &ione, hxmagma, &ione );
                magma_error = lapackf77_dlange( "F", &N, &ione, hxmagma, &N, work )
                ...
                blasf77_daxpy( &N, &c_neg_one, hxdev, &ione, hxmagma, &ione );
                magma_error = lapackf77_dlange( "F", &N, &ione, hxmagma, &N, work )
                            / (sqrt(double(N+2))*Anorm*Xnorm);
