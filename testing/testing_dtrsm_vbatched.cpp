/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_ztrsm_vbatched.cpp, normal z -> d, Mon Oct 28 11:12:52 2024
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                for (magma_int_t s=0; s < batchCount; s++) {
                    ...
                    normR = lapackf77_dlange( "I", &h_M[s], &h_N[s], h_X_tmp,      &h_ldb[s], work );
                    normX = lapackf77_dlange( "I", &h_M[s], &h_N[s], h_Bmagma_tmp, &h_ldb[s], work );
                ...
                for (magma_int_t s=0; s < batchCount; s++) {
                    ...
                    blasf77_daxpy( &NN, &c_neg_one, h_B_tmp, &ione, h_X_tmp, &ione );
                    normR = lapackf77_dlange( "I", &h_M[s], &h_N[s], h_X_tmp,       &h_ldb[s], work );
                    normX = lapackf77_dlange( "I", &h_M[s], &h_N[s], h_Blapack_tmp, &h_ldb[s], work );
                    normalize = normX*normA;
                    if (normalize == 0)
                        normalize = 1;
                    error = normR / normalize;
