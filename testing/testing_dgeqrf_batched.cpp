/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zgeqrf_batched.cpp, normal z -> d, Thu Jan 25 23:03:19 2024
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            #endif
            #else
            hipblasDgeqrfBatched( opts.handle, int(M), int(N),
            ...
            if (opts.check) {
                ...
                TESTING_CHECK( magma_dmalloc_cpu( &Q,    batchCount*ldq*min_mn ));  // M by K
                TESTING_CHECK( magma_dmalloc_cpu( &R,    batchCount*ldr*N ));       // K by N
