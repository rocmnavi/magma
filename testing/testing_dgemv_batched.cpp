/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zgemv_batched.cpp, normal z -> d, Mon Jul 15 16:58:04 2024
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    double *h_A, *h_X, *h_Y, *h_Ymagma, *h_Ydevice;
    double *d_A, *d_X, *d_Y;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &h_Ydevice, sizeY  ));
            ...
            const double** dA_array = (const double**) d_A_array;
            const double** dX_array = (const double**) d_X_array;
            const double* dA = (const double*) d_A;
            const double* dX = (const double*) d_X;
            ...
            if( opts.version == 1 ) {
                ...
                magmablas_dgemv_batched_strided(opts.transA, M, N,
            ...
            if(opts.version == 1) {
                ...
                cublasDgemvBatched(opts.handle, cublas_trans_const(opts.transA),
                                      M, N,
                                      (const double *)&alpha,
                                      (const double **)d_A_array, ldda,
                                      (const double **)d_X_array, incx,
                                      (const double *)&beta,
                                      (double **)d_Y_array, incy, batchCount);
                #else
                ...
                #endif
                #else
                hipblasDgemvBatched(opts.handle, hipblas_trans_const(opts.transA),
                                      M, N,
                                      (const double *)&alpha,
                                      (const double **)d_A_array, ldda,
                                      (const double **)d_X_array, incx,
                ...
                #ifdef MAGMA_HAVE_CUDA
                #if CUDA_VERSION >= 11070
                cublasDgemvStridedBatched(opts.handle, cublas_trans_const(opts.transA),
                                      M, N,
                                      (const double *)&alpha,
                                      (const double *)d_A, ldda, ldda*N,
                                      (const double *)d_X, incx, incx*Xm,
                                      (const double *)&beta,
                                      (double *)d_Y, incy, incy*Ym, batchCount);
                #else
                for(magma_int_t s = 0; s < batchCount; s++) {
                    magma_dgemv( opts.transA, M, N,
                         alpha, d_A + s*ldda*N,  ldda,
                ...
                #endif
                #else
                hipblasDgemvStridedBatched(opts.handle, hipblas_trans_const(opts.transA),
                                      M, N,
                                      (const double *)&alpha,
                                      (const double *)d_A, ldda, ldda*N,
                                      (const double *)d_X, incx, incx*Xm,
            ...
            magma_dgetvector( Ym*batchCount, d_Y, incy, h_Ydevice, incy, opts.queue );
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++){
                    ...
                    blasf77_daxpy( &Ym, &c_neg_one, &h_Y[s*Ym*incy], &incy, &h_Ydevice[s*Ym*incy], &incy );
                    error = lapackf77_dlange( "F", &ione, &Ym, &h_Ymagma[s*Ym*incy], &incy, work )
                          / normalize;
                    magma_error = magma_max_nan( error, magma_error );
                    error = lapackf77_dlange( "F", &ione, &Ym, &h_Ydevice[s*Ym*incy], &incy, work )
                          / normalize;
