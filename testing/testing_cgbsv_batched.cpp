/*
   ...
   @generated from testing/testing_zgbsv_batched.cpp, normal z -> c, Mon Oct 28 11:12:46 2024
 */
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int s = 0; s < batchCount; s++) {
                magmaFloatComplex* hA = h_A + s*lda*N;
            ...
            magma_csetmatrix( Nband, N*batchCount,    h_A, lda, d_A, ldda, opts.queue );
            ...
            if(opts.version == 1) {
                ...
                gpu_time = magma_sync_wtime( opts.queue );
                info = magma_cgbsv_batched_strided(
                        N, KL, KU, nrhs,
                        d_A, ldda, ldda*N,
                        dipiv, N,
                        d_B, lddb, lddb*nrhs,
                ...
                // time the async interface only
                gpu_time = magma_sync_wtime( opts.queue );
                info = magma_cgbsv_batched_strided_work(
                        N, KL, KU, nrhs,
            ...
            if (info != 0) {
                printf("magma_cgbsv_batched returned argument error %lld: %s.\n",
                        (long long) info, magma_strerror( info ));
            }
            else {
                for (int i=0; i < batchCount; i++) {
                    if (cpu_info[i] != 0 ) {
                        printf("magma_cgbsv_batched matrix %lld returned internal error %lld\n",
            ...
            for (magma_int_t s=0; s < batchCount; s++) {
                magmaFloatComplex* hA = h_A + s * lda * N + KL;
                magmaFloatComplex* hX = h_X + s * ldb * nrhs;
                magmaFloatComplex* hB = h_B + s * ldb * nrhs;
