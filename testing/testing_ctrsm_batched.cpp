/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_ztrsm_batched.cpp, normal z -> c, Mon Jul 15 16:58:04 2024
...
int main( int argc, char** argv)
{
    ...
    float          error, magma_error, cublas_error, lapack_error, *work;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                normR = lapackf77_clange( "I", &M, &N, h_X      + s*ldb*N, &ldb, work );
                normX = lapackf77_clange( "I", &M, &N, h_Bmagma + s*ldb*N, &ldb, work );
            ...
            for (int s=0; s < batchCount; s++) {
                ...
                blasf77_caxpy( &NN, &c_neg_one, h_B + s*ldb*N, &ione, h_X  + s*ldb*N, &ione );
                normR = lapackf77_clange( "I", &M, &N, h_X  + s*ldb*N,       &ldb, work );
                normX = lapackf77_clange( "I", &M, &N, h_Bcublas  + s*ldb*N, &ldb, work );
                error = normR/(normX*normA);
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++) {
                    ...
                    blasf77_caxpy( &NN, &c_neg_one, h_B + s*ldb*N, &ione, h_X + s*ldb*N, &ione );
                    normR = lapackf77_clange( "I", &M, &N, h_X + s*ldb*N,       &ldb, work );
                    normX = lapackf77_clange( "I", &M, &N, h_Blapack + s*ldb*N, &ldb, work );
                    error = normR/(normX*normA);
