/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_ztrsm.cpp, normal z -> c, Mon Jul 15 16:57:53 2024
...
int main( int argc, char** argv)
{
    ...
    float          magma_error=0, dev_error, lapack_error, *work;
    ...
    magmaFloatComplex alpha = MAGMA_C_ONE; //MAGMA_C_MAKE(  0.29, -0.86 );
    int status = 0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &work,     max(M, N) ));
            ...
            lapackf77_clacpy( MagmaFullStr, &M, &N, hB, &ldb, hBlapack, &ldb );
            lapackf77_clacpy( MagmaFullStr, &M, &N, hB, &ldb, hBmagma,  &ldb );
            ...
            normA = lapackf77_clantr( "I",
                ...
                normR = lapackf77_clange( "I", &M, &N, hX,      &ldb, work );
                normX = lapackf77_clange( "I", &M, &N, hBmagma, &ldb, work );
            ...
            blasf77_caxpy( &sizeB, &c_neg_one, hB, &ione, hX, &ione );
            normR = lapackf77_clange( "I", &M, &N, hX,    &ldb, work );
            normX = lapackf77_clange( "I", &M, &N, hBdev, &ldb, work );
            ...
            if ( opts.lapack ) {
                ...
                blasf77_caxpy( &sizeB, &c_neg_one, hB, &ione, hX, &ione );
                normR = lapackf77_clange( "I", &M, &N, hX,       &ldb, work );
                normX = lapackf77_clange( "I", &M, &N, hBlapack, &ldb, work );
