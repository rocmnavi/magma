/*
   ...
   @generated from testing/testing_zgetrf_vbatched.cpp, normal z -> s, Mon Oct 28 11:12:53 2024
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    magma_int_t     seed = 0;
    float *hA, *hR, *hA_magma, *hTmp;
    float *dA;
    float **dA_array = NULL, **hA_array = NULL, **hR_array = NULL, **hdA_array = NULL;
    ...
    TESTING_CHECK( magma_malloc_cpu((void**)&hR_array,  batchCount * sizeof(float*)) );
    TESTING_CHECK( magma_malloc_cpu((void**)&hdA_array, batchCount * sizeof(float*)) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int s = 0; s < batchCount; s++) {
                ...
                piv_size   += h_min_mn[s];
                gflops     += FLOPS_SGETRF( h_M[s], h_N[s] ) / 1e9;
            }

            TESTING_CHECK( magma_imalloc_cpu( &ipiv,     piv_size ));
            TESTING_CHECK( magma_smalloc_cpu( &hA,       hA_size  ));
            TESTING_CHECK( magma_smalloc_cpu( &hA_magma, hA_size  ));
            TESTING_CHECK( magma_smalloc_pinned( &hR,    hA_size  ));

            TESTING_CHECK( magma_smalloc( &dA,       dA_size ));
            ...
            lapackf77_slarnv( &ione, ISEED, &hA_size, hA );
            ...
            memset(hA_array[id] + col1 * h_lda[id], 0, h_M[id] * sizeof(float));
            memset(hA_array[id] + col2 * h_lda[id], 0, h_M[id] * sizeof(float));
            #endif

            memcpy(hR, hA, hA_size * sizeof(float));

            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            magma_setvector(batchCount, sizeof(float*), hdA_array, 1, dA_array, 1, opts.queue);
            ...
            if(opts.version == 1) {
                ...
                info = magma_sgetrf_vbatched_max_nocheck(
                        d_M, d_N, d_min_mn,
            ...
            hTmp = hA_magma;
            for(int s = 0; s < batchCount; s++) {
                magma_sgetmatrix( h_M[s], h_N[s], hdA_array[s], h_ldda[s], hTmp, h_lda[s], opts.queue );
                hTmp += h_lda[s] * h_N[s];
            }
            ...
            if ( opts.lapack ) {
                ...
                for (magma_int_t s=0; s < batchCount; s++) {
                    magma_int_t locinfo;
                    lapackf77_sgetrf(&h_M[s], &h_N[s], hA_array[s], &h_lda[s], hipiv_array[s], &locinfo);
                    if (locinfo != 0) {
