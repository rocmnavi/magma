/*
   ...
   @generated from testing/testing_zgetrf_vbatched.cpp, normal z -> c, Thu Jan 25 23:03:22 2024
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
get_LU_forward_error(
    ...
    magmaFloatComplex *refLU, magma_int_t ref_lda, magma_int_t *refIPIV,
    magmaFloatComplex *resLU, magma_int_t res_lda, magma_int_t *resIPIV,
    float* error, magma_int_t* pivots_match)
{
    magma_int_t i, j;
    float work[1], matnorm, residual;
    *error = MAGMA_D_ZERO;
    matnorm = lapackf77_clange("f", &M, &N, refLU, &ref_lda, work);

    for( j = 0; j < N; j++ ) {
        for( i = 0; i < M; i++ ) {
            resLU[i+j*res_lda] = MAGMA_C_SUB( resLU[i+j*res_lda], refLU[i+j*ref_lda] );
        }
    }
    residual = lapackf77_clange("f", &M, &N, resLU, &res_lda, work);
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            memset(hA_array[id] + col1 * h_lda[id], 0, h_M[id] * sizeof(magmaFloatComplex));
            memset(hA_array[id] + col2 * h_lda[id], 0, h_M[id] * sizeof(magmaFloatComplex));
            #endif
            ...
            if ( opts.check == 1 ) {
                ...
                magmaFloatComplex *htmp_gpu = hA_magma;
                magmaFloatComplex *htmp_cpu = hA;
