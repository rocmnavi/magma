/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from testing/testing_zhetrf.cpp, normal z -> d, Fri Aug 25 13:17:27 2023
    ...
    real_Double_t *time)
{
    ...
    magma_dgetmatrix(n, n, dA, ldda, A, lda, opts.queue );
    magma_dgetvector(n, dx, 1, x, 1, opts.queue );
    lapackf77_dsytrs( lapack_uplo_const(uplo), &n, &ione, A, &lda, ipiv, x, &n, &info );
    magma_dsetvector(n, x, 1, dx, 1, opts.queue );
    ...
    magma_dsytrs_gpu( uplo, n, ione, dA, ldda, ipiv, dx, n, &info, opts.queue );
    ...
    if (info != 0) {
        printf("magma_dsytrs returned error %lld: %s.\n",
               (long long) info, magma_strerror( info ));
    }
    // reset to original A
    init_matrix( opts, n, n, A, lda );
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (nopiv) {
                ...
                */
                ...
                for(int ll=0; ll<N; ll++){
                    if (MAGMA_D_REAL(h_A[ll+ll*lda])>0.)
                        inert[0]++;
                    else if (MAGMA_D_REAL(h_A[ll+ll*lda])<0.)
                        inert[1]++;
                    else
                        inert[2]++;
                }
                ...
                magma_dsetmatrix(N, N, h_A, lda, d_A, ldda, opts.queue );
                gpu_time = magma_wtime();
                magma_dsytrf_gpu( opts.uplo, N, d_A, ldda, ipiv, &info);
                gpu_time = magma_wtime() - gpu_time;
                ...
                //    h_A[kk+(N-1)*lda] = h_A[N-1+kk*lda] = 0.;
                TESTING_CHECK( magma_malloc( (void**)&dinert, 3*sizeof(int)) );
                magmablas_dsiinertia(opts.uplo, N, d_A, ldda, ipiv, dinert, opts.queue);
                magma_getvector( 3, sizeof(int), dinert, 1, inert, 1, opts.queue );
                printf("inertia: positive / negative / zero = %d / %d / %d\n",
                       inert[0], inert[1], inert[2]);
                magma_free(dinert);
