/*
    -- MAGMA (version 2.0) --
       ...
       @generated from testing/testing_zlacpy_batched.cpp, normal z -> s, Mon Jul 15 16:58:04 2024
       @author Mark Gates
...
int main( int argc, char** argv)
{
    ...
    float **hA_array, **hB_array, **dA_array, **dB_array;
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
        for( int itest = 0; itest < opts.ntest; ++itest ) {
            ...
            if ( uplo[iuplo] == MagmaLower ) {
                ...
                if ( M > N ) {
                    gbytes = batchCount * 2. * sizeof(float) * (1.*M*N - 0.5*N*(N-1)) / 1e9;
                } else {
                    gbytes = batchCount * 2. * sizeof(float) * 0.5*M*(M+1) / 1e9;
                }
            }
            else if ( uplo[iuplo] == MagmaUpper ) {
                // load & save upper trapezoid (with diagonal)
                if ( N > M ) {
                    gbytes = batchCount * 2. * sizeof(float) * (1.*M*N - 0.5*M*(M-1)) / 1e9;
                } else {
                    gbytes = batchCount * 2. * sizeof(float) * 0.5*N*(N+1) / 1e9;
                }
            }
            else {
                // load & save entire matrix
                gbytes = batchCount * 2. * sizeof(float) * 1.*M*N / 1e9;
            }

            TESTING_CHECK( magma_smalloc_cpu( &h_A, batchCount * lda * N ));
            TESTING_CHECK( magma_smalloc_cpu( &h_B, batchCount * ldb * N ));
            TESTING_CHECK( magma_smalloc_cpu( &h_R, batchCount * ldb * N ));
            TESTING_CHECK( magma_smalloc( &d_A, batchCount * ldda*N ));
            TESTING_CHECK( magma_smalloc( &d_B, batchCount * ldda*N ));
            ...
            magmablas_slacpy_batched( uplo[iuplo], M, N, dA_array, ldda, dB_array, lddb, batchCount, opts.queue );
            gpu_time = magma_sync_wtime( opts.queue ) - gpu_time;
            gpu_perf = gbytes / gpu_time;

            magma_sgetmatrix( M, N*batchCount, d_B, lddb, h_R, ldb, opts.queue );
            /* =====================================================================
            ...
            blas_slacpy_batched(uplo[iuplo], M, N, hA_array, lda, hB_array, ldb, batchCount );
            ...
            blasf77_saxpy(&sizeB, &c_neg_one, h_B, &ione, h_R, &ione);
            error = lapackf77_slange("f", &M, &NN, h_R, &ldb, work);
            bool okay = (error == 0);
            status += ! okay;
