/*
   ...
   @generated from testing/testing_zgetrf_batched.cpp, normal z -> d, Thu Jan 25 23:03:19 2024
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            memset(h_A + id*lda*N + col1 * lda, 0, M * sizeof(double));
            memset(h_A + id*lda*N + col2 * lda, 0, M * sizeof(double));
            #endif
            ...
            if (M == N ) {
                ...
                hipblasDgetrfBatched( opts.handle, int(N),
                                     (double**)dA_array, int(ldda), dipiv_device,
                                     dinfo_device, int(batchCount) );
                #endif
            }
            else {
