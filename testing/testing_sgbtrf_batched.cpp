/*
   ...
   @generated from testing/testing_zgbtrf_batched.cpp, normal z -> s, Mon Oct 28 11:12:46 2024
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc( &dA,  lddab * Nband * batchCount ));
            ...
            lapackf77_slacpy( MagmaFullStr, &Mband, &columns, h_A, &ldab, h_R, &ldab );


            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            magma_ssetmatrix( Mband, columns, h_R, ldab, dA, lddab, opts.queue );
            magma_sset_pointer( dA_array, dA, lddab, 0, 0, lddab*Nband, batchCount, opts.queue );
            magma_iset_pointer( dipiv_array, dipiv_magma, 1, 0, 0, min_mn, batchCount, opts.queue );

            if(opts.version == 1) {
                // top-level API accepting ptr arrays
                magma_time = magma_sync_wtime( opts.queue );
                info = magma_sgbtrf_batched(
                        M, N, KL, KU,
                ...
                magma_time = magma_sync_wtime( opts.queue );
                info = magma_sgbtrf_batched_strided(
                        M, N, KL, KU,
                ...
                magma_time = magma_sync_wtime( opts.queue );
                info = magma_sgbtrf_batched_work(
                    M, N, KL, KU,
                    dA_array, lddab, dipiv_array, dinfo_magma,
                    device_work, lwork,
                    batchCount, opts.queue);
                ...
                // timing async call only
                magma_time = magma_sync_wtime( opts.queue );
                info = magma_sgbtrf_batched_strided_work(
                        M, N, KL, KU,
                        dA, lddab, lddab*Nband,
                        dipiv_magma, min_mn,
                        dinfo_magma,
                ...
                magma_time = magma_sync_wtime( opts.queue );
                info = magma_sgbtrf_batched_sliding_window_loopin(
                        M,  N, KL, KU,
            ...
            magma_sgetmatrix( Mband, Nband*batchCount, dA, lddab, h_Amagma, ldab, opts.queue );

            // check correctness of results throught "dinfo_magma" and correctness of argument throught "info"
            magma_getvector( batchCount, sizeof(magma_int_t), dinfo_magma, 1, cpu_info, 1, opts.queue );
            if (info != 0) {
                printf("magma_sgbtrf_batched returned argument error %lld: %s.\n",
                        (long long) info, magma_strerror( info ));
            }
            else {
                for (int i=0; i < batchCount; i++) {
                    if (cpu_info[i] != 0 ) {
                        printf("magma_sgbtrf_batched matrix %lld returned internal error %lld\n",
            ...
            if ( opts.lapack ) {
                ...
                for (magma_int_t s=0; s < batchCount; s++) {
                    magma_int_t locinfo;
                    lapackf77_sgbtrf(&M, &N, &KL, &KU, h_A + s * ldab * Nband, &ldab, ipiv + s * min_mn, &locinfo );
                    if (locinfo != 0) {
                        printf("lapackf77_sgbtrf matrix %lld returned error %lld: %s.\n",
