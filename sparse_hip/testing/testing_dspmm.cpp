/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/testing/testing_zspmm.cpp, normal z -> d, Fri Aug 25 13:18:20 2023
       @author Hartwig Anzt
*/
...
#define hipsparseDcsrmm(handle, op, rows, num_vecs, cols, nnz, alpha, descr, dval, drow, dcol,   \
                       x, ldx, beta, y, ldy)                                                    \
    {                                                                                           \
        hipsparseSpMatDescr_t descrA;                                                            \
        hipsparseDnMatDescr_t descrX, descrY;                                                    \
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        //#ifdef PRECISION_d
        start = magma_sync_wtime( queue );
        TESTING_CHECK( hipsparseCreate( &hipsparseHandle ));
        TESTING_CHECK( hipsparseSetStream( hipsparseHandle, magma_queue_get_hip_stream(queue) ));
        ...
        for (j=0; j < 10; j++) {
            hipsparseDcsrmm(hipsparseHandle,
                           HIPSPARSE_OPERATION_NON_TRANSPOSE,
                           dA.num_rows,   n, dA.num_cols, dA.nnz,
                           (double*)&alpha, descr, (double*)dA.dval, dA.drow, dA.dcol,
                           (double*)dx.dval, dA.num_cols, (double*)&beta, 
                           (double*)dy.dval, dA.num_cols);
        }
        end = magma_sync_wtime( queue );
