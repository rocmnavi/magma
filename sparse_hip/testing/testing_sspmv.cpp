/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/testing/testing_zspmv.cpp, normal z -> s, Mon Oct 28 11:13:21 2024
       @author Hartwig Anzt
*/
    ...
    while( i < argc ) {
        ...
        for (j=0; j < 200; j++) {
            hipsparseScsrmv(hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                               hA.num_rows, hA.num_cols, hA.nnz, (float*)&alpha, descr,
                               (float*)dA.dval, dA.drow, dA.dcol, (float*)dx.dval, 
                               (float*)&beta, (float*)dy.dval);
        }
        end = magma_sync_wtime( queue );
        ...
        TESTING_CHECK( magma_svinit( &dy, Magma_DEV, hA.num_rows, 1, c_zero, queue ));
        hipsparseScsr2hyb(hipsparseHandle,  hA.num_rows, hA.num_cols,
                        descrA, dA.dval, dA.drow, dA.dcol,
                        hybA, 0, HIPSPARSE_HYB_PARTITION_AUTO);

        start = magma_sync_wtime( queue );
        for (j=0; j < 200; j++) {
            TESTING_CHECK( hipsparseShybmv( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                       &alpha, descrA, hybA,
