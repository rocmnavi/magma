/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/testing/testing_zspmv.cpp, normal z -> d, Thu Jan 25 22:58:53 2024
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        start = magma_sync_wtime( queue );
        for (j=0; j < 200; j++) {
            hipsparseDcsrmv(hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                               hA.num_rows, hA.num_cols, hA.nnz, (double*)&alpha, descr,
                               (double*)dA.dval, dA.drow, dA.dcol, (double*)dx.dval, 
                               (double*)&beta, (double*)dy.dval);
        }
        end = magma_sync_wtime( queue );
        ...
        TESTING_CHECK( magma_dvinit( &dy, Magma_DEV, hA.num_rows, 1, c_zero, queue ));
        hipsparseDcsr2hyb(hipsparseHandle,  hA.num_rows, hA.num_cols,
                        descrA, dA.dval, dA.drow, dA.dcol,
                        hybA, 0, HIPSPARSE_HYB_PARTITION_AUTO);

        start = magma_sync_wtime( queue );
        for (j=0; j < 200; j++) {
            TESTING_CHECK( hipsparseDhybmv( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                       &alpha, descrA, hybA,
