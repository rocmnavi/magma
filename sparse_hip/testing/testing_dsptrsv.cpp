/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/testing/testing_zsptrsv.cpp, normal z -> d, Fri Aug 25 13:18:22 2023
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    magma_d_matrix A={Magma_CSR}, a={Magma_CSR}, b={Magma_CSR};
    magma_d_matrix c={Magma_CSR}, d={Magma_CSR};
    magma_int_t dofs;
    double res;
    ...
    while( i < argc ) {
        ...
        if(debug)printf("%% time_magma_d_precondsetup = %.6e\n",tempo2-tempo1 );
        else printf("%.6e\t",tempo2-tempo1 );

        // vectors and initial guess
        TESTING_CHECK( magma_dvinit( &a, Magma_DEV, A.num_rows, 1, one, queue ));
        TESTING_CHECK( magma_dvinit( &b, Magma_DEV, A.num_rows, 1, zero, queue ));
        TESTING_CHECK( magma_dvinit( &c, Magma_DEV, A.num_rows, 1, zero, queue ));
        TESTING_CHECK( magma_dvinit( &d, Magma_DEV, A.num_rows, 1, zero, queue ));
        ...
        // res = norm(d)
        tempo1 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_d_applyprecond_left( MagmaNoTrans, A, a, &b, &zopts.precond_par, queue ));
        tempo2 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_d_spmv( one, zopts.precond_par.L, b, zero, c, queue ));   
        magma_dcopy( dofs, a.dval, 1 , d.dval, 1, queue );
        magma_daxpy( dofs, mone, c.dval, 1 , d.dval, 1, queue );
        res = magma_dnrm2( dofs, d.dval, 1, queue );
        ...
        // res = norm(d)
        tempo1 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_d_applyprecond_right( MagmaNoTrans, A, a, &b, &zopts.precond_par, queue ));
        tempo2 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_d_spmv( one, zopts.precond_par.U, b, zero, c, queue ));   
        magma_dcopy( dofs, a.dval, 1 , d.dval, 1, queue );
        magma_daxpy( dofs, mone, c.dval, 1 , d.dval, 1, queue );
        res = magma_dnrm2( dofs, d.dval, 1, queue );
        ...
        magma_dprecondfree( &zopts.precond_par , queue );
