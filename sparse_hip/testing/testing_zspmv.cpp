...
int main(  int argc, char** argv )
{
    ...
    while( i < argc ) {
        ...
        TESTING_CHECK( hipsparseCreateMatDescr( &descr ));
             
        TESTING_CHECK( hipsparseSetMatType( descr, HIPSPARSE_MATRIX_TYPE_GENERAL ));
        TESTING_CHECK( hipsparseSetMatIndexBase( descr, HIPSPARSE_INDEX_BASE_ZERO ));
        ...
        for (j=0; j < 200; j++) {
            hipsparseZcsrmv(hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                               hA.num_rows, hA.num_cols, hA.nnz, (hipblasDoubleComplex*)&alpha, descr,
                               (hipblasDoubleComplex*)dA.dval, dA.drow, dA.dcol, (hipblasDoubleComplex*)dx.dval, 
                               (hipblasDoubleComplex*)&beta, (hipblasDoubleComplex*)dy.dval);
        }
        end = magma_sync_wtime( queue );
        ...
        hipsparseHybMat_t hybA=NULL;
        TESTING_CHECK( hipsparseCreateMatDescr( &descrA ));
        hipsparseCreateHybMat( &hybA );
        TESTING_CHECK( magma_zvinit( &dy, Magma_DEV, hA.num_rows, 1, c_zero, queue ));
        hipsparseZcsr2hyb(hipsparseHandle,  hA.num_rows, hA.num_cols,
                        descrA, dA.dval, dA.drow, dA.dcol,
                        hybA, 0, HIPSPARSE_HYB_PARTITION_AUTO);

        start = magma_sync_wtime( queue );
        for (j=0; j < 200; j++) {
            TESTING_CHECK( hipsparseZhybmv( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                       &alpha, descrA, hybA,
