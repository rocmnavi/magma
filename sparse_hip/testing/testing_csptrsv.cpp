/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/testing/testing_zsptrsv.cpp, normal z -> c, Thu Jan 25 22:58:54 2024
       @author Hartwig Anzt
*/
...
int main(  int argc, char** argv )
{
    ...
    magma_c_matrix A={Magma_CSR}, a={Magma_CSR}, b={Magma_CSR};
    magma_c_matrix c={Magma_CSR}, d={Magma_CSR};
    magma_int_t dofs;
    float res;
    ...
    while( i < argc ) {
        ...
        // preconditioner with hipsparse trisolve
        ...
        if(debug)printf("%% time_magma_c_precondsetup = %.6e\n",tempo2-tempo1 );
        else printf("%.6e\t",tempo2-tempo1 );

        // vectors and initial guess
        TESTING_CHECK( magma_cvinit( &a, Magma_DEV, A.num_rows, 1, one, queue ));
        TESTING_CHECK( magma_cvinit( &b, Magma_DEV, A.num_rows, 1, zero, queue ));
        TESTING_CHECK( magma_cvinit( &c, Magma_DEV, A.num_rows, 1, zero, queue ));
        TESTING_CHECK( magma_cvinit( &d, Magma_DEV, A.num_rows, 1, zero, queue ));
        ...
        // res = norm(d)
        tempo1 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_c_applyprecond_left( MagmaNoTrans, A, a, &b, &zopts.precond_par, queue ));
        tempo2 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_c_spmv( one, zopts.precond_par.L, b, zero, c, queue ));   
        magma_ccopy( dofs, a.dval, 1 , d.dval, 1, queue );
        magma_caxpy( dofs, mone, c.dval, 1 , d.dval, 1, queue );
        res = magma_scnrm2( dofs, d.dval, 1, queue );
        ...
        // res = norm(d)
        tempo1 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_c_applyprecond_right( MagmaNoTrans, A, a, &b, &zopts.precond_par, queue ));
        tempo2 = magma_sync_wtime( queue );
        TESTING_CHECK( magma_c_spmv( one, zopts.precond_par.U, b, zero, c, queue ));   
        magma_ccopy( dofs, a.dval, 1 , d.dval, 1, queue );
        magma_caxpy( dofs, mone, c.dval, 1 , d.dval, 1, queue );
        res = magma_scnrm2( dofs, d.dval, 1, queue );
        ...
        magma_cprecondfree( &zopts.precond_par , queue );
