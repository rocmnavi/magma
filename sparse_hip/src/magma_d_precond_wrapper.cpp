/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/src/magma_z_precond_wrapper.cpp, normal z -> d, Mon Oct 28 11:13:19 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    // magma_dprecondfree( precond, queue );
    ...
    // ILU and related
        ...
        info = MAGMA_ERR_NOT_SUPPORTED;
        //info = magma_dilut_saad( A, b, precond, queue );
    }
        ...
        /* Here, use No-Dynamic-Parallelism vresion or normal version depending on platform */
            ...
            info = magma_dparilut_gpu_nodp(A, b, precond, queue);
            #else
            info = magma_dparilut_gpu( A, b, precond, queue );
            #endif
            if ( precond->trisolver == Magma_ISAI  ||
                precond->trisolver == Magma_JACOBI ||
                precond->trisolver == Magma_VBJACOBI ){
    ...
    magma_queue_t queue )
{
    ...
    if( trans == MagmaNoTrans ) {
        if ( precond->solver == Magma_JACOBI ) {
            ...
            printf( "error: preconditioner requires OpenMP.\n" );
            info = MAGMA_ERR_NOT_SUPPORTED;
            //CHECK( magma_dilut_saad_apply( b, x, precond, queue ));
        }
        else if ( ( precond->solver == Magma_ICC ||
    ...
    magma_queue_t queue )
{
    ...
    if( trans == MagmaNoTrans ) {
        if ( precond->solver == Magma_JACOBI ) {
            ...
            printf( "error: preconditioner requires OpenMP.\n" );
            info = MAGMA_ERR_NOT_SUPPORTED;
            //magma_dcopy( b.num_rows*b.num_cols, b.dval, 1, x->dval, 1, queue );
        }
        else if ( ( precond->solver == Magma_ICC ||
                    precond->solver == Magma_PARIC ) && 
                  ( precond->trisolver == Magma_CUSOLVE ||
