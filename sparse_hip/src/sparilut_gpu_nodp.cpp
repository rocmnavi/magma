/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/src/zparilut_gpu_nodp.cpp, normal z -> s, Fri Aug 25 13:18:14 2023
*/
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t iters =0; iters<precond->sweeps; iters++) {
        ...
        if (num_rmL>0) {
            CHECK(magma_ssampleselect_nodp(dL.nnz, num_rmL, dL.dval, &thrsL, &selecttmp_ptr, &selecttmp_size, queue));
        } else {
            thrsL = 0.0;
        }
        if (num_rmU>0) {
            CHECK(magma_ssampleselect_nodp(dU.nnz, num_rmU, dU.dval, &thrsU, &selecttmp_ptr, &selecttmp_size, queue));
        } else {
            thrsU = 0.0;
        }
