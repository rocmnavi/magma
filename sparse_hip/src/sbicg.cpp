/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/src/zbicg.cpp, normal z -> s, Mon Jul 15 16:58:19 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smtransfer( A, &Ah1, Magma_DEV, Magma_CPU, queue ));
    CHECK( magma_smconvert( Ah1, &Ah2, A.storage_type, Magma_CSR, queue ));
    magma_smfree(&Ah1, queue );
    CHECK( magma_smtransposeconjugate( Ah2, &Ah1, queue ));
    magma_smfree(&Ah2, queue );
    Ah2.blocksize = A.blocksize;
    Ah2.alignment = A.alignment;
    CHECK( magma_smconvert( Ah1, &Ah2, Magma_CSR, A.storage_type, queue ));
    magma_smfree(&Ah1, queue );
    CHECK( magma_smtransfer( Ah2, &AT, Magma_CPU, Magma_DEV, queue ));
    magma_smfree(&Ah2, queue );
