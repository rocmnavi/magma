/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/src/zbicg.cpp, normal z -> d, Mon Jul 15 16:58:19 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmtransfer( A, &Ah1, Magma_DEV, Magma_CPU, queue ));
    CHECK( magma_dmconvert( Ah1, &Ah2, A.storage_type, Magma_CSR, queue ));
    magma_dmfree(&Ah1, queue );
    CHECK( magma_dmtransposeconjugate( Ah2, &Ah1, queue ));
    magma_dmfree(&Ah2, queue );
    Ah2.blocksize = A.blocksize;
    Ah2.alignment = A.alignment;
    CHECK( magma_dmconvert( Ah1, &Ah2, Magma_CSR, A.storage_type, queue ));
    magma_dmfree(&Ah1, queue );
    CHECK( magma_dmtransfer( Ah2, &AT, Magma_CPU, Magma_DEV, queue ));
    magma_dmfree(&Ah2, queue );
