/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/control/magma_zmconvert.cpp, normal z -> s, Mon Jul 15 16:58:14 2024
       @author Hartwig Anzt
*/
...
#define hipsparseScsr2csc(handle, m, n, nnz, valA, rowA, colA, valB, rowB, colB,                \
                         action, base)                                                         \
    {                                                                                          \
        size_t bufsize;                                                                        \
        void *buf;                                                                             \
        cusparseCsr2cscEx2_bufferSize(handle, m, n, nnz, valA, rowA, colA,                     \
                                      valB, rowB, colB, HIPBLAS_C_64F, action, base,              \
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_CPU )
    {
          ...
          /*      CHECK( magma_smconvert( A, B, Magma_ELL, Magma_CSR, queue ));
            ...
            */
            ...
            // DENSE to CSR
                ...
                B->nnz=0;
                for( magma_int_t i=0; i<(A.num_rows)*(A.num_cols); i++ ) {
                    if ( MAGMA_S_REAL(A.val[i]) != 0.0 || MAGMA_S_IMAG(A.val[i]) != 0.0 )
                        (B->nnz)++;
                }
                CHECK( magma_smalloc_cpu( &B->val, B->nnz));
                CHECK( magma_index_malloc_cpu( &B->row, B->num_rows+1 ));
                ...
                for(i=0; i<(A.num_rows)*(A.num_cols); i++)
                {
                    ...
                    if ( MAGMA_S_REAL(A.val[i]) != 0 || MAGMA_S_IMAG(A.val[i]) != 0)
                    {
                        (B->val)[j] = A.val[i];
        ...
        if ( old_format == Magma_CSR && new_format == Magma_DENSE ) {
            ...
            //CHECK_CUSPARSE( hipsparseSetStream( cusparseHandle, queue->hip_stream() ));
            ...
            //hipsparseScsr2dense( cusparseHandle, A.num_rows, A.num_cols,
        ...
        // DENSE to CSR
            ...
            // hipsparseSnnz( cusparseHandle, HIPSPARSE_DIRECTION_COLUMN,
            ...
            // 
            // // conversion using CUSPARSE
            // hipsparseSdense2csr( cusparseHandle, A.num_rows, A.num_cols,
            //                     descr,
        ...
        // CSR to BCSR
            ...
            hipsparseXcsr2bsrNnz( cusparseHandle, HIPSPARSE_DIRECTION_COLUMN,
            ...
            // conversion using CUSPARSE
            hipsparseScsr2bsr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              A.num_rows, A.num_cols, descr,
                              (float*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (float*)B->dval, B->drow, B->dcol);
        }
        // BCSR to CSR
            ...
            // conversion using CUSPARSE
            hipsparseSbsr2csr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              mb, nb, descr, (float*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (float*)B->dval, B->drow, B->dcol );
        }
        // CSR to CSC
            ...
            // conversion using CUSPARSE
            hipsparseScsr2csc(cusparseHandle, A.num_rows, A.num_cols, A.nnz,
                             (float*)A.dval, A.drow, A.dcol,
                             (float*)B->dval, B->drow, B->dcol,
        ...
        // CSC to CSR
            ...
            // conversion using CUSPARSE
            hipsparseScsr2csc(cusparseHandle, A.num_cols, A.num_rows, A.nnz,
                             (float*)A.dval, A.dcol, A.drow,
                             (float*)B->dval, B->dcol, B->drow,
                             HIPSPARSE_ACTION_NUMERIC,
                             HIPSPARSE_INDEX_BASE_ZERO);
        }
        // CSR to COO
            ...
            // conversion using CUSPARSE
            hipsparseXcsr2coo( cusparseHandle, A.drow,
        ...
        // COO to CSR
            ...
            hipsparseXcoosort_bufferSizeExt(cusparseHandle, m, n, nnz, B->drowidx, B->dcol, &pBufferSizeInBytes);
            CHECK( magma_malloc( &pBuffer, sizeof(char)* pBufferSizeInBytes ));
            // step 2: setup permutation vector P to identity
            CHECK( magma_index_malloc( &P, nnz ));
            hipsparseCreateIdentityPermutation(cusparseHandle, nnz, P);

            // step 3: sort COO format by Row
            hipsparseXcoosortByRow(cusparseHandle, m, n, nnz, B->drowidx, B->dcol, P, pBuffer);
...
#else
            hipsparseSgthr(cusparseHandle, nnz, A.dval, B->dval, P, HIPSPARSE_INDEX_BASE_ZERO);
#endif

            // conversion using CUSPARSE
            hipsparseXcoo2csr( cusparseHandle, B->drowidx,
                              A.nnz, A.num_rows, B->drow,
                              HIPSPARSE_INDEX_BASE_ZERO );
