/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/control/magma_zmconvert.cpp, normal z -> c, Mon Jul 15 16:58:15 2024
       @author Hartwig Anzt
*/
...
#define hipsparseCcsr2csc(handle, m, n, nnz, valA, rowA, colA, valB, rowB, colB,                \
                         action, base)                                                         \
    {                                                                                          \
        size_t bufsize;                                                                        \
        void *buf;                                                                             \
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_CPU )
    {
        ...
        if ( old_format == Magma_CSR && new_format == Magma_DENSE ) {
            ...
            //hipsparseCcsr2dense( cusparseHandle, A.num_rows, A.num_cols,
        ...
        // DENSE to CSR
            ...
            // hipsparseCnnz( cusparseHandle, HIPSPARSE_DIRECTION_COLUMN,
            ...
            // 
            // // conversion using CUSPARSE
            // hipsparseCdense2csr( cusparseHandle, A.num_rows, A.num_cols,
            //                     descr,
        ...
        // CSR to BCSR
            ...
            // conversion using CUSPARSE
            hipsparseCcsr2bsr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              A.num_rows, A.num_cols, descr,
                              (hipblasComplex*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (hipblasComplex*)B->dval, B->drow, B->dcol);
        }
        // BCSR to CSR
            ...
            // conversion using CUSPARSE
            hipsparseCbsr2csr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              mb, nb, descr, (hipblasComplex*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (hipblasComplex*)B->dval, B->drow, B->dcol );
        }
        // CSR to CSC
            ...
            // conversion using CUSPARSE
            hipsparseCcsr2csc(cusparseHandle, A.num_rows, A.num_cols, A.nnz,
                             (hipblasComplex*)A.dval, A.drow, A.dcol,
                             (hipblasComplex*)B->dval, B->drow, B->dcol,
        ...
        // CSC to CSR
            ...
            // conversion using CUSPARSE
            hipsparseCcsr2csc(cusparseHandle, A.num_cols, A.num_rows, A.nnz,
                             (hipblasComplex*)A.dval, A.dcol, A.drow,
                             (hipblasComplex*)B->dval, B->dcol, B->drow,
                             HIPSPARSE_ACTION_NUMERIC,
                             HIPSPARSE_INDEX_BASE_ZERO);
        }
        // CSR to COO
...
#else
            hipsparseCgthr(cusparseHandle, nnz, A.dval, B->dval, P, HIPSPARSE_INDEX_BASE_ZERO);
#endif
