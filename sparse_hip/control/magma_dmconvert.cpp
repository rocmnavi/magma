/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/control/magma_zmconvert.cpp, normal z -> d, Mon Jul 15 16:58:15 2024
       @author Hartwig Anzt
*/
...
#define hipsparseDcsr2csc(handle, m, n, nnz, valA, rowA, colA, valB, rowB, colB,                \
                         action, base)                                                         \
    {                                                                                          \
        size_t bufsize;                                                                        \
        void *buf;                                                                             \
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_CPU )
    {
        ...
        if ( old_format == Magma_CSR && new_format == Magma_DENSE ) {
            ...
            //hipsparseDcsr2dense( cusparseHandle, A.num_rows, A.num_cols,
        ...
        // DENSE to CSR
            ...
            // hipsparseDnnz( cusparseHandle, HIPSPARSE_DIRECTION_COLUMN,
            ...
            // 
            // // conversion using CUSPARSE
            // hipsparseDdense2csr( cusparseHandle, A.num_rows, A.num_cols,
            //                     descr,
        ...
        // CSR to BCSR
            ...
            // conversion using CUSPARSE
            hipsparseDcsr2bsr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              A.num_rows, A.num_cols, descr,
                              (double*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (double*)B->dval, B->drow, B->dcol);
        }
        // BCSR to CSR
            ...
            // conversion using CUSPARSE
            hipsparseDbsr2csr( cusparseHandle, HIPSPARSE_DIRECTION_ROW,
                              mb, nb, descr, (double*)A.dval, A.drow, A.dcol,
                              size_b, descr,
                              (double*)B->dval, B->drow, B->dcol );
        }
        // CSR to CSC
            ...
            // conversion using CUSPARSE
            hipsparseDcsr2csc(cusparseHandle, A.num_rows, A.num_cols, A.nnz,
                             (double*)A.dval, A.drow, A.dcol,
                             (double*)B->dval, B->drow, B->dcol,
        ...
        // CSC to CSR
            ...
            // conversion using CUSPARSE
            hipsparseDcsr2csc(cusparseHandle, A.num_cols, A.num_rows, A.nnz,
                             (double*)A.dval, A.dcol, A.drow,
                             (double*)B->dval, B->dcol, B->drow,
                             HIPSPARSE_ACTION_NUMERIC,
                             HIPSPARSE_INDEX_BASE_ZERO);
        }
        // CSR to COO
...
#else
            hipsparseDgthr(cusparseHandle, nnz, A.dval, B->dval, P, HIPSPARSE_INDEX_BASE_ZERO);
#endif
