/*
    -- MAGMA (version 1.1) --
       ...
       @generated from sparse_hip/control/magma_zmatrix_tools.cpp, normal z -> s, Mon Jul 15 16:58:18 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue)
{
    ...
    U->row[ 0 ] = 0;
    CHECK(magma_smatrix_createrowptr(U->num_rows, U->row, queue));
    ...
    magma_queue_t queue)
{
    ...
    // new row pointer
    L->row[ 0 ] = 0;
    CHECK(magma_smatrix_createrowptr(L->num_rows, L->row, queue));
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t i=0; i < A.nnz; i++) {
        locsum = locsum + (MAGMA_S_ABS(A.val[i]) * MAGMA_S_ABS(A.val[i]));
    }
    ...
    magma_queue_t queue)
{
    ...
    if (A->memory_location == Magma_CPU && A->storage_type == Magma_CSR){
        ...
        for (int row=0; row<A->num_rows; row++) {
            magma_sindexsort(&A->col[A->row[row]], 0, 
