...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/magma_zmcsrcompressor_gpu.hip.cpp, normal z -> c, Fri Aug 25 13:17:54 2023
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    if ( A->memory_location == Magma_DEV && A->storage_type == Magma_CSR ) {
        ...
        hipLaunchKernelGGL(magma_cmcsrgpu_kernel1, dim3(grid1), dim3(BLOCK_SIZE1), 0, queue->hip_stream() ,  A->num_rows, A->dval, A->drow, A->dcol, B.dval, B.drow, B.dcol );

        // correct the row pointer
        dim3 grid2( 1, 1, 1);
        hipLaunchKernelGGL(magma_cmcsrgpu_kernel2, dim3(grid2), dim3(BLOCK_SIZE2), 0, queue->hip_stream() ,  A->num_rows, B.drow, A->drow );
        ...
        hipLaunchKernelGGL(magma_cmcsrgpu_kernel3, dim3(grid1), dim3(BLOCK_SIZE1), 0, queue->hip_stream() ,  A->num_rows, B.dval, B.drow, B.dcol, B2.drow, A->dval, A->drow, A->dcol );
