...
#define hipsparseZcsrsm_analysis(handle, op, rows, nnz, descrA, dval, drow, dcol, info )         \
    {                                                                                           \
        ...
        hipsparseZcsrsm2_bufferSizeExt(handle, 0, op, HIPSPARSE_OPERATION_NON_TRANSPOSE,          \
                                      rows, 1, nnz, (const hipblasDoubleComplex *)&alpha,            \
...
#if CUDA_VERSION >= 11000
#define hipsparseZcsr2csc(handle, cols, rows, nnz, dval, drow, dcol, prdval, prdcol, prdrow,     \
                         action, base)                                                          \
    {                                                                                           \
        size_t bufsize;                                                                         \
        void *buf;                                                                              \
...
#if CUDA_VERSION >= 11000
#define hipsparseZcsric0(handle, op, rows, nnz, descrA, dval, drow, dcol, info )                 \
    {                                                                                           \
        int bufsize;                                                                            \
        void *buf;                                                                              \
        csric02Info_t linfo;                                                                    \
        hipsparseCreateCsric02Info(&linfo);                                                      \
        hipsparseZcsric02_bufferSize(handle, rows, nnz, descrA, dval, drow, dcol,linfo,&bufsize);\
        if (bufsize > 0)                                                                        \
           magma_malloc(&buf, bufsize);                                                         \
        hipsparseZcsric02_analysis(handle, rows, nnz, descrA, dval, drow, dcol, linfo,           \
        ...
        hipsparseZcsric02(handle, rows, nnz, descrA, dval, drow, dcol, linfo,                    \
                         HIPSPARSE_SOLVE_POLICY_NO_LEVEL, buf);                                  \
        if (bufsize > 0)                                                                        \
           magma_free(buf);                                                                     \
    }
...
#else
#define hipsparseZcsric0(handle, op, rows, nnz, descrA, dval, drow, dcol, info )                 \
    CHECK_CUSPARSE( hipsparseZcsric0(handle, op, rows, descrA, dval, drow, dcol, info ))
#endif
    ...
    magma_queue_t queue )
{
    ...
    CHECK_CUSPARSE(
    hipsparseZcsrilu02_bufferSize( hipsparseHandle,
    ...
    CHECK_CUSPARSE( hipsparseZcsrilu02_analysis( hipsparseHandle,
            precond->M.num_rows, precond->M.nnz, descrA,
            precond->M.dval, precond->M.drow, precond->M.dcol,
    ...
    CHECK_CUSPARSE(
    hipsparseZcsrilu02( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer) );
    ...
    hipsparseZcsrsm_analysis( hipsparseHandle,
                             HIPSPARSE_OPERATION_NON_TRANSPOSE,
                             precond->M.num_rows, precond->M.nnz, descrA,
                             precond->M.dval, precond->M.drow, precond->M.dcol,
                             precond->cuinfoILU );
    CHECK_CUSPARSE( hipsparseZcsrilu0( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
    ...
    if( precond->trisolver == Magma_CUSOLVE || precond->trisolver == 0 ){
        ...
        CHECK(magma_ztrisolve_analysis(precond->U, &precond->cuinfoU, true, false, false, queue));
            ...
            hipsparseZcsr2csc(hipsparseHandle, hL.num_cols, 
                             hL.num_rows, hL.nnz,
            ...
            #ifdef MAGMA_HAVE_HIP
            hipsparseZcsr2csc(hipsparseHandle, hU.num_cols, 
                             hU.num_rows, hU.nnz,
    ...
    magma_queue_t queue )
{
    ...
    CHECK_CUSPARSE(
    hipsparseZcsric02_bufferSize( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M,
                         &buffersize ) );
    ...
    CHECK_CUSPARSE( hipsparseZcsric02_analysis( hipsparseHandle,
            precond->M.num_rows, precond->M.nnz, descrA,
            precond->M.dval, precond->M.drow, precond->M.dcol,
            info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer ));
    ...
    CHECK_CUSPARSE(
    hipsparseZcsric02( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer) );    
    ...
    // todo: Zcsric0 needs different analysis (hipsparseZcsric02_analysis)
    hipsparseZcsrsm_analysis( hipsparseHandle,
                             HIPSPARSE_OPERATION_NON_TRANSPOSE,
                             precond->M.num_rows, precond->M.nnz, descrA,
                             precond->M.dval, precond->M.drow, precond->M.dcol,
                             precond->cuinfoILU );
    hipsparseZcsric0( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                     precond->M.num_rows, precond->M.nnz, descrA,
                     precond->M.dval,
                     precond->M.drow,
                     precond->M.dcol,
