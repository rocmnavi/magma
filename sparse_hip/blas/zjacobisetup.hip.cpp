    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(zvjacobisetup_gpu, dim3(grid), dim3(threads), 0, queue->hip_stream(),  num_rows, num_vecs, b.dval, d.dval, c.dval, x->val );
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(zjacobidiagscal_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  num_rows, num_vecs, b.dval, d.dval, c->val );
    ...
    magma_queue_t queue )
{
    ...
    magma_int_t threads = BLOCK_SIZE;
    hipLaunchKernelGGL(zjacobiupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, t.dval, b.dval, d.dval, x->dval );
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        ...
        hipLaunchKernelGGL(zjacobispmvupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        ...
        // hipLaunchKernelGGL(zjacobiupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, t.dval, b.dval, d.dval, x->dval );
        // merged in one implies asynchronous update
        hipLaunchKernelGGL(zjacobispmvupdate_bw_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        hipLaunchKernelGGL(zjacobispmvupdateselect_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, num_updates, indices, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval, tmp.dval );
        //swp.dval = x->dval;
        //x->dval = tmp.dval;
        //tmp.dval = swp.dval;
    }
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(zftjacobicontractions_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  xkm2.num_rows, xkm2.dval, xkm1.dval, xk.dval, z->dval, c->dval );
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(zftjacobiupdatecheck_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  xold->num_rows, delta, xold->dval, xnew->dval, zprev->dval, c.dval, 
                flag_t, flag_fp );
