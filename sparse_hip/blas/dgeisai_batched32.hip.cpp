...
/*
    -- MAGMA (version 1.1) --
       ...
       @generated from sparse_hip/blas/zgeisai_batched32.hip.cpp, normal z -> d, Thu Jan 25 22:58:38 2024
    ...
    magma_queue_t queue )
{
    ...
    if (arch >= 300) {
        if (uplotype == MagmaLower) { //printf("in here lower new kernel\n");
            hipLaunchKernelGGL(magma_dlowerisai_regs_inv_switch, dim3(r2grid), dim3(r2block), 0, queue->hip_stream() , 
                L.num_rows,
                L.row,
                L.col,
                L.val,
            ...
            hipLaunchKernelGGL(magma_dupperisai_regs_inv_switch, dim3(r2grid), dim3(r2block), 0, queue->hip_stream() , 
                L.num_rows,
                L.row,
                L.col,
                L.val,
