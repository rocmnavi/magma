...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zpipelinedgmres.hip.cpp, normal z -> s, Mon Jul 15 16:58:11 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_spipelined_correction, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  n, k, skp, r, v );
    hipLaunchKernelGGL(magma_spipelined_copyscale, dim3(Gs2), dim3(Bs), 0, queue->hip_stream() ,  n, k, skp, r, v );
    ...
    magma_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    hipLaunchKernelGGL(magma_spipelinedsnrm2_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_ceildiv( m, BLOCK_SIZE ) );
    hipLaunchKernelGGL(magma_spipelinesscale, dim3(Gs2), dim3(Bs), 0, queue->hip_stream() ,  m, r, drnorm );
