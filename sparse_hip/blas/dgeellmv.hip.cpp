...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zgeellmv.hip.cpp, normal z -> d, Fri Aug 25 13:17:46 2023
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(dgeellmv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    ...
    magma_queue_t queue )
{
    dim3 grid( magma_ceildiv( m, BLOCK_SIZE ) );
    magma_int_t threads = BLOCK_SIZE;
    hipLaunchKernelGGL(dgeellmv_kernel_shift, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, lambda, dval, dcolind, dx, 
