...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zgeelltmv.hip.cpp, normal z -> c, Mon Oct 28 11:12:56 2024
    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_C_ZERO) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cgeelltmv_kernel<true>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cgeelltmv_kernel<false>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
    ...
    magma_queue_t queue )
{
    ...
    tmp_shift = lambda;
    hipLaunchKernelGGL(cgeelltmv_kernel_shift, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, tmp_shift, dval, dcolind, dx, 
