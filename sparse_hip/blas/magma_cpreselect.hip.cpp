...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/magma_zpreselect.hip.cpp, normal z -> c, Fri Aug 25 13:17:53 2023
    ...
    magma_queue_t queue )
{
    ...
    if( order == 1 ){ // don't copy the first
        hipLaunchKernelGGL(magma_cpreselect_gpu1, dim3(grid), dim3(block), 0, queue->hip_stream(),  A->num_rows, A->drow, A->dval, oneA->dval );
        ...
        hipLaunchKernelGGL(magma_cpreselect_gpu0, dim3(grid), dim3(block), 0, queue->hip_stream(),  A->num_rows, A->drow,  A->dval, oneA->dval );
        // #pragma omp parallel for
        // for( magma_int_t row=0; row<A->num_rows; row++){
