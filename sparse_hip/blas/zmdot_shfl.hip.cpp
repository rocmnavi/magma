    ...
    magma_queue_t queue )
{
    if ( magma_getdevice_arch() < 300 ) {
        ...
        hipLaunchKernelGGL(magma_zblockdot_kernel_shuffle_1dblock, dim3(grid), dim3(block), 32*sizeof(magmaDoubleComplex), queue->hip_stream() ,  n, k, v, r, d1 );
        int j;
        for (j=0; j < k; j++) {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(deviceReduceKernel<magmaDoubleComplex>), dim3(1), dim3(1024), 32*sizeof(magmaDoubleComplex), queue->hip_stream(), d1+grid.x*j, skp+j, grid.x);
        }
    } else {
        ...
        dim3 grid( magma_ceildiv( n, block.x ) );
        hipLaunchKernelGGL(magma_zblockdot_kernel_shuffle, dim3(grid), dim3(block), 32*k*sizeof(magmaDoubleComplex), queue->hip_stream() ,  n, k, v, r, d1 );
        int j;
        for (j=0; j < k; j++) {
            hipLaunchKernelGGL(HIP_KERNEL_NAME(deviceReduceKernel<magmaDoubleComplex>), dim3(1), dim3(1024), 32*sizeof(magmaDoubleComplex), queue->hip_stream(), d1+grid.x*j, skp+j, grid.x);
        }
