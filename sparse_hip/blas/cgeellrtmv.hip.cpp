...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zgeellrtmv.hip.cpp, normal z -> c, Mon Oct 28 11:12:57 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( alignment == 32 ) {
        hipLaunchKernelGGL(cgeellrtmv_kernel_32, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 16 ) {
        hipLaunchKernelGGL(cgeellrtmv_kernel_16, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 8 ) {
        hipLaunchKernelGGL(cgeellrtmv_kernel_8, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else {
