...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zbajac_csr.hip.cpp, normal z -> s, Mon Oct 28 11:12:55 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        hipLaunchKernelGGL(magma_sbajac_csr_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            hipLaunchKernelGGL(magma_sbajac_csr_ls_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  localiters, D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
    }
    else {
