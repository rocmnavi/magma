    ...
    magma_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        hipLaunchKernelGGL(magma_zbajac_csr_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            hipLaunchKernelGGL(magma_zbajac_csr_ls_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  localiters, D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
    }
    else {
