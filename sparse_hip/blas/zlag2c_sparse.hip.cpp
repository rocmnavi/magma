    ...
    magmaFloatComplex_ptr SA )
{
    ...
    if( thread_id < M ){
        for( int i=0; i<N; i++ ){
            SA[i*M+thread_id] = hipComplexDoubleToFloat( A[i*M+thread_id] );
        }
    }
}
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    hipLaunchKernelGGL(magmaint_zlag2c_sparse, dim3(grid), dim3(BLOCKSIZE), 0, queue->hip_stream() ,  M, N, A, SA );
    ...
    magmaFloatComplex *Bval )
{
    ...
    if(row<num_rows){
        ...
        int end = Arow[ row+1 ];
        for( j=start; j<end; j++ )
            Bval[ row*num_rows+Acol[j] ] = hipComplexDoubleToFloat( Aval[ j] );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_DEV && A.storage_type == Magma_CSR) {
        ...
        hipLaunchKernelGGL(magma_zlag2c_CSR_DENSE_kernel, dim3(Bs), dim3(Gs), 0, queue->hip_stream() ,  A.num_rows, A.num_cols, A.dval, A.drow, A.dcol, B->val );
    }
}
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_DEV && A.storage_type == Magma_CSR) {
        ...
        hipLaunchKernelGGL(magma_zlag2c_CSR_DENSE_kernel_1, dim3(Bs), dim3(Gs), 0, queue->hip_stream() ,  A.num_rows, A.num_cols, B->val );
    }
}
    ...
    magma_queue_t queue )
{
    if ( B->memory_location == Magma_DEV && B->storage_type == Magma_DENSE) {
        ...
        hipLaunchKernelGGL(magma_zlag2c_CSR_DENSE_kernel_2, dim3(Bs), dim3(Gs), 0, queue->hip_stream() ,  A.num_rows, A.num_cols, A.dval, A.drow, A.dcol, B->val );
    }
}
