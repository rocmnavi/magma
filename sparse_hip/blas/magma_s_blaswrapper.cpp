/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/magma_z_blaswrapper.cpp, normal z -> s, Fri Aug 25 13:17:44 2023
       @author Hartwig Anzt
...
#else
#define hipsparseScsrmv(handle, op, rows, cols, nnz, alpha, descr, dval, drow, dcol, x, beta, y) \
    CHECK_CUSPARSE(hipsparseScsrmv(handle,op,rows,cols,nnz,alpha,descr,dval,drow,dcol,x,beta,y))
#endif

#if CUDA_VERSION >= 11000
#define hipsparseScsrmm(handle, op, rows, num_vecs, cols, nnz, alpha, descr, dval, drow, dcol,   \
                       x, ldx, beta, y, ldy)                                                    \
    {                                                                                           \
        hipsparseSpMatDescr_t descrA=NULL;                                                       \
    ...
    if ( A.memory_location == Magma_DEV ) {
        if ( A.num_cols == x.num_rows && x.num_cols == 1 ) {
                 ...
                 A.storage_type == Magma_CSRU )
            {
                ...
                hipsparseScsrmv( hipsparseHandle,HIPSPARSE_OPERATION_NON_TRANSPOSE,
                                A.num_rows, A.num_cols, A.nnz, (float*)&alpha, descr,
                                (float*)A.dval, A.drow, A.dcol, (float*)x.dval, (float*)&beta, (float*)y.dval );
            }
            else if ( A.storage_type == Magma_CSC )
            {
                ...
                hipsparseScsrmv( hipsparseHandle,HIPSPARSE_OPERATION_TRANSPOSE,
                              A.num_rows, A.num_cols, A.nnz, (float*)&alpha, descr,
                              (float*)A.dval, A.drow, A.dcol, (float*)x.dval, (float*)&beta, (float*)y.dval );
            }
               ...
               hipsparseHandle = magma_queue_get_hipsparse_handle( queue );
               CHECK_CUSPARSE( hipsparseCreateMatDescr( &descr ));
               hipsparseSbsrmv( hipsparseHandle, dirA,
                   HIPSPARSE_OPERATION_NON_TRANSPOSE, mb, nb, A.numblocks,
                   (float*)&alpha, descr, (float*)A.dval, A.drow, A.dcol, A.blocksize, (float*)x.dval,
                   (float*)&beta, (float*)y.dval );
            }
            else {
            ...
            if ( A.storage_type == Magma_CSR ) {
                ...
                if ( x.major == MagmaColMajor) {
                    hipsparseScsrmm(hipsparseHandle,
                                   HIPSPARSE_OPERATION_NON_TRANSPOSE,
                                   A.num_rows,   num_vecs, A.num_cols, A.nnz,
                                   (float*)&alpha, descr, (float*)A.dval, A.drow, A.dcol,
                                   (float*)x.dval, A.num_cols, (float*)&beta, (float*)y.dval, A.num_cols);
                } else if ( x.major == MagmaRowMajor) {
                    /*hipsparseScsrmm2(hipsparseHandle,
                    HIPSPARSE_OPERATION_NON_TRANSPOSE,
                    ...
                    */
                ...
                } else if ( A.storage_type == Magma_CSC )
                    {
                        ...
                        hipsparseScsrmm( hipsparseHandle,HIPSPARSE_OPERATION_TRANSPOSE,
                                        A.num_rows,   num_vecs, A.num_cols, A.nnz,
                                        (float*)&alpha, descr, (float*)A.dval, A.drow, A.dcol,
                                        (float*)x.dval, A.num_cols, (float*)&beta, (float*)y.dval, A.num_cols);
                    }
