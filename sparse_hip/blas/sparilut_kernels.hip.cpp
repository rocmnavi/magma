...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zparilut_kernels.hip.cpp, normal z -> s, Mon Oct 28 11:13:02 2024
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_sparilut_L_kernel, dim3(grid1), dim3(block1), 0, queue->hip_stream() , A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
        
    hipLaunchKernelGGL(magma_sparilut_U_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() , A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_sparilut_residuals_kernel, dim3(grid1), dim3(block1), 0, queue->hip_stream(), A.num_rows, A.drow, A.dcol, A.dval, 
