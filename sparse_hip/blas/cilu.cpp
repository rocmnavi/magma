/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zilu.cpp, normal z -> c, Mon Jul 15 16:58:11 2024
*/
...
#define hipsparseCcsrsm_analysis(handle, op, rows, nnz, descrA, dval, drow, dcol, info )         \
    {                                                                                           \
        ...
        hipsparseCcsrsm2_bufferSizeExt(handle, 0, op, HIPSPARSE_OPERATION_NON_TRANSPOSE,          \
                                      rows, 1, nnz, (const hipblasComplex *)&alpha,            \
...
#if CUDA_VERSION >= 11000
#define hipsparseCcsr2csc(handle, cols, rows, nnz, dval, drow, dcol, prdval, prdcol, prdrow,     \
                         action, base)                                                          \
    {                                                                                           \
        size_t bufsize;                                                                         \
        void *buf;                                                                              \
...
#if CUDA_VERSION >= 11000
#define hipsparseCcsric0(handle, op, rows, nnz, descrA, dval, drow, dcol, info )                 \
    {                                                                                           \
        int bufsize;                                                                            \
        void *buf;                                                                              \
        csric02Info_t linfo;                                                                    \
        hipsparseCreateCsric02Info(&linfo);                                                      \
        hipsparseCcsric02_bufferSize(handle, rows, nnz, descrA, dval, drow, dcol,linfo,&bufsize);\
        if (bufsize > 0)                                                                        \
           magma_malloc(&buf, bufsize);                                                         \
        hipsparseCcsric02_analysis(handle, rows, nnz, descrA, dval, drow, dcol, linfo,           \
        ...
        hipsparseCcsric02(handle, rows, nnz, descrA, dval, drow, dcol, linfo,                    \
                         HIPSPARSE_SOLVE_POLICY_NO_LEVEL, buf);                                  \
        if (bufsize > 0)                                                                        \
           magma_free(buf);                                                                     \
    }
...
#else
#define hipsparseCcsric0(handle, op, rows, nnz, descrA, dval, drow, dcol, info )                 \
    CHECK_CUSPARSE( hipsparseCcsric0(handle, op, rows, descrA, dval, drow, dcol, info ))
#endif
    ...
    magma_queue_t queue )
{
    ...
    CHECK_CUSPARSE(
    hipsparseCcsrilu02_bufferSize( hipsparseHandle,
    ...
    CHECK_CUSPARSE( hipsparseCcsrilu02_analysis( hipsparseHandle,
            precond->M.num_rows, precond->M.nnz, descrA,
            precond->M.dval, precond->M.drow, precond->M.dcol,
    ...
    CHECK_CUSPARSE(
    hipsparseCcsrilu02( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer) );
    ...
    hipsparseCcsrsm_analysis( hipsparseHandle,
                             HIPSPARSE_OPERATION_NON_TRANSPOSE,
                             precond->M.num_rows, precond->M.nnz, descrA,
                             precond->M.dval, precond->M.drow, precond->M.dcol,
                             precond->cuinfoILU );
    CHECK_CUSPARSE( hipsparseCcsrilu0( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
    ...
    if( precond->trisolver == Magma_CUSOLVE || precond->trisolver == 0 ){
        ...
        CHECK(magma_ctrisolve_analysis(precond->U, &precond->cuinfoU, true, false, false, queue));
            ...
            hipsparseCcsr2csc(hipsparseHandle, hL.num_cols, 
                             hL.num_rows, hL.nnz,
            ...
            #ifdef MAGMA_HAVE_HIP
            hipsparseCcsr2csc(hipsparseHandle, hU.num_cols, 
                             hU.num_rows, hU.nnz,
    ...
    magma_queue_t queue )
{
    ...
    CHECK_CUSPARSE(
    hipsparseCcsric02_bufferSize( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M,
                         &buffersize ) );
    ...
    CHECK_CUSPARSE( hipsparseCcsric02_analysis( hipsparseHandle,
            precond->M.num_rows, precond->M.nnz, descrA,
            precond->M.dval, precond->M.drow, precond->M.dcol,
            info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer ));
    ...
    CHECK_CUSPARSE(
    hipsparseCcsric02( hipsparseHandle,
                         precond->M.num_rows, precond->M.nnz, descrA,
                         precond->M.dval, precond->M.drow, precond->M.dcol,
                         info_M, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, pBuffer) );    
    ...
    // todo: Zcsric0 needs different analysis (hipsparseCcsric02_analysis)
    hipsparseCcsrsm_analysis( hipsparseHandle,
                             HIPSPARSE_OPERATION_NON_TRANSPOSE,
                             precond->M.num_rows, precond->M.nnz, descrA,
                             precond->M.dval, precond->M.drow, precond->M.dcol,
                             precond->cuinfoILU );
    hipsparseCcsric0( hipsparseHandle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
                     precond->M.num_rows, precond->M.nnz, descrA,
                     precond->M.dval,
                     precond->M.drow,
                     precond->M.dcol,
