...
{
    HIP_DYNAMIC_SHARED( magmaDoubleComplex, dot)
    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_Z_ZERO) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgeelltmv_kernel<true>), dim3(grid), dim3(threads), MEM_SIZE, queue->hip_stream() ,  m, n, num_vecs, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgeelltmv_kernel<false>), dim3(grid), dim3(threads), MEM_SIZE, queue->hip_stream() ,  m, n, num_vecs, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
