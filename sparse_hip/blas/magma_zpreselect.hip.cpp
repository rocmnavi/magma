    ...
    magma_queue_t queue )
{
    ...
    if( order == 1 ){ // don't copy the first
        hipLaunchKernelGGL(magma_zpreselect_gpu1, dim3(grid), dim3(block), 0, queue->hip_stream(),  A->num_rows, A->drow, A->dval, oneA->dval );
        ...
        hipLaunchKernelGGL(magma_zpreselect_gpu0, dim3(grid), dim3(block), 0, queue->hip_stream(),  A->num_rows, A->drow,  A->dval, oneA->dval );
        // #pragma omp parallel for
        // for( magma_int_t row=0; row<A->num_rows; row++){
