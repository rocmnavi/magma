...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zpipelinedgmres.hip.cpp, normal z -> d, Fri Aug 25 13:17:51 2023
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_dpipelined_correction, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  n, k, skp, r, v );
    hipLaunchKernelGGL(magma_dpipelined_copyscale, dim3(Gs2), dim3(Bs), 0, queue->hip_stream() ,  n, k, skp, r, v );
    ...
    magma_queue_t queue )
{
    dim3  blocks( 1 );
    dim3 threads( 512 );
    hipLaunchKernelGGL(magma_dpipelineddnrm2_kernel, dim3(blocks), dim3(threads), 0, queue->hip_stream() ,  m, r, lddr, drnorm );

    dim3 Bs( BLOCK_SIZE );
    dim3 Gs2( magma_ceildiv( m, BLOCK_SIZE ) );
    hipLaunchKernelGGL(magma_dpipelinedscale, dim3(Gs2), dim3(Bs), 0, queue->hip_stream() ,  m, r, drnorm );
