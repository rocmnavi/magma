    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zparilut_L_kernel, dim3(grid1), dim3(block1), 0, queue->hip_stream() , A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
        
    hipLaunchKernelGGL(magma_zparilut_U_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() , A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zparilut_residuals_kernel, dim3(grid1), dim3(block1), 0, queue->hip_stream(), A.num_rows, A.drow, A.dcol, A.dval, 
