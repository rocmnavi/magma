...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zgeelltmv.hip.cpp, normal z -> d, Fri Aug 25 13:17:46 2023
    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_D_ZERO) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(dgeelltmv_kernel<true>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(dgeelltmv_kernel<false>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
    ...
    magma_queue_t queue )
{
    ...
    tmp_shift = lambda;
    hipLaunchKernelGGL(dgeelltmv_kernel_shift, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, tmp_shift, dval, dcolind, dx, 
