...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zmergecg.hip.cpp, normal z -> c, Mon Oct 28 11:12:59 2024
       @author Hartwig Anzt
...
{
    HIP_DYNAMIC_SHARED( magmaFloatComplex, temp)    
    int Idx = threadIdx.x;
    int blockSize = 128;
    ...
    magma_int_t alignment  )
{
    ...
    HIP_DYNAMIC_SHARED( magmaFloatComplex, shared)
    ...
    magma_queue_t queue )
{
    ...
    if ( A.storage_type == Magma_CSR )
        hipLaunchKernelGGL(magma_ccgmerge_spmvcsr_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  A.num_rows, A.dval, A.drow, A.dcol, dd, dz, d1 );
    else if ( A.storage_type == Magma_ELLPACKT )
        hipLaunchKernelGGL(magma_ccgmerge_spmvellpack_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  A.num_rows, A.max_nnz_row, A.dval, A.dcol, dd, dz, d1 );
    else if ( A.storage_type == Magma_ELL )
        hipLaunchKernelGGL(magma_ccgmerge_spmvell_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  A.num_rows, A.max_nnz_row, A.dval, A.dcol, dd, dz, d1 );
        ...
        hipsparseCcsrmv( cusparseHandle,HIPSPARSE_OPERATION_NON_TRANSPOSE,
                        A.num_rows, A.num_cols, A.nnz, (hipblasComplex*)&c_one, descr,
                        (hipblasComplex*)A.dval, A.drow, A.dcol, (hipblasComplex*)dd, (hipblasComplex*)&c_zero, (hipblasComplex*)dz );
        hipsparseDestroyMatDescr( descr );
        hipsparseDestroy( cusparseHandle );
        cusparseHandle = 0;
        descr = 0;
        hipLaunchKernelGGL(magma_ccgmerge_spmvellpackrt_kernel2, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  A.num_rows, dz, dd, d1 );
    }
    else if ( A.storage_type == Magma_SELLP && A.alignment == 1 ) {
            hipLaunchKernelGGL(magma_ccgmerge_spmvell_kernelb1, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  A.num_rows, A.blocksize, 
            ...
            if ( A.alignment == 8)
                hipLaunchKernelGGL(magma_ccgmerge_spmvsellpt_kernel_8, dim3(gridsellp), dim3(block), Mssellp, queue->hip_stream() ,  A.num_rows, A.blocksize, A.alignment, 
                    A.dval, A.dcol, A.drow, dd, dz);

            else if ( A.alignment == 16)
                hipLaunchKernelGGL(magma_ccgmerge_spmvsellpt_kernel_16, dim3(gridsellp), dim3(block), Mssellp, queue->hip_stream() ,  A.num_rows, A.blocksize, A.alignment, 
                    A.dval, A.dcol, A.drow, dd, dz);

            else if ( A.alignment == 32)
                hipLaunchKernelGGL(magma_ccgmerge_spmvsellpt_kernel_32, dim3(gridsellp), dim3(block), Mssellp, queue->hip_stream() ,  A.num_rows, A.blocksize, A.alignment, 
                    A.dval, A.dcol, A.drow, dd, dz);
    ...
    if ( A.alignment == 32 ) {
        hipLaunchKernelGGL(magma_ccgmerge_spmvellpackrt_kernel_32, dim3(gridellrt), dim3(num_threads ), Mellrt, queue->hip_stream() ,  A.num_rows, A.dval, A.dcol, A.drow, dd, dz, d1, 
                                                 A.alignment, real_row_length );
    }
    else if ( A.alignment == 16 ) {
        hipLaunchKernelGGL(magma_ccgmerge_spmvellpackrt_kernel_16, dim3(gridellrt), dim3(num_threads ), Mellrt, queue->hip_stream() ,  A.num_rows, A.dval, A.dcol, A.drow, dd, dz, d1, 
                                                 A.alignment, real_row_length );
    }
    else if ( A.alignment == 8 ) {
        hipLaunchKernelGGL(magma_ccgmerge_spmvellpackrt_kernel_8, dim3(gridellrt), dim3(num_threads ), Mellrt, queue->hip_stream() ,  A.num_rows, A.dval, A.dcol, A.drow, dd, dz, d1, 
                                                 A.alignment, real_row_length );
    }
    else {
    ...
    while( Gs.x > 1 ) {
        ...
        hipLaunchKernelGGL(magma_ccgreduce_kernel_spmv1, dim3(Gs_next.x/2), dim3(Bs.x/2), Ms/2, queue->hip_stream(),  Gs.x,  A.num_rows, aux1, aux2 );
    ...
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    hipLaunchKernelGGL(magma_ccg_rhokernel, dim3(Gs2), dim3(Bs2), 0, queue->hip_stream(),  skp );
    ...
    magma_queue_t queue )
{
    ...
    magmaFloatComplex_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    hipLaunchKernelGGL(magma_ccgmerge_xrbeta_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream(),  n, dx, dr, dd, dz, skp, d1);  
    ...
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        hipLaunchKernelGGL(magma_ccgreduce_kernel_spmv1, dim3(Gs_next.x/2), dim3(Bs.x/2), Ms/2, queue->hip_stream(),  Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
        if ( b ) { aux1 = d1; aux2 = d2; }
    ...
    dim3 Bs2( 2 );
    dim3 Gs2( 1 );
    hipLaunchKernelGGL(magma_ccg_alphabetakernel, dim3(Gs2), dim3(Bs2), 0, queue->hip_stream(),  skp );

    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_ceildiv( n, local_block_size ) );
    hipLaunchKernelGGL(magma_ccg_d_kernel, dim3(Gs3), dim3(Bs3), 0, queue->hip_stream(),  n, skp, dr, dd );  
    ...
    magma_queue_t queue )
{
    ...
    dim3 Bs( local_block_size );
    dim3 Gs( magma_ceildiv( n, local_block_size ) );
    hipLaunchKernelGGL(magma_cpcgmerge_xrbeta_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream(),  n, dx, dr, dd, dz, skp );  
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_cmcdotc_one_kernel_1, dim3(Gs), dim3(Bs), Ms, queue->hip_stream(),  n, dr, dh, d1);  

    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        hipLaunchKernelGGL(magma_ccgreduce_kernel_spmv2, dim3(Gs_next.x/2), dim3(Bs.x/2), Ms/2, queue->hip_stream(),  Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
        if ( b ) { aux1 = d1; aux2 = d2; }
    ...
    dim3 Bs3( local_block_size );
    dim3 Gs3( magma_ceildiv( n, local_block_size ) );
    hipLaunchKernelGGL(magma_ccg_d_kernel, dim3(Gs3), dim3(Bs3), 0, queue->hip_stream(),  n, skp, dh, dd );  
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_cjcgmerge_xrbeta_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  n, diag, dx, dr, dd, dz, dh, d1, skp );  
