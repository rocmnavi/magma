...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zgeellrtmv.hip.cpp, normal z -> d, Thu Jan 25 22:58:29 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( alignment == 32 ) {
        hipLaunchKernelGGL(dgeellrtmv_kernel_32, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 16 ) {
        hipLaunchKernelGGL(dgeellrtmv_kernel_16, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 8 ) {
        hipLaunchKernelGGL(dgeellrtmv_kernel_8, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else {
