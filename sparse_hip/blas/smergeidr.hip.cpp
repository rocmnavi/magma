...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zmergeidr.hip.cpp, normal z -> s, Fri Aug 25 13:17:49 2023
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_sidr_smoothing_1_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  num_rows, num_cols, drs, dr, dt );
    ...
    magma_queue_t queue )
{
    dim3 Bs( BLOCK_SIZE );
    dim3 Gs( magma_ceildiv( num_rows, BLOCK_SIZE ) );
    hipLaunchKernelGGL(magma_sidr_smoothing_2_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  num_rows, num_cols, omega, dx, dxs);
