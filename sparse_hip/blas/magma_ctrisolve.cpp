/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/magma_ztrisolve.cpp, normal z -> c, Mon Oct 28 11:13:01 2024
*/
...
magma_int_t magma_ctrisolve_analysis(magma_c_matrix M, magma_solve_info_t *solve_info, bool upper_triangular, bool unit_diagonal, bool transpose, magma_queue_t queue)
{
    magma_int_t info = 0;
...
cleanup:
    ...
    CHECK_CUSPARSE(hipsparseSetMatFillMode(descr, fill_mode));
    {
        ...
        CHECK_CUSPARSE(hipsparseCcsrsm2_bufferSizeExt(hipsparseHandle, algo, M_op, op,
                                                     M.num_rows, 1, M.nnz, (const hipblasComplex*)&one,
                                                     descr, (hipblasComplex*)M.dval, M.drow, M.dcol,
                                                     NULL, M.num_rows,
                                                     solve_info->descr,
        ...
        if (buffer_size > 0)
            magma_malloc(&solve_info->buffer, buffer_size);
        CHECK_CUSPARSE(hipsparseCcsrsm2_analysis(hipsparseHandle, algo, M_op, op,
                                                M.num_rows, 1, M.nnz, (const hipblasComplex*)&one, descr,
                                                (hipblasComplex*)M.dval, M.drow, M.dcol, NULL,
...
magma_int_t magma_ctrisolve(magma_c_matrix M, magma_solve_info_t solve_info, bool upper_triangular, bool unit_diagonal, bool transpose, magma_c_matrix b, magma_c_matrix x, magma_queue_t queue)
{
    magma_int_t info = 0;
...
cleanup:
    ...
    {
        int algo = 0;
        magmablas_clacpy(MagmaFull, M.num_rows, b.num_cols, b.dval, M.num_rows,
                         x.dval, M.num_rows, queue);
        CHECK_CUSPARSE(hipsparseCcsrsm2_solve(hipsparseHandle, algo, M_op, op,
                                             M.num_rows, b.num_cols, M.nnz, (const hipblasComplex*)&one,
                                             descr, (hipblasComplex*)M.dval, M.drow, M.dcol, (hipblasComplex*)x.dval,
                                             M.num_rows, solve_info.descr,
                                             HIPSPARSE_SOLVE_POLICY_NO_LEVEL,
                                             solve_info.buffer));
    }
