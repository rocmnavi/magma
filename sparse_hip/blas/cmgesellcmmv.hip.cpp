...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zmgesellcmmv.hip.cpp, normal z -> c, Mon Jul 15 16:58:11 2024
    ...
    magma_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        if ( alignment == 1) {
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_1_3D_tex<true>), dim3(grid), dim3(block), 0, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
            
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_1_3D_tex<false>), dim3(grid), dim3(block), 0, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
        }
        else if ( alignment == 4) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_4_3D_tex<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_4_3D_tex<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
        }
        else if ( alignment == 8) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_8_3D_tex<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_8_3D_tex<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
        }
        else if ( alignment == 16) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_16_3D_tex<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_16_3D_tex<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
        }
        else if ( alignment == 32) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_32_3D_tex<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_32_3D_tex<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, texdx, beta, dy );
        }
        else {
        ...
        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_1_3D<true>), dim3(grid), dim3(block), 0, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_1_3D<false>), dim3(grid), dim3(block), 0, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
        }
        else if ( alignment == 4) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_4_3D<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_4_3D<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
        }
        else if ( alignment == 8) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_8_3D<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_8_3D<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
        }
        else if ( alignment == 16) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_16_3D<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_16_3D<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
        }
        else if ( alignment == 32) {
            dim3 block( blocksize, alignment, num_vecs/2 );
            if ( beta == MAGMA_C_MAKE( 0.0, 0.0 ) )
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_32_3D<true>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
            else
            hipLaunchKernelGGL(HIP_KERNEL_NAME(zmgesellptmv_kernel_32_3D<false>), dim3(grid), dim3(block), Ms, queue->hip_stream() ,  m, n, num_vecs, blocksize, alignment, alpha,
                dval, dcolind, drowptr, dx, beta, dy );
        }
        else {
            printf("error: alignment %d not supported.\n", int(alignment) );
