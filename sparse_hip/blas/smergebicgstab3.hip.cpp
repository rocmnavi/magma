...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zmergebicgstab3.hip.cpp, normal z -> s, Fri Aug 25 13:17:48 2023
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_sbicgmerge1_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  n, skp, v, r, p );
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_sbicgmerge2_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  n, skp, r, v, s );
    ...
    magma_queue_t queue )
{
    dim3 Bs( BLOCK_SIZE );
    dim3 Gs( magma_ceildiv( n, BLOCK_SIZE ) );
    hipLaunchKernelGGL(magma_sbicgmerge3_kernel, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  n, skp, p, s, t, x, r );
    ...
    magma_queue_t queue )
{
    ...
    dim3 Gs( 1 );
    if ( type == 1 )
        hipLaunchKernelGGL(magma_sbicgmerge4_kernel_1, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  skp );
    else if ( type == 2 )
        hipLaunchKernelGGL(magma_sbicgmerge4_kernel_2, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  skp );
    else if ( type == 3 )
        hipLaunchKernelGGL(magma_sbicgmerge4_kernel_3, dim3(Gs), dim3(Bs), 0, queue->hip_stream() ,  skp );
    else
