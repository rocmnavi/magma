    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magma_zvalinit_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , num_el, dval);
    ...
    magma_queue_t queue)
{
    ...
    dim3 grid(dimgrid1, dimgrid2, dimgrid3);
    dim3 block(blocksize1, blocksize2, 1);
    hipLaunchKernelGGL(magma_zindexinit_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , num_el, dind);
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magma_zmatrixcup_count, dim3(grid1), dim3(block1), 0, queue->hip_stream(), num_rows, A.drow, A.dcol, B.drow, B.dcol, inserted);
    ...
    hipLaunchKernelGGL(magma_zmatrixcup_fill, dim3(grid1), dim3(block1), 0, queue->hip_stream(), num_rows, A.drow, A.dcol, A.dval, B.drow, B.dcol, B.dval,
...
#else
    hipsparseZgthr(handle, A->nnz, (hipblasDoubleComplex*)A->dval, (hipblasDoubleComplex*)csrVal_sorted, P, 
        HIPSPARSE_INDEX_BASE_ZERO);
#endif
