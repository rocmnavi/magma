    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zgpumemzero_z, dim3(grid), dim3(block), 0, queue->hip_stream() , 
   ...
   // magma_zprint_gpu( 32, 32, L.dval, 32, queue );
    ...
    //hipFuncSetCacheConfig(reinterpret_cast<const void*>(hipFuncCachePreferShared);


    if( uplotype == MagmaLower ){
        hipLaunchKernelGGL(magma_zlocations_lower_kernel), dim3(grid), dim3(block), 0, queue->hip_stream() , 
                        LC.num_rows,
                        LC.drow,
                        LC.dcol,
                        LC.dval,
        ...
        hipLaunchKernelGGL(magma_zlocations_upper_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , 
                        LC.num_rows,
                        LC.drow,
                        LC.dcol,
                        LC.dval,
    ...
    hipLaunchKernelGGL(magma_zfilltrisystems_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() , 
                        L.num_rows,
                        L.drow,
                        L.dcol,
                        L.dval,
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zbackinsert_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , 
            M->num_rows,
            M->drow,
            M->dcol,
            M->dval,
