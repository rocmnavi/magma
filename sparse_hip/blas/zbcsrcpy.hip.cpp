    ...
    magma_queue_t queue )
{
    dim3 dimBlock( BLOCK_SIZE, 1, 1 );
        ...
        hipLaunchKernelGGL(zbcsrvalcpy_kernel, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() ,  size_b, num_blocks, Aval, Bval );

        dim3 dimGrid2( dimgrid3, dimgrid1, 1 );

        hipLaunchKernelGGL(zbcsrvalzro_kernel, dim3(dimGrid2), dim3(dimBlock), 0, queue->hip_stream() ,  size_b, num_zblocks, Bval2 );
