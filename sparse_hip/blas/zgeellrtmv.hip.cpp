    ...
    magma_queue_t queue )
{
    ...
    if ( alignment == 32 ) {
        hipLaunchKernelGGL(zgeellrtmv_kernel_32, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 16 ) {
        hipLaunchKernelGGL(zgeellrtmv_kernel_16, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else if ( alignment == 8 ) {
        hipLaunchKernelGGL(zgeellrtmv_kernel_8, dim3(grid), dim3(threads), Ms, queue->hip_stream() ,  m, n, alpha, dval, dcolind, drowlength, dx, beta, dy, 
                                                 alignment, real_row_length );
    }
    else {
