...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/magma_zmatrixtools_gpu.hip.cpp, normal z -> d, Mon Oct 28 11:13:03 2024
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magma_dvalinit_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , num_el, dval);
    ...
    magma_queue_t queue)
{
    ...
    dim3 grid(dimgrid1, dimgrid2, dimgrid3);
    dim3 block(blocksize1, blocksize2, 1);
    hipLaunchKernelGGL(magma_dindexinit_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() , num_el, dind);
    ...
    magma_queue_t queue)
{
    ...
    hipLaunchKernelGGL(magma_dmatrixcup_count, dim3(grid1), dim3(block1), 0, queue->hip_stream(), num_rows, A.drow, A.dcol, B.drow, B.dcol, inserted);
    ...
    hipLaunchKernelGGL(magma_dmatrixcup_fill, dim3(grid1), dim3(block1), 0, queue->hip_stream(), num_rows, A.drow, A.dcol, A.dval, B.drow, B.dcol, B.dval,
...
#else
    hipsparseDgthr(handle, A->nnz, (double*)A->dval, (double*)csrVal_sorted, P, 
        HIPSPARSE_INDEX_BASE_ZERO);
#endif
