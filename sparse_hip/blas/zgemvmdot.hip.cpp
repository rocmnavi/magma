    ...
    magma_queue_t queue )
{
    ...
    if (k>1) {
        hipLaunchKernelGGL(magma_zblockdot_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  Gs.x, n, k, v, r, d1 );
    }
    else {
        hipLaunchKernelGGL(magma_zdot_kernel, dim3(Gs), dim3(Bs), Ms, queue->hip_stream() ,  Gs.x, n, v, r, d1 );
    }
/*
    // not necessary to zero GPU mem
    hipLaunchKernelGGL(magma_zgpumemzero, dim3(Gs), dim3(Bs), 0, queue->hip_stream ,  d1, n*k,1 );
    hipLaunchKernelGGL(magma_zgpumemzero, dim3(Gs), dim3(Bs), 0, queue->hip_stream ,  d2, n*k,1 );
    //magmablas_zlaset( MagmaFull, n, k, d1, n, UNKNOWN );
    //magmablas_zlaset( MagmaFull, n, k, d2, n, UNKNOWN );
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        hipLaunchKernelGGL(magma_zblockreduce_kernel, dim3(Gs_next.x), dim3(Bs.x), Ms, queue->hip_stream ,  Gs.x, n, k, aux1, aux2 );
        Gs.x = Gs_next.x;
        b = 1 - b;
...
*/
    ...
    if ( k>1) {
        while( Gs.x > 1 ) {
            ...
            hipLaunchKernelGGL(magma_zblockreduce_kernel_fast, dim3(Gs_next.x/2), dim3(Bs.x/2), Ms/2, queue->hip_stream() ,  Gs.x, n, k, aux1, aux2 );
        ...
        while( Gs.x > 1 ) {
            Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
            if ( Gs_next.x == 1 ) Gs_next.x = 2;
            hipLaunchKernelGGL(magma_zreduce_kernel_fast, dim3(Gs_next.x/2), dim3(Bs.x/2), Ms/2, queue->hip_stream() ,  Gs.x, n, aux1, aux2 );
            Gs_next.x = Gs_next.x /2;
            Gs.x = Gs_next.x;
            b = 1 - b;
            if ( b ) { aux1 = d1; aux2 = d2; }
