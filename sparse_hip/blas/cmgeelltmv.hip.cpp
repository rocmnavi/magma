...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zmgeelltmv.hip.cpp, normal z -> c, Thu Jan 25 22:58:33 2024
...
{
    HIP_DYNAMIC_SHARED( magmaFloatComplex, dot)
    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_C_ZERO) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cmgeelltmv_kernel<true>), dim3(grid), dim3(threads), MEM_SIZE, queue->hip_stream() ,  m, n, num_vecs, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(cmgeelltmv_kernel<false>), dim3(grid), dim3(threads), MEM_SIZE, queue->hip_stream() ,  m, n, num_vecs, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
