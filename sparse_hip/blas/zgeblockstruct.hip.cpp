    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(magma_zmisai_blockstruct_row_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  A->num_rows, maxbs, A->drow, A->dcol, A->dval );
    ...
    if( uplotype == MagmaLower ){printf("enter here\n");
        hipLaunchKernelGGL(magma_zmisai_blockstruct_fill_l_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  A->num_rows, maxbs, A->drow, A->dcol, A->dval );
    } else {
        hipLaunchKernelGGL(magma_zmisai_blockstruct_fill_u_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  A->num_rows, maxbs, A->drow, A->dcol, A->dval );
    }
