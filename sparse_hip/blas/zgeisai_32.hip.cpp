    ...
    magma_queue_t queue )
{
    ...
    if (arch >= 300) {
        hipLaunchKernelGGL(magma_zgpumemzero_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                rhs, L.num_rows, WARP_SIZE, 1);

        if (uplotype == MagmaLower) {
            hipLaunchKernelGGL(magma_zlocations_lower_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                            M->num_rows,
                            M->drow,
                            M->dcol,
                            M->dval,
            ...
            hipLaunchKernelGGL(magma_zlocations_upper_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                            M->num_rows,
                            M->drow,
                            M->dcol,
                            M->dval,
        ...
        if (uplotype == MagmaLower) {
            ...
            hipLaunchKernelGGL(magma_zlowertrisystems_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                L.num_rows,
                L.drow,
                L.dcol,
                L.dval,
            ...
            hipLaunchKernelGGL(magma_zuppertrisystems_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                L.num_rows,
                L.drow,
                L.dcol,
                L.dval,
        ...
        */
        ...
        for (int z=0; z < recursive; z++) {
            ...
            hipLaunchKernelGGL(magma_zfilltrisystems_32kernel, dim3(r3grid), dim3(r3block), 0, queue->hip_stream() , 
                                32000*z,
                                limit,
                                L.drow,
                                L.dcol,
            ...
            // routine 2
            if (uplotype == MagmaLower) {
                hipLaunchKernelGGL(ztrsv_lower_32kernel_switch, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                        trisystems,
                        rhs+32000*32*z,
                        sizes+32000*z,
                        limit );
                ...
                hipLaunchKernelGGL(ztrsv_upper_32kernel_switch, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                        trisystems,
                        rhs+32000*32*z,
                        sizes+32000*z,
                        limit );
        ...
        // routine 3
        hipLaunchKernelGGL(magma_zbackinsert_32kernel, dim3(r1grid), dim3(r1block), 0, queue->hip_stream() , 
                M->num_rows,
                M->drow,
                M->dcol,
                M->dval,
