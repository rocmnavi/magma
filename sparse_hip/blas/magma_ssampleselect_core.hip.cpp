...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/magma_dsampleselect_core.hip.cpp, normal d -> s, Fri Aug 25 13:18:18 2023
*/
...
__device__ void launch_sampleselect(float* __restrict__ in, float* __restrict__ tmp, float* __restrict__ tree,
    ...
    if (size <= bitonic_cutoff) {
        hipLaunchKernelGGL(select_bitonic_basecase, dim3(1), dim3(bitonic_cutoff), 0, 0, in, out, size, rank);
        return;
    }

    // launch kernels:
    // sample and build searchtree
    hipLaunchKernelGGL(build_searchtree, dim3(1), dim3(sample_size), 0, 0, in, tree, size);
    ...
    // count buckets
    hipLaunchKernelGGL(count_buckets_write, dim3(num_grouped_blocks), dim3(block_size), 0, 0, in, tree, localcounts, oracles, size, local_work);
    hipLaunchKernelGGL(prefix_sum_counts, dim3(searchtree_width), dim3(num_grouped_blocks), 0, 0, localcounts, totalcounts, num_grouped_blocks);
    hipLaunchKernelGGL(sampleselect_findbucket, dim3(1), dim3(searchtree_width / 2), 0, 0, totalcounts, rank, bucket_idx, rank_out);
    hipLaunchKernelGGL(collect_bucket_indirect, dim3(num_grouped_blocks), dim3(block_size), 0, 0, in, oracles, localcounts, tmp, size, bucket_idx, nullptr, local_work);
    hipLaunchKernelGGL(sampleselect_tailcall, dim3(1), dim3(1), 0, 0, tmp, in, tree, count_tmp, out);
#endif
}
...
void launch_sampleselect_nodp(hipStream_t stream, float* __restrict__ in, float* __restrict__ tmp, float* __restrict__ tree,
                                    float* __restrict__ out, int32_t* __restrict__ count_tmp, int32_t size, int32_t rank) {

    if (size <= bitonic_cutoff) {
        hipLaunchKernelGGL(select_bitonic_basecase, dim3(1), dim3(bitonic_cutoff), 0, stream, in, out, size, rank);
    ...
    // launch kernels:
    // sample and build searchtree
    hipLaunchKernelGGL(build_searchtree, dim3(1), dim3(sample_size), 0, stream, in, tree, size);
    ...
    // count buckets
    hipLaunchKernelGGL(count_buckets_write, dim3(num_grouped_blocks), dim3(block_size), 0, stream, in, tree, localcounts, oracles, size, local_work);
    hipLaunchKernelGGL(prefix_sum_counts, dim3(searchtree_width), dim3(num_grouped_blocks), 0, stream, localcounts, totalcounts, num_grouped_blocks);
    hipLaunchKernelGGL(sampleselect_findbucket, dim3(1), dim3(searchtree_width / 2), 0, stream, totalcounts, rank, bucket_idx, rank_out);
    hipLaunchKernelGGL(collect_bucket_indirect, dim3(num_grouped_blocks), dim3(block_size), 0, stream, in, oracles, localcounts, tmp, size, bucket_idx, nullptr, local_work);
    
    sampleselect_tailcall_nodp(stream, tmp, in, tree, count_tmp, out);
}

void sampleselect_tailcall_nodp(hipStream_t stream, float* __restrict__ in, float* __restrict__ tmp, float* __restrict__ tree,
    int32_t* __restrict__ count_tmp, float* __restrict__ out) {
    //if (threadIdx.x != 0) {
    //    return;
    //}
    ...
    */
    ...
    hipStreamSynchronize(stream);
    hipMemcpy((void*)&bi0, bucket_idx, sizeof(bi0), hipMemcpyDeviceToHost);
    hipMemcpy((void*)&size, totalcounts + bi0, sizeof(size), hipMemcpyDeviceToHost);
    hipMemcpy((void*)&rank, rank_out, sizeof(rank), hipMemcpyDeviceToHost);

    launch_sampleselect_nodp(stream, in, tmp, tree, out, count_tmp, size, rank);
}


void sampleselect_nodp(hipStream_t stream, float* __restrict__ in, float* __restrict__ tmp, float* __restrict__ tree,
    int32_t* __restrict__ count_tmp, int32_t size, int32_t rank, float* __restrict__ out) {
