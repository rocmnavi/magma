...
/*
    -- MAGMA (version 1.1) --
       ...
       @generated from sparse_hip/blas/zjacobisetup.hip.cpp, normal z -> c, Thu Jan 25 22:58:30 2024
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(cvjacobisetup_gpu, dim3(grid), dim3(threads), 0, queue->hip_stream(),  num_rows, num_vecs, b.dval, d.dval, c.dval, x->val );
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(cjacobidiagscal_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  num_rows, num_vecs, b.dval, d.dval, c->val );
    ...
    magma_queue_t queue )
{
    ...
    magma_int_t threads = BLOCK_SIZE;
    hipLaunchKernelGGL(cjacobiupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, t.dval, b.dval, d.dval, x->dval );
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        ...
        hipLaunchKernelGGL(cjacobispmvupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        ...
        // hipLaunchKernelGGL(cjacobiupdate_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, t.dval, b.dval, d.dval, x->dval );
        // merged in one implies asynchronous update
        hipLaunchKernelGGL(cjacobispmvupdate_bw_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval );
    }
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<maxiter; i++ ) {
        hipLaunchKernelGGL(cjacobispmvupdateselect_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  t.num_rows, t.num_cols, num_updates, indices, A.dval, A.drow, A.dcol, t.dval, b.dval, d.dval, x->dval, tmp.dval );
        //swp.dval = x->dval;
        //x->dval = tmp.dval;
        //tmp.dval = swp.dval;
    }
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(cftjacobicontractions_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  xkm2.num_rows, xkm2.dval, xkm1.dval, xk.dval, z->dval, c->dval );
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(cftjacobiupdatecheck_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream(),  xold->num_rows, delta, xold->dval, xnew->dval, zprev->dval, c.dval, 
                flag_t, flag_fp );
