...
/*
    -- MAGMA (version 1.1) --
       ...
       @generated from sparse_hip/blas/zgeisai_batched32.hip.cpp, normal z -> s, Mon Jul 15 16:58:14 2024
    ...
    magma_queue_t queue )
{
    ...
    if (arch >= 300) {
        if (uplotype == MagmaLower) { //printf("in here lower new kernel\n");
            hipLaunchKernelGGL(magma_slowerisai_regs_inv_switch, dim3(r2grid), dim3(r2block), 0, queue->hip_stream() , 
                L.num_rows,
                L.row,
                L.col,
                L.val,
            ...
            hipLaunchKernelGGL(magma_supperisai_regs_inv_switch, dim3(r2grid), dim3(r2block), 0, queue->hip_stream() , 
                L.num_rows,
                L.row,
                L.col,
                L.val,
