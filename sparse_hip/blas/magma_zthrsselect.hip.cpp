    ...
    magma_queue_t queue )
{
    ...
    // start = magma_sync_wtime( queue );
    hipLaunchKernelGGL(magma_zfindlargest_kernel, dim3(grid1), dim3(block1), 0, queue->hip_stream(), total_size, val, float_val, thrs1);
    hipLaunchKernelGGL(magma_zreduce_thrs, dim3(grid2), dim3(block1), 0, queue->hip_stream(),  thrs1, thrs2 );
    hipLaunchKernelGGL(magma_zreduce_thrs, dim3(grid3), dim3(block1), 0, queue->hip_stream(),  thrs2, thrs1 );
    hipLaunchKernelGGL(magma_zreduce_thrs, dim3(grid4), dim3(block1), 0, queue->hip_stream(),  thrs1, thrs2 );
...
// magma_zprint_gpu( total_size, 1, val, total_size, queue );
 // magma_dprint_gpu( total_size, 1, ddouble_val, total_size, queue );
    ...
    start = magma_sync_wtime( queue );
    hipLaunchKernelGGL(zthreshselect_kernel, dim3(grid22), dim3(block), 0, queue->hip_stream(), sampling, total_size, subset_size, float_val, thrs[0], thrs1, dummy);
    ...
    // start = magma_sync_wtime( queue );
    // zthreshselect_kernel<<<gridvolta, block, 0, queue->hip_stream()>>>
    //     (sampling, total_size, subset_size, float_val, thrs[0], thrs1, dummy);
    // // second kernel identifies the largest of these thresholds
    // magma_zreduce_thrs<<<gridvolta2, block1, 0, queue->hip_stream()>>>
    //     ( thrs1, thrs2 );
    // magma_zreduce_thrs<<<grid4, block1, 0, queue->hip_stream()>>>
    //      ( thrs2, thrs1 );
    //      
    // end = magma_sync_wtime( queue );
