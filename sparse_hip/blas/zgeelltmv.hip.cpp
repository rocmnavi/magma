    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_Z_ZERO) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zgeelltmv_kernel<true>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(zgeelltmv_kernel<false>), dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
    ...
    magma_queue_t queue )
{
    ...
    tmp_shift = lambda;
    hipLaunchKernelGGL(zgeelltmv_kernel_shift, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, tmp_shift, dval, dcolind, dx, 
