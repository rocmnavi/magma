...
/*
    -- MAGMA (version 2.7.2) --
       ...
       @generated from sparse_hip/blas/zgeisai_maxblock.hip.cpp, normal z -> s, Fri Aug 25 13:17:56 2023
    ...
    magma_queue_t queue )
{
    ...
    // check for pattern L
    hipLaunchKernelGGL(magma_sselect_pattern_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  L.num_rows, 1, L.drow, selections_d, sizes_d );
    // check for pattern L2
    hipLaunchKernelGGL(magma_sselect_pattern_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  L.num_rows, 2, L2.drow, selections_d, sizes_d );
    // check for pattern L3
    hipLaunchKernelGGL(magma_sselect_pattern_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  L.num_rows, 3, L3.drow, selections_d, sizes_d );
    // check for pattern L4
    hipLaunchKernelGGL(magma_sselect_pattern_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  L.num_rows, 4, L4.drow, selections_d, sizes_d );
    // check for pattern L5
    hipLaunchKernelGGL(magma_sselect_pattern_kernel, dim3(grid), dim3(block), 0, queue->hip_stream() ,  L.num_rows, 5, L5.drow, selections_d, sizes_d );

    //now allocate the roptr for MT
    CHECK( magma_index_malloc( &MT->drow, L.num_rows+1 ) );
    // global nonzero count + generate rowptr
    hipLaunchKernelGGL(magma_sselect_rowptr_kernel, dim3(grid0), dim3(block0), 0, queue->hip_stream() ,  L.num_rows, sizes_d, MT->drow );
    ...
    hipLaunchKernelGGL(magma_sselect_insert_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  L.num_rows, 1, 
                L.drow, L.dcol, L.dval,
                MT->drow, MT->dcol, MT->dval,
                selections_d, sizes_d );    
            
    hipLaunchKernelGGL(magma_sselect_insert_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  L.num_rows, 2, 
                L2.drow, L2.dcol, L2.dval,
                MT->drow, MT->dcol, MT->dval,
                selections_d, sizes_d );    
            
    hipLaunchKernelGGL(magma_sselect_insert_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  L.num_rows, 3, 
                L3.drow, L3.dcol, L3.dval,
                MT->drow, MT->dcol, MT->dval,
                selections_d, sizes_d );   
            
    hipLaunchKernelGGL(magma_sselect_insert_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  L.num_rows, 4, 
                L4.drow, L4.dcol, L4.dval,
                MT->drow, MT->dcol, MT->dval,
                selections_d, sizes_d );    
            
    hipLaunchKernelGGL(magma_sselect_insert_kernel, dim3(grid2), dim3(block2), 0, queue->hip_stream() ,  L.num_rows, 5, 
