...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from sparse_hip/blas/zgeellmv.hip.cpp, normal z -> c, Thu Jan 25 22:58:29 2024
    ...
    magma_queue_t queue )
{
    ...
    hipLaunchKernelGGL(cgeellmv_kernel, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    ...
    magma_queue_t queue )
{
    dim3 grid( magma_ceildiv( m, BLOCK_SIZE ) );
    magma_int_t threads = BLOCK_SIZE;
    hipLaunchKernelGGL(cgeellmv_kernel_shift, dim3(grid), dim3(threads), 0, queue->hip_stream() ,  m, n, nnz_per_row, alpha, lambda, dval, dcolind, dx, 
