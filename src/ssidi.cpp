/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/dsidi.cpp, normal d -> s, Mon Oct 28 11:11:46 2024
*/
...
/***************************************************************************//**
    ...
    of a float precision symmetric matrix using the factors from
    ssytrf.
    ...
    A       REAL array, dimension (LDA,N)
            On entry, the output from ssytrf.
    ...
    ipiv    INTEGER array, dimension (N) 
            The pivot vector from ssytrf.

    @param[out]
    det     REAL array, dimension (2)
            Determinant of the original matrix:
            Determinant = det(0) * 10^det(1) 
            with 1<=fabsf(det(0))<10 or det(0) = 0.
    ...
    and ssytrf has set info != 0.

    This routine is based on the LINPACK ssidi routine 
    ...
    *info = 0;
    ...
    if (!nodet || !noert) {
        ...
        for(k = 0; k<n; k++) {
            ...
            if (ipiv[k]-1 < 0) {
                /* 2 by 2 block
                   use det (d  s)  =  (d/t * c - t) * t  ,  t = fabsf(s)
                           (s  c)
            ...
            if (!nodet) {
                det[0] *= d;
                if (det[0] != 0.) {
                    while (fabsf(det[0]) < 1.0) {
                        det[0] *= ten;
                        det[1] -= 1.0;
                    }
                    while (fabsf(det[0]) >= ten) {
                        det[0] /= ten;
                        det[1] += 1.0;
                    }
                }
    ...
    if (!noinv) {
        ...
        while (k < n) {
            ...
            if (ipiv[k]-1 >= 0) {
                ...
                *A(k,k) = 1. / *A(k,k);
                if (k > 0) {
                    blasf77_scopy(&k, A(0, k), &ione, work, &ione);
                    for(j = 0; j<k; j++) {
                        const magma_int_t jp1 = j+1;
                        *A(j,k) = blasf77_sdot(&jp1, A(0,j),&ione, work,&ione);
                        blasf77_saxpy(&j, &work[j], A(0,j),&ione, A(0,k),&ione);
                    }
                    *A(k,k) += blasf77_sdot(&k, work,&ione, A(0,k),&ione);
                }
                kstep = 1;
            } else {
                /* 2 by 2 */
                ...
                *A(k,k+1) = -akkp1/d;
                if (k > 0) {
                    blasf77_scopy(&k, A(0,k+1),&ione, work,&ione);
                    for(j = 0; j<k; j++) {
                        const magma_int_t jp1 = j+1;
                        *A(j,k+1) = blasf77_sdot(&jp1, A(0,j),&ione, work,&ione);
                        blasf77_saxpy(&j, &work[j], A(0,j),&ione, A(0,k+1),&ione);
                    }
                    *A(k+1,k+1) += blasf77_sdot(&k, work,&ione, A(0,k+1),&ione);
                    *A(k  ,k+1) += blasf77_sdot(&k, A(0,k),&ione, A(0,k+1),&ione);
                    blasf77_scopy(&k, A(0,k),&ione, work,&ione);
            ...
            if (ks != k) {
                ...
                blasf77_sswap(&ksp1, A(0,ks),&ione, A(0,k),&ione);
                for(jb = ks; jb<=k; jb++) {
                    j = k + ks - jb;
                    temp = *A(j,k);
                    *A(j,k) = *A(ks,j);
