/*
   ...
   @generated from src/zgetrf_vbatched.cpp, normal z -> c, Fri Aug 25 13:16:49 2023
*/

#include "magma_internal.h"
#include "batched_kernel_param.h"

#define CGETRF2_VBATCHED_PAR_SWAP
...
#define dipiv_array(i_)    dipiv_array, i_
    ...
    for (i = 0; i < max_minmn; i += nb) {
        ...
        // panel
        arginfo = magma_cgetrf_recpanel_vbatched(
                    m, n, minmn,
        ...
        // swap left
        #ifdef CGETRF2_VBATCHED_PAR_SWAP
        ...
        if ( (i + ib) < max_n){
            ...
            if ( (i + ib) < max_m){
                ...
                magmablas_cgemm_vbatched_core(
                        ...
                        m, n, minmn,
                        MAGMA_C_NEG_ONE, dA_array(i+ib, i   ), ldda,
                                         dA_array(i   , i+ib), ldda,
                        MAGMA_C_ONE,     dA_array(i+ib, i+ib), ldda,
                        batchCount, queue );
...
/***************************************************************************//**
    ...
    dA_array    Array of pointers on the GPU, dimension (batchCount).
            Each is a COMPLEX array on the GPU, dimension (LDDA[i],N[i]).
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    // split workspace as needed by magma_cgetrf_vbatched_max_nocheck
    ...
    // call magma_cgetrf_vbatched_max_nocheck
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if (arginfo != 0) {
        ...
        arginfo = magma_cgetrf_vbatched_max_nocheck_work(
                    m, n,
                    max_m, max_n, max_minmn, max_mxn,
                    dA_array, ldda,
