/*
   ...
   @generated from src/zgbtrf_batched.cpp, normal z -> c, Thu Jan 25 23:02:58 2024
*/
...
/***************************************************************************//**
    Purpose
    -------
    CGBTRF computes an LU factorization of a complex m-by-n band matrix AB
   ...
   On entry:                       On exit:
    ...
    dAB_array    Array of pointers, dimension (batchCount).
            Each is a COMPLEX array on the GPU, dimension (LDAB,N)
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(minmn <= 64) {
        ...
        info_fused = magma_cgbtrf_batched_fused_sm(
                        m,  n, kl, ku,
    ...
    if(nb >= n) {
        info_sliding_window = magma_cgbtrf_batched_sliding_window_loopout(
                            m, n, kl, ku,
                            dAB_array, lddab, dipiv_array, info_array,
                            device_work, lwork, batchCount, queue );
    }
    else{
        info_sliding_window = magma_cgbtrf_batched_sliding_window_loopin(
                            m, n, kl, ku,
                            dAB_array, lddab, dipiv_array, info_array,
                            batchCount, queue );
    }
    ...
    for(magma_int_t j = 0; j < minmn; j++) {
        ...
        // adjust ju_array
        magma_cgbtrf_set_fillin(n, kl, ku, dAB_array, lddab, dipiv_array, ju_array, j, batchCount, queue);
...
////////////////////////////////////////////////////////////////////////////////
/// @see magma_cgbtrf_batched_work.
/// This is the (pointer + stride) interface of magma_cgbtrf_batched_work
extern "C" magma_int_t
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magmaFloatComplex** dAB_array   = (magmaFloatComplex**)queue->get_dAarray();
    magma_int_t**      dipiv_array   = (magma_int_t**)queue->get_dBarray();

    // query workspace
    magma_int_t my_work[1] = {-1};
    magma_cgbtrf_batched_work(m, n, kl, ku, NULL, lddab, NULL, NULL, NULL, my_work, batchCount, queue);
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount){
        ...
        magma_cset_pointer(dAB_array,   (magmaFloatComplex*)(dAB + i * strideAB), lddab, 0, 0, strideAB,   batch, queue);
...
/// @see magma_cgbtrf_batched. This is the (pointer + stride) interface of magma_cgbtrf_batched
extern "C" magma_int_t
