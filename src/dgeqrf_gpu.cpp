/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/zgeqrf_gpu.cpp, normal z -> d, Mon Oct 28 11:11:38 2024
*/
...
/***************************************************************************//**
    ...
    nb      INTEGER
            ...
            Users with no specific preference of nb can call magma_get_dgeqrf_nb()
...
magma_dgeqrf_expert_gpu_work(
    magma_int_t m, magma_int_t n,
    magmaDouble_ptr dA, magma_int_t ldda,
    double *tau, magmaDouble_ptr dT,
    magma_int_t *info,
    ...
    magma_queue_t queues[2] )
{
    ...
    if (nb <= 1 || nb >= minmn) {
        h_workspace_bytes += 2*m*n*sizeof(double);
    }
    else {
        h_workspace_bytes += lwork * sizeof(double);
    }
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= minmn) {
        ...
        magma_dgetmatrix(m, n, dA, ldda, work, m, queues[0] );
        lhwork = m*n;
        lapackf77_dgeqrf( &m, &n, work, &m, tau, work+m*n, &lhwork, info );
        magma_dsetmatrix( m, n, work, m, dA, ldda, queues[0] );
        return *info;
    }
