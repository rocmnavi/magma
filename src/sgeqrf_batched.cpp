/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/zgeqrf_batched.cpp, normal z -> s, Mon Jul 15 16:57:35 2024
*/
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magma_int_t use_fused_update = magma_use_sgeqrf_batched_fused_update(m, n, batchCount);
    ...
    ptr_count  = magma_roundup(ptr_count, byte_alignment/sizeof(float*));

    // calculate workspace, if any
    magma_int_t workspace_bytes = 0;
    if(use_fused_update == 0) {
        workspace_bytes += sizeR     * sizeof(float);
        workspace_bytes += sizeT     * sizeof(float);
        workspace_bytes += sizeW     * sizeof(float);
        workspace_bytes += ptr_count * sizeof(float*);
    }
    ...
    // NOTE: to update the tuning of magma_use_sgeqrf_batched_fused_update
    ...
    if( use_fused_update == 1 ) {
        for(int inb = 16; inb >= 1; inb /= 2) {
            arginfo = magma_sgeqrf_panel_fused_update_batched(
                    m, n, inb,
                    dA_array, 0, 0, ldda,
    ...
    magma_sset_pointer( dW_array, dW,    1, 0, 0, nb*n,                 2*batchCount, queue );

    arginfo = magma_sgeqrf_expert_batched(
                m, n, nb,
                dA_array, ldda,
                dR_array, lddr,
                dT_array, lddt,
    ...
    magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magma_sgeqrf_batched_work(m, n, NULL, ldda, NULL, NULL, NULL, device_lwork, batchCount, queue);

    if(device_lwork[0] > 0) {
        magma_malloc((void**)&device_work, device_lwork[0]);
    }

    arginfo = magma_sgeqrf_batched_work( m, n,
