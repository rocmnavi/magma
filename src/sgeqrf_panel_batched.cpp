/*
   ...
   @generated from src/zgeqrf_panel_batched.cpp, normal z -> s, Mon Oct 28 11:12:00 2024
*/
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if( m == n && m <= 32 ){
        ...
        for(magma_int_t j = 0; j < minmn; j+=nb) {
            ...
            magma_int_t sgeqr2_reg_info = magma_sgeqr2_fused_reg_batched(m-j, ib, dA_array, Ai+j, Aj+j, ldda, tau_array, taui+j, info_array, 1, 1, queue );
            magma_int_t sgeqr2_sm_info  = magma_sgeqr2_fused_sm_batched(m-j, ib, dA_array, Ai+j, Aj+j, ldda, tau_array, taui+j, info_array, nthreads, 1, 1, queue );
            magma_int_t slarf_reg_info  = magma_slarf_fused_reg_batched(m-j, n-(j+ib), nb, ib, dA_array, Ai+j, Aj+j+ib, ldda, dA_array, Ai+j, Aj+j, ldda, tau_array, taui+j, 1, 1, queue );
            magma_int_t slarf_sm_info   = magma_slarf_fused_sm_batched(m-j, n-(j+ib), nb, ib, dA_array, Ai+j, Aj+j+ib, ldda, dA_array, Ai+j, Aj+j, ldda, tau_array, taui+j, nthreads, 1, 1, queue );
