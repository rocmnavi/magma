/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/zgetrf_gpu.cpp, normal z -> d, Thu Jan 25 23:02:41 2024
...
/***************************************************************************//**
    ...
    nb      INTEGER
            ...
            Users with no specific preference of nb can call magma_get_dgetrf_nb()
            or magma_get_dgetrf_native_nb() to get the value of nb as determined
    ...
    magma_event_t events[2], magma_queue_t queues[2] )
{
    ...
    if (mode == MagmaHybrid) {
        if ( nb <= 1 || 4*nb >= n ) {
            h_workspace_bytes +=  m * n * sizeof(double); // all of A
        }
        else {
            ldwork = maxm;
            h_workspace_bytes += nb * ldwork * sizeof(double); // dAP on CPU
            d_workspace_bytes += nb * maxm * sizeof(double);   // dAP on GPU
            if( !(m == n) ) {
                d_workspace_bytes += lddat * maxm * sizeof(double); // separate memory for dAT
            }
        }
    }
    else {
        // native workspace
        d_workspace_bytes += nb * maxm * sizeof(double); // dAP
    ...
    *info = 0;
    ...
    // assign pointers
    if( mode == MagmaHybrid ) {
        work = (double*)host_work;
        dAP  = (double*)device_work;
    ...
    if ( nb <= 1 || 4*nb >= min(m,n) ) {
        if (mode == MagmaHybrid) {
            ...
            magma_dgetrf_recpanel_native( m, n, recnb, dA(0,0), ldda, dipiv, dipivinfo, dinfo, 0, events, queues[0], queues[1]);
    ...
    for( j=0; j < minmn-nb; j += nb ) {
        ...
        if (mode == MagmaHybrid) {
            ...
            magma_dgetrf_recpanel_native( rows, nb, recnb, dAP(0,0), maxm, dipiv+j, dipivinfo, dinfo, j, events, queues[0], queues[1]);
    ...
    if ( jb > 0 ) {
        ...
        if (mode == MagmaHybrid) {
            ...
            magma_dgetrf_recpanel_native( rows, jb, recnb, dAP(0,0), maxm, dipiv+j, dipivinfo, dinfo, j, events, queues[1], queues[0]);
    ...
    *info = 0;
    ...
    if (nb <= 1 || 4*nb >= min(m,n) )
        if (mode == MagmaHybrid) {
            ...
            magma_dgetmatrix( m, n, dA(0,0), ldda, work, m, NULL);
            lapackf77_dgetrf( &m, &n, work, &m, ipiv, info );
            magma_dsetmatrix( m, n, work, m, dA(0,0), ldda, NULL);
            magma_free_cpu( work );  work=NULL;
...
/***************************************************************************//**
    ...
    @see magma_dgetrf_expert_gpu_work
    @ingroup magma_getrf
*******************************************************************************/
extern "C" magma_int_t
magma_dgetrf_gpu(
    ...
    magma_int_t *info )
{
    ...
    magma_int_t nb    = magma_get_dgetrf_nb( m, n );
    magma_int_t recnb = 32;
    ...
    magma_int_t *info )
{
    ...
    magma_int_t nb    = magma_get_dgetrf_native_nb( m, n );
    magma_int_t recnb = 32;
