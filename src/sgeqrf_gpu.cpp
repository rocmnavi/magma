/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/zgeqrf_gpu.cpp, normal z -> s, Thu Jan 25 23:02:44 2024
*/
...
/***************************************************************************//**
    ...
    nb      INTEGER
            ...
            Users with no specific preference of nb can call magma_get_sgeqrf_nb()
    ...
    magma_queue_t queues[2] )
{
    ...
    if (nb <= 1 || nb >= minmn) {
        h_workspace_bytes += 2*m*n*sizeof(float);
    }
    else {
        h_workspace_bytes += lwork * sizeof(float);
    }
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= minmn) {
        ...
        magma_sgetmatrix(m, n, dA, ldda, work, m, queues[0] );
        lhwork = m*n;
        lapackf77_sgeqrf( &m, &n, work, &m, tau, work+m*n, &lhwork, info );
        magma_ssetmatrix( m, n, work, m, dA, ldda, queues[0] );
        return *info;
    }
