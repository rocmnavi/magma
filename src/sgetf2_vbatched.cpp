/*
   ...
   @generated from src/zgetf2_vbatched.cpp, normal z -> s, Mon Jul 15 16:57:35 2024
*/
#include "magma_internal.h"
#include "batched_kernel_param.h"

static magma_int_t
magma_sgetf2_vbatched_v1(
...
#define ipiv_array(i) ipiv_array, (i)
    ...
    for(j=0; j < max_minmn; j+=nb) {
        ...
        for(magma_int_t jj = 0; jj < ib; jj++) {
            ...
            // isamax
            magma_isamax_vbatched(max_m-gbj, m, n, dA_array(Ai+gbj, Aj+gbj), ldda, ipiv_array(Ai+gbj), info_array, gbj, gbstep, batchCount, queue);

            // sswap
            magma_sswap_vbatched(max_n, m, n, dA_array(Ai+gbj, Aj), ldda, ipiv_array, gbj, batchCount, queue);

            // scal+ger
            magma_sscal_sger_vbatched( max_m-gbj, ib-jj, m, n, dA_array(Ai+gbj, Aj+gbj), ldda, info_array, gbj, gbstep, batchCount, queue);
        }

        // trsm
        magmablas_strsm_vbatched_core(
            MagmaLeft, MagmaLower, MagmaNoTrans, MagmaUnit,
            ib, max_n-j-ib, m, n, MAGMA_S_ONE,
        ...
        magmablas_sgemm_vbatched_core(
            ...
            m, n, minmn,
            MAGMA_S_NEG_ONE, dA_array, Ai+j+ib, Aj+j,    ldda,
                             dA_array, Ai+j,    Aj+j+ib, ldda,
            MAGMA_S_ONE,     dA_array, Ai+j+ib, Aj+j+ib, ldda,
            batchCount, queue );
    }
    ...
    magma_int_t gbstep, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    arginfo = magma_sgetf2_fused_vbatched(
