/*
    -- MAGMA (version 2.0) --
       ...
       @generated from src/zlarft_batched.cpp, normal z -> c, Mon Jul 15 16:57:35 2024
*/
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magmablas_clarft_ctrmv_sm32x32_batched(
        k, k, tau_array, taui,
        T_array, Ti, Tj, ldt,
        T_array, Ti, Tj, ldt,
        batchCount, queue);
        ...
        magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magma_cgemm_batched_core( MagmaConjTrans, MagmaNoTrans,
                              k, k, n,
    ...
    magmablas_claset_internal_batched(
            MagmaLower, k, k, MAGMA_C_ZERO, MAGMA_C_ZERO,
    ...
    for (j=0; j < k; j += nb) {
        ...
        if (prev_n > 0 && mycol > 0) {
            ...
            for (i=0; i < prev_n; i += nb) {
                ...
                if (rows > 0 && mycol > 0) {
                    magmablas_clarft_recctrmv_sm32x32_batched(
                            rows, mycol,
                            tau_array,      taui+j,
