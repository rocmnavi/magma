/*
   ...
   @generated from src/zgetf2_batched.cpp, normal z -> d, Fri Aug 25 13:16:43 2023
*/
    ...
    for( panelj=0; panelj < min_mn; panelj += nb)
    {
        ...
        for (step=0; step < ib; step++) {
            ...
            if ((m-panelj) > MAX_NTHREADS) {
                ...
                arginfo = magma_idamax_batched(m-gbj, dA_array, ai+gbj, aj+gbj, ldda, 1, ipiv_array, ai+gbj, gbj, gbstep, info_array, batchCount, queue);
                ...
                if (gbj < m) {
                    arginfo = magma_dscal_dger_batched( m-gbj, ib-step, dA_array, ai+gbj, aj+gbj, ldda, info_array, gbj, gbstep, batchCount, queue );
                    if (arginfo != 0 ) return arginfo;
                }
            }
            else {
...
#define ipiv_array(i) ipiv_array, i
    ...
    magma_int_t arginfo = 0;
    if(n <= stop_nb){
        arginfo = magma_dgetf2_fused_batched(m, n, dA_array(ai,aj), ldda, ipiv_array, info_array, batchCount, queue);
    }
    else{
        magma_int_t n1, n2;
        n1 = n/2;
        ...
        // panel 1
        arginfo = magma_dgetf2_batched_v2(
                    m, n1, stop_nb,
                    dA_array(ai,aj), ldda,
    ...
    magma_queue_t queue)
{
    ...
    for(magma_int_t inb = 32; inb >= 2; inb/=2 ) {
        arginfo = magma_dgetf2_batched_v2(m, n, inb, dA_array, ai, aj, ldda, ipiv_array, dpivinfo_array, info_array, batchCount, queue);
        if(arginfo == 0) break;
    }

    // negative arginfo means that fused panel did not launch
    if( arginfo != 0 ) {
        arginfo = magma_dgetf2_batched_v1(m, n, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
    }
